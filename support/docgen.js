...
const logDevices = (devmodels) => {
    ...
    devmodels.forEach((devmodel) => {
        ...
        const pathImg = (iobDevice.icon.startsWith('http')) ? iobDevice.icon : iobDevice.icon.replace(new RegExp('img/', 'g'), '').replace(new RegExp('.png', 'g'), '');
        const icon = (iobDevice.icon.startsWith('http')) ? iobDevice.icon : `(https://github.com/ioBroker/ioBroker.zigbee/blob/master/admin/${iobDevice.icon}`;
        let brand;
        const models = [];
        let zmodels;
