...
async function updateMetaOnAdapter(adapter, devId, meta, metaStates) {
    try {
        ...
        delete existingMeta.mapped;
        delete existingMeta.message;
        delete existingMeta.options;
        delete m.options;
        delete m.mapped;
        delete m.message;
        ...
        for (const key of Object.keys(m)) {
            if (!recursiveObjectCompare(m[key], existingMeta[key])) {
                existingMeta[key] = m[key];
                changed = true;
            }
        }
        if (changed) {
            adapter.log.warn(`Statescontroller updated meta for ${devId} from ${message} to ${JSON.stringify(existingMeta)}`);
            await adapter.extendObject(devId, {native: { meta: existingMeta }});
        }
