...
function adIdtoZbIdorIeee(adapter, source) {
    ...
    if (s.startsWith('group')) {
        return Number(s.substring(6));
    }
    if (s.startsWith('0x')) {
        return s;
    }
    if (s.length === 16 && Number(`0x${s}`)) {
        return `0x${s}`;
    }
    return 'illegal'
}
...
function getNetAddress(address) {
    ...
    if (TcpData) {
        ...
        return {
            strAddress: `tcp://${hostarr.length > 1 ? `${hostarr[0]}:${hostarr[1]}` : hostarr[0]}`,
            host: hostarr[0],
            port: (hostarr.length > 1 ? hostarr[1] : undefined)
        };
    }
    return {};
}
