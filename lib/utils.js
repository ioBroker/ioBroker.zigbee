...
// get ZbId
// In: an Object ID (zigbee.0.deadbeef123455678 or zigbee.0.group_12)
// out: 0xdeadbeef12345678 for a device, 12 for a group
function getZbId(adapterDevId) {
    const idx = adapterDevId.indexOf('group');
    if (idx > 0) return adapterDevId.substr(idx+6);
    const splitDev = adapterDevId.split('.');
    if (splitdev[2]) return splitdev[2];
    return 'IllegalDevId';
}
