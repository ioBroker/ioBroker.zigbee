...
async function updateMetaOnAdapter(adapter, devId, meta, metaStates) {
    try {
        ...
        for (const metastate of metaStates) {
            const isnotempty = isNotEmpty(meta.state[metastate]);
            if (meta.state && typeof meta.state == 'object' && isNotEmpty(meta.state[metastate]))
                writeMeta.state[metastate] = meta.state[metastate];
        }
        if (typeof existingMeta.state === 'object') for (const ems of Object.keys(existingMeta.state))
        {
            if (!writeMeta.state.hasOwnProperty(ems)) writeMeta.state[ems] = undefined;
        }
...
/**
 * Normalise a list of IEEE addresses as entered by the user: an array or a string separated by whitespace,
 * commas or semicolons; '0x' optional, any letter case. Returns the valid addresses as lower-case '0x' strings
 * (a Set, in the order of the input) and the entries that are no IEEE address.
 */
function parseIeeeList(value) {
    const ieees = new Set();
    const invalid = [];
    ...
    for (const entry of entries) {
        ...
        if (trimmed === '') continue;
        const ieee = `0x${trimmed.toLowerCase().replace(/^0x/, '')}`;
        if (/^0x[0-9a-f]{16}$/.test(ieee)) ieees.add(ieee);
        else invalid.push(entry);
    }
    return { ieees, invalid };
}
...
exports.parseIeeeList           = parseIeeeList;