...
class Ota {
                ...
                case 'startMultiOta':
                    {
                        ...
                        for (const target of msg) {
                            await this.startOta({message: {devId: target}});
                        }
                    }
                    break;
                case 'checkMultiOta':
                    this.sendTo(obj.from, obj.command, await this.multiCheckOtaAvail(obj), obj.callback);
                    break;
    ...
    async multiCheckOtaAvail(obj) {
        const msg = obj?.message ?? [];
        const results = {
            available:[],
            upToDate:[],
            deviceCount: msg.length,
            Checked: 0
        };
        for (const target of msg) {
            const result = await this.checkOtaAvail({message: {devId: target}});
            switch (result.status) {
                case 'available':
                    results.available.push(msg);
                    break;
                case 'not_available':
                    results.upToDate.push(msg);
                    break;
            }
            results.checked++;
            const obj = await this.adapter.getObjectAsync('info.ota');
            if (!obj) await this.adapter.setObjectNotExists('info.ota', {
                type: 'state',
                common: {
                    role:'state',
                    name:'OTA Control and info',
                    type:'string',
                    read:'true',
                    write:'true',
                    def:''
                },
                native: {}}, function() { this.adapter.setStateAsync('info.ota', JSON.stringify(results)); });
            else  await this.adapter.setStateAsync('info.ota', JSON.stringify(results));
        }
    ...
    async checkOtaAvail(obj) {
        ...
        if (!device) {
            ...
            if (obj.from) this.adapter.sendTo(obj.from, obj.command, {
                status: 'fail',
        ...
        try {
            ...
            if (obj.from) this.adapter.sendTo(obj.from, obj.command, result, obj.callback);
        } catch (error) {
