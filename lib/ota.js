...
class Ota {
    ...
    async startOtaForEntity(entity) {
        ...
        if (this.inProgress.has(devIeee)) {
            this.error(`Update or check already in progress for '${devLabel(this.adapter, devIeee, entity?.mapped?.model)}', skipping...`);
            return;
        }
        ...
        if (stateObj && stateObj.common && stateObj.common.deactivated) {
            this.info(`Device '${devLabel(this.adapter, devIeee, entity?.mapped?.model)}' is deactivated, skipping ota check`);
            return;
        }
        ...
        if ((availablestate && (!availablestate.val)) || (lqi && lqi.val < 1)) {
            this.info(`Device '${devLabel(this.adapter, devIeee, entity?.mapped?.model)}' is marked unavailable, skipping ota check`);
            return;
        }
        const result = { status: 'unknown', decice:entity?.name ?? 'no name'};
        this.inProgress.add(entity.device.ieeeAddr);
        try {
            this.info('Start firmware update for ' + "'" + devLabel(this.adapter, entity.device.ieeeAddr, entity.mapped?.model) + "'");

            const onProgress = (progress, remaining) => {
                let message = `Update of '${devLabel(this.adapter, entity.device.ieeeAddr, entity.mapped?.model)}' at ${progress}%`;
                if (remaining) {
            ...
            const message = `Update of '${devLabel(this.adapter, entity.device.ieeeAddr, entity.mapped?.model)}' failed (${error.message})`;
            result.status = 'fail';
            result.msg = message;
            this.error(message);
        }
        ...
        try {
            ...
            if (entity?.mapped?.ota) {
                ...
                if (available.currentFileVersion !== available.otaFileVersion) {
                    this.debug(`current Firmware for '${devLabel(this.adapter, ieeeAddr, entity?.mapped?.model)}' is ${available.currentFileVersion} new is ${available.otaFileVersion}`);
                }
            } else {
                result.status = 'not_supported';
            }
            this.debug(`Firmware update for '${devLabel(this.adapter, ieeeAddr, entity?.mapped?.model)}' is ${result.status}`);
        } catch (error) {
            const message = `Failed to check if update available for '${devLabel(this.adapter, ieeeAddr, entity?.mapped?.model)}' ${error.message}`;
            result.status = 'fail';
            result.msg = message;
