...
class Ota {
    ...
    async multiCheckOtaAvail(obj) {
        ...
        for (const target of msg) {
            ...
            if (!obj) {
                await this.adapter.setObjectNotExists('info.ota', {
                    type: 'state',
                    common: {
                        role: 'state',
                    ...
                    this.adapter.setStateAsync('info.ota', JSON.stringify(results));
                });
            } else {
                await this.adapter.setStateAsync('info.ota', JSON.stringify(results));
            }
        }
    ...
    async checkOtaAvail(obj) {
        ...
        try {
            ...
            result.status = 'fail';
            result.msg = message;
            if (!message.includes(`Device didn't respond to OTA request`)) {
                this.warn(message);
            }
            if (obj.from) {
                this.adapter.sendTo(obj.from, obj.command, result, obj.callback);
