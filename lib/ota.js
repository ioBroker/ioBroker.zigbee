...
class Ota {
    ...
    start(zbController, stController) {
        ...
        this.adapter.setStateAsync(`${this.adapter.namespace}.info.ota_avail`, '[]', true);
    ...
    async otafromState(devId, statedesc, command) {
        ...
        if (device) {
            ...
            if (command === 'checkota') await this.checkOtaforDevice(device);
            if (command === 'startota') this.startOtaForDevice(device)
    ...
    async startOtaForDevice(device) {
        ...
        if (stateObj && stateObj.common && stateObj.common.deactivated) {
            this.info(`Device ${devIeee} is deactivated, skipping...`);
            return;
        }
        ...
        if ((availablestate && (!availablestate.val)) || (lqi && lqi.val < 1)) {
            this.info(`Device ${devIeee} is marked unavailable, skipping...`);
            return;
        }
        ...
        await this.adapter.setStateAsync(`${this.adapter.namespace}.info.ota_avail`, JSON.stringify(Array.from(this.otaAvailable).map((ieee) => ieee.replace('0x', ''))), true);
        return result;
    }
    ...
    async checkDeviceObjects(ieeeAddr) {
        ...
        if ((obj ?? true) && obj?.common?.deactivated) {
            this.info(`Device ${ieeeAddr} is deactivated, skipping...`);
            return 'deactivated';
        }
        const availablestate = await this.adapter.getStateAsync(`${objID}.available`);
        const lqi = await this.adapter.getStateAsync(`${objID}.link_quality`);
        if ((availablestate && (!availablestate.val)) || (lqi && lqi.val < 1)) {
            this.info(`Device ${ieeeAddr} is marked unavailable, skipping...`);
            return 'unavailable';
        };
        return '';
    }
    ...
    async checkOtaforDevice(device) {
        ...
        if (otaAvailableSize != this.otaAvailable.size) {
            const arr = Array.from(this.otaAvailable);
            await this.adapter.setStateAsync(`${this.adapter.namespace}.info.ota_avail`, JSON.stringify(Array.from(this.otaAvailable).map((ieee) => ieee.replace('0x', ''))), true);
        }
        return result;
    }
