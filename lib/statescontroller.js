...
class StatesController extends EventEmitter {
    ...
    onStateChange(id, state) {
        ...
        if (state && !state.ack) {
            ...
            if (this.checkDebugDevice(id)) {
                ...
                this.emit('device_debug', { ID:debugId, data: { ID: deviceId, flag:'01' }});
            } else
    ...
    async publishFromState(deviceId, model, stateKey, state, options, debugId) {
        ...
        if (elevated)  {
            this.warn(`ELEVATED O02: Change state '${stateKey}' at device ${deviceId} type '${model}'`);
            this.emit('device_debug', { ID:debugId, data: { ID: deviceId, model: model, flag:'02', IO:false }});
        }

        const devStates = await this.getDevStates(deviceId, model);
        if (!devStates) {
            if (elevated) {
                this.error(`ELEVATED OE1: no device states for device ${deviceId} type '${model}'`);
                this.emit('device_debug', { ID:debugId, data: { error: 'NOSTATES' , IO:false }});
            }
            return;
        }
        ...
        if (!stateDesc) {
            ...
            if (elevated) this.emit('device_debug', { ID:debugId, data: { states:[{id:state.ID, value:'unknown', payload:'unknown'}], error: 'NOSTKEY' , IO:false }});
            return;
        }
        ...
        if (value === undefined || value === '') {
            if (elevated) {
                this.emit('device_debug', { ID:debugId, data: { states:[{id:state.ID, value:'--', payload:'error'}],error: 'NOVAL' , IO:false }});
    ...
    async publishToState(devId, model, payload, debugId) {
        try {
            ...
            if (has_elevated_debug)
            {
                ...
                this.emit('device_debug', { ID:debugId, data: { flag:'01', IO:true }});
            }
            if (!devStates) {
                if (has_elevated_debug) {
                    this.elevatedMessage(devId, `ELEVATED IE02 - NOSTATE: no device states for device ${devId} type '${model}'`, true)
                    this.emit('device_debug', { ID:debugId, data: { error:'NOSTATE',states:[{ id:'--', value:'--', payload:payload}], IO:true }});
                }
                return;
            }
                ...
                try {
                    ...
                    for (const stateInd in states) {
                        ...
                        if (has_elevated_debug) {
                            ...
                            this.emit('device_debug', { ID:debugId, data: { states:[{id:stateID, value:value, payload:payload }],flag:'02', IO:true }});
                        }
                    ...
                    if (has_elevated_debug) {
                        this.elevatedMessage(devId, `ELEVATED IE03 - ETATE: error when enumerating states of ${devId} for payload ${JSON.stringify(payload)}, ${(e ? e.name : 'undefined')} (${(e ? e.message : '')}).`, true);
                        this.emit('device_debug', { ID:debugId, data:{ error:'ESTATE', IO:true }})
                    }
                }
                if (!has_published && has_elevated_debug) {
                    this.elevatedMessage(devId, `ELEVATED IE04 - NOVAL: No value published for device ${devId}`, true);
                    this.emit('device_debug', { ID:debugId, data:{ error:'NOVAL', IO:true }})
                }
            }
            else {
                if (has_elevated_debug) {
                    this.elevatedMessage(devId, `ELEVATED IE05 - NOSTATE: No states matching the payload ${JSON.stringify(payload)} for device ${devId}`, true);
                    this.emit('device_debug', { ID:debugId, data:{ error:'NOSTATE', IO:true }})
                }
            }
        }
        catch (error) {
            this.debug('no message for now');
        }
    }
    ...
    async onZigbeeEvent(type, entity, message) {
        ...
        if (has_elevated_debug) {
            this.warn(`ELEVATED I00: Zigbee Event of Type ${type} from device ${safeJsonStringify(device.ieeeAddr)}, incoming event: ${safeJsonStringify(msgForState)}`);
            this.emit('device_debug', { ID:debugId, data: { ID: device.ieeeAddr, payload:safeJsonStringify(msgForState), flag:'01', IO:true }});
        ...
        if (!converters.length) {
            if (type !== 'readResponse') {
                this.debug(`No converter available for '${mappedModel.model}' '${devId}' with cluster '${cluster}' and type '${type}'`);
                if (has_elevated_debug) {
                    this.warn(`ELEVATED IE00 - NOCONV: No converter available for '${mappedModel.model}' '${devId}' with cluster '${cluster}' and type '${type}'`);
                    this.emit('device_debug', { ID:debugId, data: { error:'NOCONV', IO:true }});
                }
            }
            return;
        }
        ...
        try {
            ...
            if (cluster !== '64529') {
                this.error(`Error while processing converters DEVICE_ID: '${devId}' cluster '${cluster}' type '${type}' : (${error && error.message ? error.message : 'no error message'})`);
            }
        }
        */
