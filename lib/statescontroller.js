    ...
    updateDev(dev_id, dev_name, model, callback) {
       const __dev_name = this.verifyDeviceName(dev_id, dev_name);
        ...
        if (icon.startsWith('http')) {
            this.warn(`download icon from ${icon}`);
            try {           
                const model_modif = model.replace('/','-');
                pathToIcon = this.adapter.adapterDir + '/admin/img/' + model_modif + '.png';
                this.downloadIcon(icon, pathToIcon);          
                icon = 'img/' + model_modif + '.png';
            } catch (e) {
                this.debug(`icon not found from ${icon} saved into ${pathToIcon}`);
            }
        }
