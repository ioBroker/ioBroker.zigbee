...
class StatesController extends EventEmitter {
    ...
    getStashedErrors() {
        ...
        try {
            ...
            {
                rv.errors = this.stashedErrors;
            }
            if (Object.keys(this.stashedUnknownModels).length > 0) {
                rv.unknownModels = this.stashedUnknownModels;
            }
        }
        catch (error) {
    ...
    stashErrors(key, msg, error) {
        if (!this.stashedErrors.hasOwnProperty(key))
        {
            ...
            this.stashedErrors[key] = { message:msg, ts:[Date.now()], error:error };
        }
        else this.stashedErrors[key].ts.push(Date.now());
    }

    stashUnknownModel(model, msg) {
        if (!this.stashedUnknownModels.hasOwnProperty(model)) {
            this.stashedUnknownModels[model] = { message:msg, ts:[Date.now()], error:true };
            this.error(`Unknown ${model}: ${msg}`)
        }
        else this.stashedUnknownModels[model].ts.push(Date.now());
    }
