...
class StatesController extends EventEmitter {
    ...
    async publishToState(devId, model, payload) {
        ...
        let extra_payload = undefined;
        if (payload.hasOwnProperty('action') && typeof payload.action == 'string' && payload.action.endsWith('_release') {
            extra_payload = {'action': payload.action.replace('_release')};
        }
        ...
        if (devStates.states !== undefined) {
            try {
                ...
                for (const stateInd in states) {
                    ...
                    if (extra_payload) {
                        if (statedesc.getter) {
                            extra_value = statedesc.getter(extra_payload);
                        } 
                    }
                    // checking value
                    if (value === undefined || value === null) {
                        if (extra_value != undefined && extra_value != null) {
                            extra_states[stateID] = {'statedesc': statedesc, 'value':extra_value};
                        } 
                        continue;
                    }
                    ...
                    this.TriggerStateUpdate(devId, statedesc, value);
...
*/
                ...
                if (!has_published && extra_states != {}) {
                    foreach (candidateID in extra_states) {
                        const candidate = extra_states[candidateID];
                        if (candidate) {
                            if (has_elevated_debug) {
                                this.warn(`ELEVATED I2a: value generated '${JSON.stringify(candidate.value)}' from device ${devId} for candidate '${candidate.statedesc.name}'`);
                            }
                            TriggerStateUpdatea(devId, candidate.statedesc, candidate.value);
                        }
                    }
                    has_published = true;
                }
    ...
    async TriggerStateUpdate(devId, statedesc, value)
    {
        const common = {
            name: statedesc.name,
            type: statedesc.type,
