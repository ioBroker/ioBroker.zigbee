    ...
    async processConverters(converters, devId, model, mappedModel, message, meta, debugId, has_elevated_debug) {
        ...
        for (const converter of converters) {
            ...
            const payload = await new Promise((resolve, reject) => {
                ...
                if (has_elevated_debug) this.emit('device_debug', {ID:debugId,data: { flag:`02.${cnt}a`, IO:true }, message:` data: ${safeJsonStringify(message.data)} options: ${safeJsonStringify(options)} meta:${safeJsonStringify(metapost)} result:${safeJsonStringify(payloadConv)}`})
                if (typeof payloadConv === 'object') {
                    resolve(payloadConv);
                }
            });
            if (has_elevated_debug) this.emit('device_debug', {ID:debugId,data: { flag:`02.${cnt}b`, IO:true }, message:`converter ${cnt} : Cluster ${converter.cluster} => payload : ${JSON.stringify(payload)}`})
            ...
            if (Object.keys(payload).length > 0 && Object.keys(options).length > 0) {
                ...
                if (has_elevated_debug) this.emit('device_debug', {ID:debugId,data: { flag:`02.${cnt}c`, IO:true }, message:`converter ${cnt} : Cluster ${converter.cluster} => processed payload : ${JSON.stringify(payload)}`})
            }
