...
class StatesController extends EventEmitter {
    ...
    async triggerComposite(_deviceId, model, stateDesc, interactive) {
        ...
        //this.warn(`triggerComposite with ${deviceId} / ${model}, ${key}.`)
        this.compositeData[key] = this.adapter.setTimeout(async () => {
            ...
            this.adapter.getStatesOf(deviceId, parts.join('.'),async (err,states) => {
                if (!err && states) {
                    ...
                    //this.warn(`setting state ${deviceId}.${stateDesc.compositeState} with ${JSON.stringify(components)}`)
                    this.adapter.setState(`${deviceId}.${stateDesc.compositeState}`, JSON.stringify(components));
                }
            })
//            await this.updateState(deviceId, stateDesc.compositeState, JSON.stringify(this.compositeData[key].components));
//            this.warn(`triggerComposite done supported (${deviceId} / ${model}, ${TimeoutHandleName}, ${key}).`)
...
//        this.retainDeviceNames();
    ...
    async updateState(devId, name, value, common) {
        ...
        if (obj) {
            if (!obj.common.deactivated) {
                ...
                const stobj = await this.adapter.getObjectAsync(id);
                ...
                if (stobj) {
                    ...
                    if (id.includes('.')) {
                        const channels = state.split('.');
                        channels.pop();
                        const SubChannels = [obj.id];
                        for (const channel of channels) {
                            SubChannels.push(channel);
                            const id = SubChannels.join('.');
                            const obj = await this.adapter.getObjectAsync(id)
                            if (obj && obj.type == 'channel') continue;
    ...
    async createChannel(devobj, state) {
        if (!devobj && !typeof state == 'string') return;
    }
