...
class StatesController extends EventEmitter {
    ...
    async collectOptions(devId, modelId, callback) {
        let states;
        let result = {};
        // find model states for options and get it values
        const devStates = await this.getDevStates('0x'+devId, modelId);
        if (!devStates) {
            callback(result);
            return;
        }
        states = devStates.states.filter((statedesc) => statedesc.isOption || statedesc.inOptions);
        if (!states) {
            callback(result);
            return;
        }
    ...
    async getDevStates(deviceId, modelId) {
        try {
    ...
    async publishFromState(deviceId, modelId, stateKey, state, options) {
        this.debug(`Change state '${stateKey}' at device ${deviceId} type '${modelId}'`);
        const devStates = await this.getDevStates(deviceId, modelId);
        if (!devStates) {
            return;
        }
        ...
        if (!stateDesc) {
            this.error(`No state available for '${modelId}' with key '${stateKey}'`);
            return;
        }
...
//                        delete new_common.name;
    ...
    async syncDevStates(dev) {
        const devId = dev.ieeeAddr.substr(2),
            modelId = dev.modelID,
            hasGroups = dev.type === 'Router';
        // devId - iobroker device id
        const devStates = await this.getDevStates(dev.ieeeAddr, modelId);
        if (!devStates) {
            return;
        }
    ...
    async publishToState(devId, modelID, payload) {
        const devStates = await this.getDevStates('0x'+devId, modelID);
        if (!devStates) {
            return;
        }
