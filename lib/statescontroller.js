    ...
    async updateState(devId, name, value, common) {
        ...
        if (obj) {
            if (!obj.common.deactivated) {
                ...
                // first check value
                if (value !== undefined) {
                    if (stateId.includes('a4c138f0b824fda5.dis')) {
                        this.warn(`received value A ${value}  for ${stateId} with min ${stobj && stobj.common ? stobj.common.min : 'no state object'} and max ${stobj && stobj.common ? stobj.common.max : 'no state object'}`);
                    }
