...
class StatesController extends EventEmitter {
    ...
    */
    ...
    async downloadIconToAdmin(url, path) {
        this.warn('AAA: downloadToAdmin called with ' + url + ' and ' + path);
        const namespace = `${this.adapter.name}.admin`;
        const src = `${this.adapter.adapterDir}/admin/${path}`;
        ...
        fs.readFile(src, (err, data) => {
            ...
            if (data) {
                this.adapter.writeFile(namespace, path, data, (err) => {
                    if (err) {
                        this.error('error writing file ' + path + JSON.stringify(err))
                        return;
                    }
                    this.warn('wrote file ' + path)
                })
            }
        })
    }
