...
class StatesController extends EventEmitter {
    ...
    leaveDevice(ieeeAddr, model) {
        ...
        if (ieeeAddr) {
            ...
            /* AE: removed - this does not seem to solve the issue, and adds unnecessary code.
            const deleteDelay = this.localConfig.getDeviceOption(devId, model, 'delay_device_delete') || -1;
            if (deleteDelay > 0) {
            if (!delaydDeleteQueue) delayedDeleteQueue = {};
                const deviceDelayObj = this.delayedDeleteQueue[devId] ? this.delayedDeleteQueue[devId] : { };
                if (deviceDelayObj.handle) clearTimeout(deviceDelayObj.handle);
                deviceDelayObj.handle = setTimeout(() => {
                    this.info(`Delete device ${devId} from iobroker after waiting ${deleteDelay} seconds.`);
                    this.stController.deleteObj(devId)
                }, deleteDelay * 1000);
                this.info(`delaying device object removal for ${devId} for ${deleteDelay} seconds`);
                return;
            }
            */
    ...
    async onZigbeeEvent(type, entity, message) {
        ...
        /*if (entity?.options?.filter_state) {
            const state = await this.adapter.getState(`${devId}.state`);
            meta.state.state = state?.val ? 'ON' : 'OFF';
        }
        */
