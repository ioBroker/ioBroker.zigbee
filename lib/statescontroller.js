...
class StatesController extends EventEmitter {
    ...
    async updateState(devId, name, value, common, native) {
        ...
        if (Object.keys(new_common).length || Object.keys(new_native).length != (Object.keys(stobj?.native ?? {}).length)) {
    ...
    /*async updateMeta(devId, meta, metaStates) {
        try {
            const m = meta ?? {};
            const obj = await this.adapter.getObjectAsync(devId);
            ...
            if (!metaStates.includes('state')) {
            try {
                delete m.state.state;
                if (Object.keys(m.state).length() < 1) delete m.state;
