...
//            const stateKey = id.split('.')[3];
    ...
    async publishToState(devId, model, payload) {
        ...
        if (devStates.states !== undefined) {
            ...
            if (model === 'ptvo.switch') {
                this.warn('publishToState caught '+ JSON.stringify(payload) + 'for ' + devId);
            }
            for (const stateInd in states) {
                const statedesc = states[stateInd];
                let value;
                ...
                if (model === 'ptvo.switch') {
                    this.warn(`publishtoState: detected value ${JSON.stringify(value)}`);
                }
                ...
                if (statedesc.isEvent) {
                    ...
                    if (statedesc.prepublish) {
                        ...
                        if (stateInd.hasOwnProperty('createChild')) {
