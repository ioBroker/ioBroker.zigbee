...
class StatesController extends EventEmitter {
        ...
        try {
            ...
            if (model === 'group') {
                ...
                if (!stateModel) {
                    ...
                    if (this.stashUnknownModel(`getDevStates:${deviceId}`,`Model "${model}" not found for Device ${deviceId}`) > 9)
                        this.knownUnknownModels.add(model);
    ...
    async publishToDefinedState(devId, model, statedesc, payload, debugId) {
        ...
        if (statedesc.getter) {
            ...
            value = payload[statedesc.prop] || payload[statedesc.id];
        }
