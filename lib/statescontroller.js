...
class StatesController extends EventEmitter {
    ...
    async publishToState(devId, model, payload) {
        ...
        {
            this.warn(`ELEVATED I01: message received '${JSON.stringify(payload)}' from device ${devId} type '${model}'`);
        }
        if (!devStates) {
            if (has_debug) this.error(`ELEVATED IE02: no device states for device ${devId} type '${model}'`)
            return;
        }
            ...
            try {
                ...
                for (const stateInd in states) {
                    ...
                    if (has_elevated_debug && statedesc.id !== 'msg_from_zigbee') {
                        this.warn(`ELEVATED I02: value generated '${JSON.stringify(value)}' from device ${devId} for '${statedesc.name}'`);
                    }
                ...
                if (has_elevated_debug)
                    this.error(`ELEVATED IE03: error when enumerating states of ${devId} for payload ${JSON.stringify(payload)}, ${(e ? e.name : 'undefined')} (${(e ? e.message : '')}).`);
            }
            if (!has_published && has_elevated_debug) {
                this.error(`ELEVATED IE04: No value published for device ${devId}`);

            }
        }
        else {
            if (has_elevated_debug)
                this.error(`ELEVATED IE05: No states matching the payload ${JSON.stringify(payload)} for device ${devId}`);
        }
    }
