...
class StatesController extends EventEmitter {
    ...
    async toggleDeviceDebug(id) {
        ...
        if (this.debugDevices === undefined) {
            this.debugDevices = await this.getDebugDevices()
        }
        const idx = this.debugDevices.indexOf(stateKey);
        if (idx < 0) {
            this.debugDevices.push(stateKey);
        } else {
            this.debugDevices.splice(idx, 1);
        }
        this.updateDebugDevices()
    ...
    onStateChange(id, state) {
        ...
        if (id.includes('logLevel')) {
            if (state && !state.ack) {
                this.adapter.updateDebugLevel(state.val);
            }
        }
    ...
    async getDevStates(deviceId, model) {
        try {
            ...
            if (this.stashedErrors.knownUnknownModels.has(model)) {
                return {states: modelDefinitions.commonStates};
            }
            if (model === 'group') {
    ...
    stashErrors(key, msg, error) {
        try {
            ...
            this.adapter.setState('info.lasterror', JSON.stringify({
                error: key,
                data: this.stashedErrors.errors[key]
            }), true)
        } catch { /* */
        }
    }
    ...
    stashUnknownModel(model, msg) {
        try {
            if (!this.stashedErrors.unknownModels.hasOwnProperty(model)) {
                ...
                if (this.stashedErrors.unknownModels[model].count == 1) {
                    this.stashedErrors.unknownModels[model].ts.push(Date.now());
                } else {
            ...
            this.adapter.setState('info.lasterror', JSON.stringify({
                model: model,
                data: this.stashedErrors.unknownModels[model]
            }), true)
    ...
    async triggerComposite(_deviceId, stateDesc, interactive, stateDefinitions) {
        ...
        if (this.compositeData[key] == undefined) {
            this.compositeData[key] = {
                triggers: [stateDesc.id],
                stateDefinitions: stateDefinitions.filter((d) => d.compositeState == stateDesc.compositeState)
            };
        } else {
            this.compositeData[key].triggers.push(stateDesc.id);
        }
        ...
        this.compositeData[key].handle = this.adapter.setTimeout(async () => {
            ...
            this.adapter.setState(`${deviceId}.${stateDesc.compositeState}`, JSON.stringify(components), true);
    ...
    handleLinkedFunctResult(lfArr, devId, state) {
        if (this.handleOption(devId, state.stateDesc)) {
            return;
        }
        lfArr.push(state);
    }
    ...
    async handleStateResend(entity, end_with_device_query) {
        ...
        if (end_with_device_query) {
            states.end_with_device_query = true;
        }
    ...
    async publishFromStates(deviceId, model, stateIDs, options, debugID) {
        ...
        for (const stateID of Object.keys(stateIDs).sort((a, b) => {
            if (a == 'device_query' || a == 'state') {
                return (b == 'device_query' ? -1 : 1);
            }
            return a.localeCompare(b)
        })) {
            const state = await this.adapter.getStateAsync(`${adId}.${stateID}`);
            if (state && state.hasOwnProperty('val')) {
                if (stateIDs[stateID] != null) {
                    state.val = stateIDs[stateID];
                }
    ...
    async publishFromState(deviceId, model, stateKey, state, options, debugID) {
        ...
        if (has_elevated_debug) {
            ...
            this.emit('device_debug', {
                ID: debugID,
                data: {ID: deviceId, model: model, flag: '02', IO: false},
                message: message
            });
        }
        ...
        if (!stateDesc) {
            ...
            if (has_elevated_debug) {
                this.emit('device_debug', {
                    ID: debugID,
                    data: {states: [{id: state.ID, value: 'unknown', payload: 'unknown'}], error: 'NOSTKEY', IO: false},
                    message: message
                });
            }
            return;
        ...
        if (value === undefined || value === '') {
            if (has_elevated_debug) {
                ...
                this.emit('device_debug', {
                    ID: debugID,
                    data: {
                        states: [{id: state.ID, value: '--', payload: 'error', ep: stateDesc.epname}],
                        error: 'NOVAL1',
                        IO: false
                    },
                    message: message
        ...
        if (stateDesc.id === 'send_payload') {
            if (value === 'configure' || value === 'reconfigure' || value === 'interview') {
                this.emit('zb_devicecommand', value, deviceId, stateDesc);
            } else {
                try {
                    const json_value = JSON.parse(value);
                    const payload = {
                        device: deviceId.replace('0x', ''),
                        payload: json_value,
                        model: model,
                    ...
                    if (has_elevated_debug) {
                        this.emit('device_debug', {
                            ...
                            data: {
                                flag: '04',
                                payload: value,
                                states: [{id: stateDesc.id, value: json_value, payload: 'none'}],
                                IO: false
                            }
                        });
                    }
                    if (Array.isArray(json_value.read)) {
                        for (const item of json_value.read) {
                            try {
                                this.emit('send_payload', {
                                    device: deviceId.replace('0x', ''),
                                    payload: {read: item},
                                    model: model,
                                    stateModel: stateModel,
                                    acknowledge: true
                                }, debugID, has_elevated_debug);
                            } catch (error) {
                                if (has_elevated_debug) {
                                    this.emit('device_debug', {
                                        ID: debugID,
                                        data: {
                                            error: 'EXSEND',
                                            states: [{id: stateDesc.id, value: value, payload: error.message}],
                                            IO: false
                                        },
        ...
        if (stateDesc.id === 'device_query') {
            ...
            this.emit('device_query', deviceId, debugID, has_elevated_debug, (devId) => {
                setTimeout(() => {
                    const idx = this.deviceQueryBlock.indexOf(id);
                    if (idx > -1) {
                        this.deviceQueryBlock.splice(idx);
                    }
                }, 10000)
                ...
                try {
                    if (typeof linkedFunct === 'function') {
                        ...
                        if (res) {
                            if (res.hasOwnProperty('stateDesc')) { // we got a single state back
                                if (!res.stateDesc.isOption) {
                                    stateList.push(res);
                                }
                            } else {
                                res.forEach((ls) => {
                                    if (!ls.stateDesc.isOption) {
                                        stateList.push(ls)
                                    }
                                });
                            }
                    ...
                    if (has_elevated_debug) {
                        this.emit('device_debug', {
                            ID: debugID,
                            data: {
                                states: [{id: state.ID, value: state.val, payload: 'unknown'}],
                                error: 'EXLINK',
                                IO: false
                            }
                        });
    ...
    setDeviceActivated(id, inActive) {
        this.adapter.extendObject(id, {
            common: {
                ...
                color: inActive ? '#888888' : null,
                statusStates: inActive ? null : {onlineId: `${id}.available`}
            }
        });
    }
    ...
    async deleteObj(devId) {
        ...
        try {
            ...
            return {
                status: false,
                message: `Cannot delete Object ${devId}: ${error && error.message ? error.message : 'without error message'}`
            };
        }
    }
    ...
    async checkIfModelUpdate(entity) {
        ...
        if (obj && obj.common.type !== model) {
            ...
            await this.deleteOrphanedDeviceStates(device.ieeeAddr, model, false, () => {
            }, true);
        }
    ...
    deepCompare(item1, item2) {
        if (typeof item1 != typeof item2) {
            return false;
        } // different object, definitely different.
        switch (typeof item1) {
            case 'boolean':
            case 'number':
            case 'string':
            ...
            case 'object':
                ...
                for (const key of Object.keys(item1)) {
                    if (!this.deepCompare(item1[key], item2[key])) {
                        return false;
                    }
                }
                return true;
    ...
    async updateState(devId, name, value, common) {
        ...
        if (value !== undefined && type === 'number') {
            ...
            if (nval < minval) {
                ...
                new_common.color = new_common.color ? new_common.color : null;
            }
        }
        ...
        if (new_common.color === null && (stobj?.common?.color === undefined || stobj?.common.color === null)) {
            delete new_common.color;
        }
        ...
        if (typeof value !== type) {
            ...
            switch (type) {
                ...
                case 'object' :
                    break;
                default:
                    if (['array', 'file', 'json', 'mixed', 'multistate', 'number', 'object', 'string'].includes(type)) {
                        break;
                    }
    ...
    async updateDev(dev_id, dev_name, model, callback) {
        ...
        this.adapter.zbController.setDeviceEnable(dev_id, (obj?.common?.deactivated == true));
    ...
    CleanupRequired(set) {
        try {
            if (typeof set === 'boolean') {
                this.cleanupRequired = set;
            }
            return this.cleanupRequired;
        } catch (error) {
            if (this.debugActive) {
                this.debug(`Error setting cleanup required: ${error && error.message ? error.message : 'no message available'}`);
            }
        }
    }
    ...
    async getFromDefinedState(devId, stateDesc) {
        ...
        if (stateDesc.setter) {
            return stateDesc.setter(state.val);
        }
        return state?.val;
    }
    ...
    async publishToDefinedState(devId, model, statedesc, payload, debugId) {
        ...
        if (statedesc.compositeGetter) {
            ...
            if (compositePayload) {
                await this.publishToState(devId, model, compositePayload, debugId);
            }
        }
        ...
        if (value === undefined || value === null) {
            ...
            if (has_elevated_debug) {
                this.emit('device_debug', {
                    ID: debugId,
                    data: {states: [{id: stateID, value: value, payload: payload}], flag: '04', IO: true},
                    message
                });
            } else if (this.debugActive) {
                this.debug(message);
        ...
        if (statedesc.isEvent) {
            ...
            if (has_elevated_debug) {
                this.emit('device_debug', {ID: debugId, data: {flag: 'SUCCESS', IO: true}});
            }
    ...
    async publishToState(devId, model, payload, debugId) {
        try {
            ...
            if (has_elevated_debug) {
                this.emit('device_debug', {
                    ID: debugId,
                    data: {deviceID: devId, flag: '03', IO: true},
                    message: message
                });
            } else if (this.debugActive) {
                this.debug(message);
            ...
            if (!devStates) {
                ...
                if (has_elevated_debug) {
                    this.emit('device_debug', {
                        ID: debugId,
                        data: {error: 'NOSTATE', states: [{id: '--', value: '--', payload: payload}], IO: true},
                        message: message
                    });
                } else if (this.debugActive) {
                    this.debug(message);
            ...
            if (devStates.states !== undefined) {
                try {
                    ...
                    if (has_elevated_debug) {
                        this.emit('device_debug', {ID: debugId, data: {error: 'ESTATE', IO: true}, message: message});
                    } else if (this.debugActive) {
                        this.debug(message);
                    }
                }
                ...
                if (Object.keys(publishedStates).length == 0) {
                    if (has_elevated_debug) {
                        this.emit('device_debug', {
                            ID: debugId,
                            data: {
                                states: [{id: 'no state', value: 'no value', inError: true, payload: payload}],
                                flag: '04',
                                IO: true
                            },
                            message: message
                ...
                if (has_elevated_debug) {
                    this.emit('device_debug', {ID: debugId, data: {error: 'NOSTATE', IO: true}, message});
                } else if (this.debugActive) {
                    this.debug(message);
                }
            }
     ...
     * @param metaStates
     * @param debugId
     * @param has_elevated_debug
     */
    async processConverters(converters, devId, model, mappedModel, message, meta, metaStates, debugId, has_elevated_debug) {
        let cnt = 0;
        const publish = async (payload, dID) => {
            if (typeof payload === 'object') {// && Object.keys(payload).length > 0) {
                ...
                this.emit('device_debug', {
                    ID: debugId,
                    data: {error: `EPAYLD`, IO: true},
                    message: ` payload ${JSON.stringify(payload)} is empty`
                })
            }
        };
        ...
        for (const converter of converters) {
            ...
            chain.push(new Promise((resolve) => {
                if (has_elevated_debug) {
                    this.emit('device_debug', {
                        ID: debugId,
                        data: {flag: `02.${cnt}a`, IO: true},
                        message: `converter ${cnt} : Cluster ${converter.cluster}`
                    })
                }
                try {
                    ...
                    if (has_elevated_debug) {
                        ...
                        this.emit('device_debug', {
                            ID: debugId,
                            data: {flag: `02.${idx}b`, IO: true},
                            message: ` data: ${safeJsonStringify(message.data)} options: ${safeJsonStringify(options)} meta:${safeJsonStringify(metapost)} result:${safeJsonStringify(payloadConv)}`
                        })
                    }
                    ...
                    if (has_elevated_debug) {
                        this.emit('device_debug', {
                            ID: debugId,
                            data: {error: 'EPROC', IO: true},
                            message: `${msg} - ${error?.stack ?? 'no stack available'}`
                        });
                    }
                    this.warn(msg);
                    resolve({})
        ...
        if (Object.keys(payload).length > 0 && Object.keys(options).length > 0) {
            ...
            if (has_elevated_debug) {
                this.emit('device_debug', {
                    ID: debugId,
                    data: {flag: `02.${cnt}d`, IO: true},
                    message: `${premsg} => processed payload : ${JSON.stringify(payload)}`
                })
            }
        } else if (has_elevated_debug) {
            this.emit('device_debug', {
                ID: debugId,
                data: {flag: `02.${cnt}c`, IO: true},
                message: `candidates: ${JSON.stringify(candidates)} => payload ${JSON.stringify(payload)}`
            })
        }
        ...
        await this.updateMeta(devId, meta);
    ...
    async onZigbeeEvent(type, entity, message) {
        ...
        if (message.device.type != 'Coordinator') {
                        ...
                        switch (cluster) {
                            ...
                            case 'lightingColorControl': {
                                ...
                                this.boundReads[key] = setTimeout(() => {
                                    try {
                                        this.adapter.emit('sendToZigbee', {
                                            message: {
                                                id: target.address,
                                                ep: target.endpoint,
                                                cid: cluster,
        ...
        if (has_elevated_debug) {
            ...
            this.emit('device_debug', {
                ID: debugId,
                data: {ID: device.ieeeAddr, payload: safeJsonStringify(msgForState), flag: '01', IO: true},
                message: message
            });
        ...
        if (!converters.length) {
            if (type !== 'readResponse' && type !== 'commandQueryNextImageRequest') {
                ...
                if (has_elevated_debug) {
                    this.emit('device_debug', {ID: debugId, data: {error: 'NOCONV', IO: true}, message: message});
                } else if (this.debugActive) {
                    this.debug(message);
                }
            }
        ...
        const metaStates = utils.getMetaStates(entity?.options?.meta_state ?? '');


        try {
            await this.processConverters(converters, devId, model, mappedModel, message, meta, metaStates, debugId, has_elevated_debug);
        } catch (error) {
            //   'Error: Expected one of: 0, 1, got: 'undefined''
            if (cluster !== '64529') {
                if (has_elevated_debug) {
                    this.emit('device_debug', {ID: debugId, data: {error: 'EPROC', IO: true}});
                }
    ...
    recursiveObjectCompare(a, b) {
        ...
        if (typeof a == 'object') {
            for (const key of Object.keys(a)) {
                if (this.recursiveObjectCompare(a[key], b[key])) {
                    continue;
                }
                return false;
            }
     ...
     * @param meta
     */
    async updateMeta(devId, meta) {
        try {
            const m = meta ?? {};
            const obj = await this.adapter.getObjectAsync(devId);
            const existingMeta = obj?.native?.meta ?? {};
            const message = JSON.stringify(existingMeta);
            delete m.device;
            delete m.logger;

            let changed = false;
            for (const key of Object.keys(m)) {
                if (!this.recursiveObjectCompare(m[key], existingMeta[key])) {
                    existingMeta[key] = m[key];
                    changed = true;
                }
            }
            if (changed) {
    ...
    async stop() {
        try {
            if (this.localConfig && typeof this.localConfig.saveConfig === 'function') {
                this.localConfig.saveConfig();
            }
        } catch (error) {
            this.warn(`Error in StatesController.stop(): ${error?.message ?? 'no message'}`);
        }
    }
}
