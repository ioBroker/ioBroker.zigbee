...
class StatesController extends EventEmitter {
    ...
    onStateChange(id, state) {
        ...
        if (state && !state.ack) {
            ...
            if (this.checkDebugDevice(id)) {
                ...
                this.emit('device_debug', { ID:debugId, data: { ID: deviceId, flag:'O01' }});
            } else
    ...
    async publishFromState(deviceId, model, stateKey, state, options, debugId) {
        ...
        if (!stateDesc) {
            ...
            if (elevated) this.emit('device_debug', { ID:debugId, data: { states:[{id:state.ID, value:'unknown', payload:'unknown'}], error: '02' , IO:false }});
            return;
        }
        ...
        if (value === undefined || value === '') {
            if (elevated) {
                this.emit('device_debug', { ID:debugId, data: { states:[{id:state.ID, value:'--', payload:'error'}],error: '03' , IO:false }});
        ...
        if (stateModel && stateModel.linkedStates) {
            stateModel.linkedStates.forEach(linkedFunct => {
                try {
                    ...
                    this.sendError(e);
                    if (elevated) this.emit('device_debug', { ID:debugId, data: { states:[{id:state.ID, value:state.val, payload:'unknown'}], error: 'ex' , IO:false }});
        ...
        try {
            ...
            if (!devStates) {
                if (has_elevated_debug) {
                    ...
                    this.emit('device_debug', { ID:debugId, data: { error:'01',states:[{ id:'--', value:'--', payload:payload}], IO:true }});
                }
                return;
            }
    ...
    async onZigbeeEvent(type, entity, message) {
        ...
        if (!converters.length) {
            if (type !== 'readResponse') {
                ...
                if (has_elevated_debug) {
                    this.warn(`ELEVATED IE00: No converter available for '${mappedModel.model}' '${devId}' with cluster '${cluster}' and type '${type}'`);
                    this.emit('device_debug', { ID:debugId, data: { error:'00', IO:true }});
                }
            }
            return;
        }
        ...
        */
        ...
        this.processConverters(converters, devId, model, mappedModel, message, meta, debugId)
            .catch((error) => {
                //   'Error: Expected one of: 0, 1, got: 'undefined''
                if (cluster !== '64529') {
                    if (has_elevated_debug) this.emit('device_debug', { ID:debugId, data: { error:'ex', IO:true }});
