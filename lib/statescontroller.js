...
class StatesController extends EventEmitter {
    ...
    async publishToState(devId, model, payload, debugId) {
        try {
            ...
            if (has_elevated_debug) this.emit('device_debug', { ID:debugId, data: { deviceID: devId, flag:'03', IO:true }, message:message});
    ...
    async processConverters(converters, devId, model, mappedModel, message, meta, debugId) {
        ...
        for (const converter of converters) {
            ...
            const payload = await new Promise((resolve, reject) => {
                ...
                this.emit('device_debug', {ID:debugId,data: { flag:`02.${cnt}x`, IO:true }, message:` data: ${safeJsonStringify(message.data)} meta:${safeJsonStringify(meta)} result:${safeJsonStringify(payloadConv)}`})
                if (typeof payloadConv === 'object') {
                    resolve(payloadConv);
                }
            });
            cnt++
            this.emit('device_debug', {ID:debugId,data: { flag:`02.${cnt}a`, IO:true }, message:`converter ${cnt} : Cluster ${converter.cluster} => payload : ${JSON.stringify(payload)}`})
            ...
            if (Object.keys(payload).length > 0 && Object.keys(options).length > 0) {
                ...
                this.emit('device_debug', {ID:debugId,data: { flag:`02.${cnt}b`, IO:true }, message:`converter ${cnt} : Cluster ${converter.cluster} => processed payload : ${JSON.stringify(payload)}`})
            }
