...
class Developer {
    ...
    clusterIdForCluster(cluster) {
        if (!cluster) return 0;
        if (isNaN(Number(cluster))) return ZigbeeHerdsman.zcl.Clusters[cluster]?.ID ?? 0;
        return Number(cluster);
    }

    async sendToZigbee(obj) {
        if (this.zbController) try {
            const zcl = ZigbeeHerdsman.Zcl;
            ...
            const cid = this.clusterIdForCluster(obj.message.cid);
            const cmdType = obj?.message?.cmdType ?? 'foundation';
            let cmd;
            const zclData = obj?.message?.zclData ?? {};
            if (cmdType === 'functional') {
                cmd = zcl.Utils.getClusterCommand(zcl.Utils.getCluster(cid), obj.message.cmd);
            } else if (cmdType === 'functionalResp') {
                cmd = zcl.Utils.getClusterCommandResponse(zcl.Utils.getCluster(cid),obj.message.cmd);
            } else if (cmdType === 'foundation') {
                cmd = zcl.Utils.getGlobalCommand((obj.message.cmd));
            } else {
                if (obj.callback) this.adapter.sendTo(obj.from, obj.command, {localErr: 'Invalid cmdType'}, obj.callback);
                return;
            }
            ...
            if (!cid || !cmd) {
                if (obj.callback) this.adapter.sendTo(obj.from, obj.command, {localErr: 'Incomplete data (cid or cmd)'}, obj.callback);
                return;
            }
            ...
            await this.zbController.publish(publishTarget, cid, cmd.name, zclData, cfg, ep, cmdType, (err, msg) => {
                ...
                result.statusCode = 0;
                if (obj.callback) this.adapter.sendTo(obj.from, obj.command, result, obj.callback);
            ...
            if (exception instanceof ZclStatusError) {
                ...
                if (obj.callback) this.adapter.sendTo(obj.from, obj.command, {msg: exception.message}, obj.callback);
            }
