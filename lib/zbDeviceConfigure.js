...
class DeviceConfigure extends BaseExtension {
    ...
    PushDeviceToQueue(device, mappedDevice, priority) {
        ...
        if (item) {
            ...
            if (priority) {
                ...
                if (idx > -1) {
                    this.deviceConfigureQueue.splice(idx, 1);
                }
            } else {
                return;
            }
    ...
    shouldConfigure(device, mappedDevice) {
        ...
        if (this.configuring.has(device.ieeeAddr) || device.interviewing) {
            return false;
        }
        const t = Date.now();
    ...
    async doConfigure(device, mappedDevice) {
        ...
        try {
            if (mappedDevice) {
                ...
                if (mappedDevice.configure === undefined) {
                    return `No configure available for ${device.ieeeAddr} ${mappedDevice.model}.`;
                }
                ...
                if (typeof mappedDevice.configure === 'function') {
                    await mappedDevice.configure(device, coordinatorEndpoint, mappedDevice);
                } else {
                    const promises = [];
