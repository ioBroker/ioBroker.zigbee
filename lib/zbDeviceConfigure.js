    ...
    async doConfigure(device, mappedDevice) {
        ...
        try {
            if (mappedDevice) {
                ...
                await Promise.all(promises.map(callback => callback(device, coordinatorEndpoint, this)))
            ...
            if (error.stack.includes('UNSUPPORTED_ATTRIBUTE')) {
                ...
                this.sendError(error);
                this.warn(` ${device.ieeeAddr} ${device.modelID} Failed to configure. --> ${error && error.stack ? error.stack : ''} `);
