...
class DeviceConfigure extends BaseExtension {
    ...
    async handleConfigureQueue() {
        if (this.deviceConfigureQueue.length < 1) {
            this.warn('end of queue reached, killing interval')
            clearInterval(this.configureIntervall);
            this.configureIntervall == null;
            return;
        }
        const configureItem = this.deviceConfigureQueue.shift();
        this.warn(`DeviceConfigureQueue configuring ${configureItem.dev.ieeeAddr} ${configureItem.dev.modelID}`)
    ...
    PushDeviceToQueue(device, mappedDevice) {
        ...
        for (const candidate of this.deviceConfigureQueue) {
            if (candidate.id == id) {
                this.warn('no duplicate entry in queue');
                return;
            }
        }
    ...
    shouldConfigure(device, mappedDevice) {
        ...
        this.warn(`should configure: ${device.meta.hasOwnProperty('configured') ? device.meta.configured: 'none'} -  ${zigbeeHerdsmanConverters.getConfigureKey(mappedDevice)}`);
        return device.meta.hasOwnProperty('configured') && device.meta.configured !== zigbeeHerdsmanConverters.getConfigureKey(mappedDevice);
    }
        ...
        try {
            ...
            for (const device of await this.zigbee.getClients()) {
                ...
                if (this.shouldConfigure(device, mappedDevice)) {
                    this.warn(`DeviceConfigure ${device.ieeeAddr} ${device.modelID} needed`);
    ...
    onZigbeeEvent(data, mappedDevice) {
        try {
            ...
            if (com) {
                this.warn(`checking configure on message :  next attempt in ${30000 - (Date.now() - com.timestamp)} seconds`);
        ...
        try {
            ...
            if (error.stack.includes('UNSUPPORTED_ATTRIBUTE')) {
                ...
                if (error && error.message && error.message.match(/(\d+)ms/gm)) {
                    ...
                    this.warn(`Timeout trying to configure ${device.ieeeAddr} ${device.modelID}.`)
