...
class DeviceConfigure extends BaseExtension {
    constructor(zigbee, options) {
        ...
        this.retriedUnsupportedClusters = new Set();
    }
    ...
    async handleConfigureQueue() {
        ...
        this.info(`DeviceConfigureQueue configuring '${this.devLabel(configureItem.dev.ieeeAddr, configureItem.mapped?.model)}'`)
    ...
    shouldConfigure(device, mappedDevice) {
        ...
        if (this.configuring.has(device.ieeeAddr) || device.interviewState === 'IN_PROGRESS') return false;
        const t = Date.now();
        ...
        this.debug(`should configure for device '${this.devLabel(device.ieeeAddr, mappedDevice.model)}' (${mappedDevice.model}: ${device.meta.hasOwnProperty('configured') ? device.meta.configured: 'none'} -  ${cfgkey} (query took ${Date.now()- t} ms)`);
        return result;
    }
        ...
        try {
            ...
            for (const device of await this.zigbee.getClientIterator()) {
                ...
                if (this.shouldConfigure(device, mappedDevice)) {
                    this.info(`DeviceConfigure '${this.devLabel(device.ieeeAddr, mappedDevice?.model)}' needed - Device added to Configuration Queue`);
                    this.PushDeviceToQueue(device, mappedDevice);
                } else {
                    this.debug(`DeviceConfigure '${this.devLabel(device.ieeeAddr, mappedDevice?.model)}' not needed`);
                }
            }
        } catch (error) {
        ...
        try {
            ...
            if (com) {
                ...
                if (Date.now() - com.timestamp > 30000 && !this.configuring.has(device.ieeeAddr)) {
                    ...
                    this.info('Configure on Message for ' + "'" + this.devLabel(device.ieeeAddr, mappedDevice.model) + "'");
                    this.doConfigure(device, mappedDevice);
                }
                return;
            }
        ...
        try {
            if (mappedDevice !== undefined && device !== undefined) {
                ...
                try {
                    ...
                    this.sendError(error);
                    this.warn(`DeviceConfigure failed '${this.devLabel(device.ieeeAddr, mappedDevice?.model)}'`);
                }
            }
        } catch (error) {
            this.sendError(error);
            this.error(`Failed to DeviceConfigure.configure '${this.devLabel(device.ieeeAddr, mappedDevice?.model)}': ${error && error.message ? error.message : 'no error message'})`);
        }
    }
        ...
        try {
            if (mappedDevice) {
                ...
                if (cfgKey != device.meta.configured)
                    this.info(`Configuring '${this.devLabel(device.ieeeAddr, mappedDevice.model)}' ${device.meta.configured ? 'from V'+ device.meta.configured : ''} to V${cfgKey}`);
                else
                    this.info(`Reconfiguring '${this.devLabel(device.ieeeAddr, mappedDevice.model)}' with V${cfgKey}`);
                ...
                delete this.configureOnMessageAttempts[device.ieeeAddr];
                device.save();
                this.info(`DeviceConfigure to V${device.meta.configured} for '${this.devLabel(device.ieeeAddr, mappedDevice.model)}' successful.`);
                return '';
            }
        } catch (error) {
            this.configuring.delete(device.ieeeAddr);
            ...
            if (errmsg.includes(`UNSUPPORTED_ATTRIBUTE`)) {
                device.meta.configured = cfgKey;
                this.configuring.delete(device.ieeeAddr);
                delete this.configureOnMessageAttempts[device.ieeeAddr];
                device.save();
            ...
            if ((errmsg.includes(`UNSUPPORTED_CLUSTER`) && !this.retriedUnsupportedClusters.has(device.ieeeAddr)) || hasmatch) {
                // timeout message - we do want to start the configure chain
                if (!hasmatch) await this.tryInterview(device);
                if (this.configureOnMessageAttempts.hasOwnProperty(device.ieeeAddr)) {
                    ...
                    if ( com.count < 0) {
                        delete this.configureOnMessageAttempts[device.ieeeAddr];
                        this.info(`Configure on message abandoned for '${this.devLabel(device.ieeeAddr, mappedDevice.model)}' after failing ${com.attempts} times.`)
                    }
                    else this.info(`Timeout trying to configure '${this.devLabel(device.ieeeAddr, mappedDevice.model)}' (${com.count}).`)
                }
                else {
                    this.info(`Timeout trying to configure '${this.devLabel(device.ieeeAddr, mappedDevice.model)}' (starting CoM).`)
                ...
                return `Configuration timed out '${this.devLabel(device.ieeeAddr, mappedDevice.model)}'. The device did not repond in time to the configuration request. Another attempt will be made when the device is awake.`;
            } else {
                this.sendError(error);
                const msg = `'${this.devLabel(device.ieeeAddr, mappedDevice.model)}' Failed to configure. --> ${errmsg}${errmsg.includes(`UNSUPPORTED_CLUSTER`) ? ' - You may need to restart the adapter and reconfigure the device for this device to work properly' : ''}`
                this.warn(msg);
                return msg;
