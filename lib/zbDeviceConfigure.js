...
class DeviceConfigure extends BaseExtension {
    ...
    shouldConfigure(device, mappedDevice) {
        ...
        this.warn(`should configure: ${device.meta.hasOwnProperty('configured') ? device.meta.configured: 'none'} -  ${cfgkey}`);
        const result = device.meta.hasOwnProperty('configured') && device.meta.configured !== cfgkey;
        this.warn(`ZHC03,${Date.now()},${Date.now()-t},${result},${cfgkey}`)
        return result;
    }
    ...
    async onZigbeeStarted() {
        try {
            ...
            for (const device of await this.zigbee.getClients()) {
                ...
                this.warn(`ZHC04,${Date.now()},${Date.now()-t},${device && device.ieeeAddr ? device.ieeeAddr : '0x0'},${mappedDevice ? 'found' : 'notfound'}`)
