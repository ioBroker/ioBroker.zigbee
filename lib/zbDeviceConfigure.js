...
class DeviceConfigure extends BaseExtension {
    ...
    shouldConfigure(device, mappedDevice) {
        if (!device || !mappedDevice) {
          this.warn("should configure with NULL : " + JSON.stringify(device) + " - " + JSON.stringify(mappedDevice))
            return false;
        }
    ...
    async onZigbeeStarted() {
        try {
            ...
            for (const device of await this.zigbee.getClients()) {
                ...
                if (forcedConfigureOnEachStart.find((d) => d.hasOwnProperty('zigbeeModel') && d.zigbeeModel.includes(device.modelID))) {
