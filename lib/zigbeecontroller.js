...
class ZigbeeController extends EventEmitter {
    ...
    */
    ...
    async addPairingCode(code) {
      this.warn(`calling addPairingCode with ${code}`);
      if (code) {
    ...
    async addDevToGroup(devId, groupId, epid) {
        try {
            ...
            this.warn(`addDevFromGroup - entity: ${utils.getEntityInfo(entity)}`);
            ...
            this.warn(`addDevToGroup ${groupId} with ${memberIDs.length} members ${safeJsonStringify(memberIDs)}`);
            if (epid != undefined) {
...
/*
                        let PreMsg = "Members before adding"
                        const m1 = await this.getGroupMembersFromController(groupId)
                        for (let idx in m1) {
                          PreMsg = `${PreMsg} ${m1[idx].ieee}`
                        }
*/
                        await (ep.addToGroup(group.mapped));
//                        await (ep.addToGroup(groupId));
/*
                        const m2 = await this.getGroupMembersFromController(groupId)
                        let PostMsg = "Members after adding"
                        for (let idx in m2) {
                          PostMsg = `${PostMsg} ${m2[idx].ieee}`
                        }
                        this.warn(`Group removal: ${PreMsg} -> ${PostMsg}`)
                        */
                ...
                if (entity.endpoint.inputClusters.includes(4)) {
                    this.warn(`adding endpoint ${entity.endpoint.ID} to group`);
                    await entity.endpoint.addToGroup(group.mapped);
//                      await entity.endpoint.addToGroup(groupId);
                } else {
                    let added = false;
                    for (const ep of entity.endpoints) {
                        if (ep.inputClusters.includes(4)) {
/*
                            this.warn(`adding endpoint ${ep.ID} to group`);
...
*/
    ...
    async removeDevFromGroup(devId, groupId, epid) {
        this.warn(`removeDevFromGroup with ${devId}, ${groupId}, ${epid}`)
        let entity;
        try {
            ...
            this.warn(`removeDevFromGroup - entity: ${utils.getEntityInfo(entity)}`);
            this.warn(`removeDevFromGroup ${groupId} with ${memberIDs.length} members ${safeJsonStringify(memberIDs)}`);

            if (epid != undefined) {
                for (const ep of entity.endpoints) {
                  this.warn(`checking ep ${ep.ID} of ${devId} (${epid})`)
...
/*
                        const m1 = await this.getGroupMembersFromController(groupId)
                        let PreMsg = "Members before removal"
                        for (let idx in m1) {
                          PreMsg = `${PreMsg} ${m1[idx].ieee}`
                        }
*/
                        await ep.removeFromGroup(group.mapped);
//                        await ep.removeFromGroup(groupId);
/*                        const m2 = await this.getGroupMembersFromController(groupId)
                        let PostMsg = "Members after removal"
                        for (let idx in m2) {
                          PostMsg = `${PostMsg} ${m2[idx].ieee}`
                        }
                        this.warn(`Group removal: ${PreMsg} -> ${PostMsg}`)
