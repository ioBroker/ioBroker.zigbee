    ...
    async getMap(callback) {
        try {
            ...
            await Promise.all(devices.filter((d) => d.type !== 'EndDevice').map(async device =>
            {
                ...
                else errors.push(`Omitted collecting routing table for '${resolved ? resolved.name : 'unresolved device'} (${resolved ? resolved.device.modelID : 'unknown'}') : LQI timed out`);
