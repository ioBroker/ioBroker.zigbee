...
class ZigbeeController extends EventEmitter {
    ...
    */
                ...
                if (converter.hasOwnProperty('convertGet')) {
                    for (const ckey of converter.key) {
                        try {
                            ...
                            this.warn(`read state ${JSON.stringify(ckey)} of ${entity.device.ieeeAddr}/${entity.device.endpoints[0].ID} after device query`);
                        } catch (error) {
                            if (elevated) {
                                ...
                                this.info(`failed to read state ${JSON.stringify(ckey)} of ${entity.device.ieeeAddr}/${entity.device.endpoints[0].ID} after device query`);
                        }
                    }
                }
            }
    ...
    async deviceQuery(deviceId, debugID, elevated, callback) {
        ...
        const idx = this.deviceQueryActive[deviceId]
        if (idx > -1) this.deviceQueryActive.splice(idx);
