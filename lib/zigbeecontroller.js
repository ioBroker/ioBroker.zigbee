...
const { LibraryLogBridge } = require('./libraryLog');
...
class ZigbeeController extends EventEmitter {
    ...
    */
    constructor(adapter) {
        ...
        this.localConfig = adapter.localConfig;
        this.zbcontrollerStarted = 0;
        // devices seen since the start (database, joins, interviews, leaves): ieee -> { paired, model, lastSeen, rejected }
        this.seenDevices = new Map();
        // the log output of zigbee-herdsman and zigbee-herdsman-converters goes into the adapter log
        this.libraryLog = new LibraryLogBridge(adapter);
        this.libraryLog.setup();
        this.libraryLog.attach(ZigbeeHerdsman, zigbeeHerdsmanConverters);
    }

    /**
     * zigbee-herdsman asks this before it interviews a joining device (rejoins included): false makes it send
     * the device away with a leave request - no interview, no database entry, no object.
     */
    async acceptJoiningDevice(ieeeAddr) {
        const ieee = String(ieeeAddr).toLowerCase();
        const seen = this.noteSeen(ieee, { paired: false });
        if (this.localConfig && this.localConfig.isBlocked(ieee)) {
            seen.rejected++;
            this.info(`Join of '${ieee}' rejected: the device is on the join blocklist`);
            return false;
        }
        return true;
    }

    /** Remember a device that was seen, as a source for the blocklist editor in the admin. */
    noteSeen(ieeeAddr, data) {
        const ieee = String(ieeeAddr).toLowerCase();
        const seen = this.seenDevices.get(ieee) || { paired: false, model: undefined, rejected: 0 };
        Object.assign(seen, data, { lastSeen: Date.now() });
        this.seenDevices.set(ieee, seen);
        return seen;
    }

    /** The seen devices: unpaired first, newest first. */
    getSeenDevices() {
        return [...this.seenDevices.entries()]
            .map(([ieee, seen]) => ({ ieee, ...seen }))
            .sort((a, b) => (a.paired === b.paired ? b.lastSeen - a.lastSeen : (a.paired ? 1 : -1)));
    }

    /** After the start: the devices of the database are 'seen', and the blocklist is reported. */
    noteSeenFromDatabase() {
        for (const device of this.getClientIterator(false)) {
            this.noteSeen(device.ieeeAddr, { paired: device.interviewState === 'SUCCESSFUL', model: device.modelID });
        }
        const blocklist = this.localConfig ? this.localConfig.getBlocklist() : [];
        if (blocklist.length === 0) return;
        this.info(`Join blocklist active for ${blocklist.join(', ')}`);
        for (const ieee of blocklist) {
            if (this.seenDevices.has(ieee)) {
                this.warn(`Device '${ieee}' is on the join blocklist but still in the database - the blocklist only refuses a new join, delete the device to get rid of it`);
            }
        }
    }

    setDeviceDisabled(id, status) {
        try {
            const ieee = `0x${id.split('.').pop()}`;
            if (status) this.disabledDevices.add(ieee);
            else this.disabledDevices.delete(ieee);
    ...
    async configure(options) {
        ...
        if (!options.extPanIdFix) {
            ...
            herdsmanSettings.network.extendedPanID = options.net.extPanId;
        }
        ...
        // join blocklist (localConfig): read live, so a change in the admin needs no restart
        herdsmanSettings.acceptJoiningDeviceHandler = async ieeeAddr => this.acceptJoiningDevice(ieeeAddr);
        this.herdsmanSettings = herdsmanSettings;
    ...
    // Start controller
    async start() {
        this.libraryLog.setup(); // forwarding on again after a stop() of the reconnect path
        let closing;
        try {
            if (!this.herdsman) {
                this.configure(this.storedOptions);
                this.herdsman = new ZigbeeHerdsman.Controller(this.herdsmanSettings, this.adapter.log);
                this.zbcontrollerStarted = 0;
            }
            this.zbcontrollerStarted++;
            if (this.zbcontrollerStarted > 1) this.warn(`zbcontroller start started ${this.zbcontrollerStarted} times`);
            ...
            this.spliceDeviceJoinedEvent(true);
            ...
            // the close takes the coordinator a moment (two broadcasts); the devices below are registered
            // in the meantime, so a device that reports right now does not hit 'Model ... not found'
            closing = this.closeNetwork().then(() => true, error => {
                this.warn(`Unable to close the network after start: ${error.message}`);
                return false;
            });
        } catch (e) {
            try {
                this.spliceDeviceJoinedEvent(false);
                const debNetworkParam = JSON.parse(JSON.stringify(await this.herdsman.getNetworkParameters()));
                const extendedPanIDDebug = typeof debNetworkParam.extendedPanID === 'string' ? debNetworkParam.extendedPanID.replace('0x', '') : debNetworkParam.extendedPanID;
        ...
        try {
            ...
            for (const device of deviceIterator) {
                ...
                const msg = ("'" + this.devLabel(entity.device.ieeeAddr, entity.mapped?.model) + "'" +
        ...
        if (await closing) this.info('Network closed for new devices until pairing is opened');
        this.emit('ready');
    }
    ...
    getGroupIterator() {
        if (this.herdsman.database) {
            return this.herdsman.getGroupsIterator()
        }
        else {
            return[].values();
        }
    ...
    async getCoordinatorIeee() {
        if (!this.herdsmanStarted) return  '0x0000000000000001';
        const coordinatorCandidates = await this.herdsman?.getDevicesByType('Coordinator') ?? [];
        if (coordinatorCandidates.length == 0) return '0x0000000000000001';
        else return coordinatorCandidates[0]?.ieeeAddr ?? '0xffffffffffffffff';
    }

    getBindings() {
        const iterator = this.herdsman.getDevicesIterator();
        const rv = {  }
        for (const device of iterator) {
            for (const ep of device.endpoints) {
                const ep_binds = ep.binds;
                if (ep_binds && ep_binds.length) {
                    const bindObj = {
                        address: device.ieeeAddr,
                        endpoint: ep.ID,
                        binds: {
                        }
                    }
                    rv[`${device.ieeeAddr}.${ep.ID}`] = bindObj;
                    for (const bind of ep_binds) {
                        const ckey = bind.target.groupID ? `${bind.target.groupID}` : `${bind.target.deviceIeeeAddress}.${bind.target.ID}`;
                        if (!bindObj.binds.hasOwnProperty(ckey)) {
                            bindObj.binds[ckey] = {
                                address: bind.target.groupID || bind.target.deviceIeeeAddress,
                                type: bind.target.groupID ? 'group' : 'endpoint',
                                endpoint: bind.target.ID || -1,
                                clusters : [bind.cluster.ID]

                            }
                        }
                        else bindObj.binds[ckey].clusters.push(bind.cluster.ID);
    ...
    async zbDeviceCommand(command, devId, statedesc) {
        let actionResult = {error: 'zbDeviceCommand: illegal command ' + command};
        try {
            switch (command) {
                case 'configure':
                case 'reconfigure':
                    actionResult = await this.reconfigure(devId);
                    break;
                case 'interview':
                    actionResult = await this.interview(devId);
                    break;
                case 'startota':
                case 'checkota':
                case 'start_ota':
            ...
            if (actionResult?.error) {
                ...
                if (!(actionResult?.ignored)) {
                    if (statedesc) {
                        this.emit('acknowledge_state', devId, '', statedesc, command);
                    }
                }
                return true;
    ...
    async reconfigure(devId) {
        ...
        if (entity) {
            try {
                const result = await this.callExtensionMethod(
                    'doConfigure',
                    [entity.device, entity.mapped],
                );
                const msg = result.join(',');
                if (msg.length > 5) return {error: msg}
                else return {};
            } catch (error) {
                return {error: `Reconfigure failed ${entity.device.ieeeAddr} ${entity.device.modelID}, (${error.message})`};
            }
        } else {
    ...
    async interview(devId)
    {
        const entity = await this.resolveEntity(devId);
        if (entity) try {
            this.info(`manual Interview for '${this.devLabel(entity.device.ieeeAddr, entity.mapped?.model)}'`);
            await entity.device.interview(true);
            this.info(`interview for '${this.devLabel(entity.device.ieeeAddr, entity.mapped?.model)}' complete`);
        } catch (error) {
    ...
    async getReportings() {
        ...
        for (const device of iterator) {
            for (const ep of device.endpoints) {
                const ep_binds = ep.configuredReportings;
                if (ep_binds && ep_binds.length) {
                    rv[`${device.ieeeAddr}.${ep.ID}`] = {
                        device: device.ieeeAddr,
                        endpoint: ep.ID,
                        reporting:ep_binds
                    }
                }
    ...
    async getGroupMembersFromController(id) {
            ...
            if (group) {
                ...
                for (const member of groupMembers) {
                    ...
                    if (device && device.ieeeAddr) {
                        ...
                        debugMessage.push(utils.endpointData(member, {}));
                    }
                }
            } else {
                return undefined;
        ...
        this.debug(`GroupMembersFromController for ${id} is ${JSON.stringify(debugMessage)}`);
        return members;
    }
    ...
    selectName(model, device, modelZigbee) {
        if (modelZigbee && modelZigbee[0] != device && this.adapter.config.newDeviceNames) return modelZigbee[0];
        if (typeof model == 'string' && !model.startsWith('_') && model.length > 2) return model;
        return device ?? 'unknown';
    }

    enpdointFromEpName(entity, ep) {
        if (!entity?.device || !entity?.mapped) return undefined;
        const mapped = entity.mapped;
    ...
    async resolveEntity(key, ep) {
        try {
            ...
            if (device) {
                ...
                const options = mapped ? this.localConfig.getOptions(device.ieeeAddr, mapped.model) : {};
                return {
                    type: 'device',
                    device,
                    mapped,
                    endpoint: this.enpdointFromEpName({ device, mapped }, ep),
                    endpoints: device.endpoints,
                    name: this.selectName(mapped?.model, device.modelID, mapped?.zigbeeModel),
                    id: utils.zbIdorIeeetoAdId(this.adapter, device.ieeeAddr, false),
                    options:options,
                };
            }
            else {
                this.error(`resolve_entity failed for ${JSON.stringify(_key)} called from ${(new Error()).stack?.split('\n')[2]?.trim().split(' ')[1]}`);
            }
        }
        catch (error)
        {
    ...
    async stop() {
        ...
        try {
            ...
            // the libraries are quiet now - nothing more is logged into a stopping adapter
            this.libraryLog.stop();
        }
    }
    ...
    /**
     * Close the network on the coordinator. zigbee-herdsman starts the coordinator with joining
     * allowed (ember: TRUST_CENTER_POLICY ALLOW_JOINS) and its pairing countdown only reports its end
     * with permitJoinChanged(false) - the coordinator keeps accepting joins until permitJoin(0) is
     * sent, which sets it to rejoins only, clears the transient link key and broadcasts
     * 'permit joining 0'. herdsman emits permitJoinChanged(false) from inside that call; the flag
     * keeps handlePermitJoinChanged from closing the network a second time.
     */
    async closeNetwork() {
        this._closingNetwork = true;
        try {
    ...
    async handlePermitJoinChanged(data)
    {
        try {
            ...
            if (data.permitted) {
                ...
                this._permitJoinTime = 0;
                if (this.herdsmanStarted && !this._closingNetwork) await this.closeNetwork();
            }
        }
        catch (error) {
    ...
    // Zigbee events
    spliceDeviceJoinedEvent(activation) {
        if (activation) {
            this.herdsmanListeners = this.herdsman.rawListeners('deviceLeave').slice();
            this.herdsman.removeAllListeners('deviceLeave');
            this.herdsman.on('deviceLeave', this.insertedDeviceLeave.bind(this))
        }
        else {
            this.herdsmanListeners.forEach(fn => this.herdsman.on('deviceLeave', fn));
        }
    }

    async insertedDeviceLeave(message) {
        this.debug('spliced: insertedDeviceLeave');
    ...
    async handleDeviceLeave(message) {
        try {
            ...
            if (message.ieeeAddr) this.noteSeen(message.ieeeAddr, { paired: false });
    ...
    async handleDeviceAnnounce(message) {
        ...
        if (this.herdsman.getPermitJoin() && (entity?.device?.interviewState != 'SUCCESSFUL')) {
            this.debug(`ignoring device announcement for ${entity?.device?.modelID ? entity?.device?.modelID : 'unknown model'} due to interview state ${entity?.device?.interviewState} while the network is open.`);
    ...
    async handleDeviceJoined(message) {
        ...
        const ieeeAddr = message?.device?.ieeeAddr ?? message?.ieeeAddr;
        if (ieeeAddr) this.noteSeen(ieeeAddr, { paired: false });
        //const entity = await this.resolveEntity(message.device || message.ieeeAddr);
        // this.emit('new', entity);
    ...
    async handleDeviceInterview(message) {
        ...
        */
        try {
            const entity = await this.resolveEntity(message.device || message.ieeeAddr);
            const friendlyName = `${entity.mapped?.model ?? entity.device?.modelID ?? 'unknown'} (${entity.device?.ieeeAddr ?? message.ieeeAddr ?? '0x0000000000000000'})`;//this.devLabel(entity.device.ieeeAddr, entity.mapped?.model);
            const onZigbeeEventparameters = [message, entity.mapped ? entity.mapped: null];

            switch (message.status) {
                case 'successful': {
                    this.info(`Successfully interviewed '${friendlyName}'`);
                    const msgArr = [];
                    ...
                    if (entity.device?.ieeeAddr) this.noteSeen(entity.device.ieeeAddr, { paired: true, model: entity.mapped?.model ?? entity.device.modelID });
                    this.emit('new', entity, true);
                    break;
                }
                case 'failed': {
                    if (entity.device?.ieeeAddr) this.noteSeen(entity.device.ieeeAddr, { paired: false, model: entity.device.modelID });
    ...
    async getMap(callback) {
        try {
            ...
            let cnt = 0;
            this.emit('pairing', `Map Devices done:${cnt}`)
    ...
    async publish(deviceID, cid, cmd, zclData, cfg, ep, type, callback, zclSeqNum) {
        ...
        if (this.debugActive) this.debug(`Zigbee publish to '${this.devLabel(deviceID, entity.mapped?.model)}', ${cid} - cmd ${cmd} - payload ${JSON.stringify(zclData)} - cfg ${JSON.stringify(cfg)} - endpoint ${ep}`);
    ...
    getReadKeys(stateDesc, deviceId, model) {
        try {
            const allowedKeys = this.localConfig.getDeviceOption(deviceId, model, 'link_states',false ) || (this.adapter.config.readBrightnessAndState ? ['brightness', 'state'] : undefined);
            if (allowedKeys && stateDesc.readKeys)
                return stateDesc.readKeys.map(function(o) { return { key:o, converter:undefined }}).filter((o)=> allowedKeys.includes(o.key));
            return undefined;
        }
        catch (error) {
            console.warn(`getReadKeys: ${error?.message}`);
        }
    }
    ...
    async publishFromState(deviceId, model, stateModel, stateList, options, debugID, has_elevated_debug) {
        ...
        if (has_elevated_debug)
        {
            ...
            const message = `Publishing to '${this.devLabel(deviceId, model)}' of model ${model} with ${stateNames.join(', ')}`;
            this.emit('device_debug', { ID:debugID, data: { ID: deviceId, flag: '03', IO:false }, message: message});
        }
        else
            if (this.debugActive) this.debug(`main publishFromState : '${this.devLabel(deviceId, model)}' ${model} ${safeJsonStringify(stateList)}`);
        if (model === 'group') {
            isGroup = true;
        ...
        try {
            ...
            if (this.debugActive) this.debug(`entity: '${this.devLabel(deviceId, model)}' ${model} ${safeJsonStringify(utils.entityData(entity,{entity:true, device:true, model: true}))}))}`);
            const mappedModel = entity ? entity.mapped : undefined;

            if (!mappedModel) {
                if (this.debugActive) this.debug(`No mapped model for ${model}`);
                if (has_elevated_debug) {
                    const message=`No mapped model '${this.devLabel(deviceId, model)}' (model ${model})`;
            ...
            const meta = await utils.getMeta(this.adapter, `${utils.zbIdorIeeetoAdId(this.adapter, deviceId, false)}`);

            meta.device= entity.device;
            meta.mapped = model === 'group' ? [] : mappedModel;
            meta.logger = this;
            meta.state = meta.state ?? {};


            for (const changedState of stateList) {
                ...
                if (stateDesc.isOption) {
                    if (has_elevated_debug) {
                        const message = `No converter needed on option state for '${this.devLabel(deviceId, model)}' of type ${model}`;
                        this.emit('device_debug', { ID:debugID, data: { flag: `SUCCESS` , IO:false }, message:message});
                    }
                    else
                        if (this.debugActive) this.debug(`No converter needed on option state for '${this.devLabel(deviceId, model)}' of type ${model}`);

                    continue;
                }
                // force read for brightness
                const readKeys = this.getReadKeys(stateDesc, deviceId, model);

                for (const c of mappedModel.toZigbee) {
                    if (c === undefined)
                        continue;
                    if (typeof c == 'function') {
                        this.warn(`WARNING: toZigbee has function`)
                        continue;
                    }
                    ...
                    if (!c.hasOwnProperty('key')) {
                        ...
                        {
                            if (readKeys && c.hasOwnProperty('convertGet')) {
                                readKeys.forEach(function(rk) {if (rk.key && rk.converter === undefined) rk.converter = c });
                            }
                            converter = c;
                            if (has_elevated_debug) {
                                const message = `Setting converter to keyless converter for '${this.devLabel(deviceId, model)}' of type ${model}`;
                                this.emit('device_debug', { ID:debugID, data: { flag: `s4.${msg_counter}` , IO:false }, message:message});
                            }
                            else
                                if (this.debugActive) this.debug(`Setting converter to keyless converter for '${this.devLabel(deviceId, model)}' of type ${model}`);
                            msg_counter++;
                        }
                        else
                        {
                            if (has_elevated_debug)
                            {
                                const message = `ignoring keyless converter for '${this.devLabel(deviceId, model)}' of type ${model}`;
                                this.emit('device_debug', { ID:debugID, data: { flag: `i4.${msg_counter}` , IO:false} , message:message});
                            }
                            else
                                if (this.debugActive) this.debug(`ignoring keyless converter for '${this.devLabel(deviceId, model)}' of type ${model}`);
                            msg_counter++;
                        }
                        continue;
                    }
                    ...
                    {
                        if (!options.first_converter || converter == undefined) {
                            const message = `${(converter===undefined?'Setting':'Overriding')}' converter to converter with key(s)'${JSON.stringify(c.key)}}`;
                            if (has_elevated_debug) {
                                this.emit('device_debug', { ID:debugID, data: { flag: `${converter===undefined ? 's' : 'o'}4.${msg_counter}` , IO:false }, message:message});
                    ...
                    if (readKeys && c.hasOwnProperty('convertGet')) {
                        readKeys.forEach(function(rk) { if (c.key.includes(rk.key)) rk.converter = c; })
                    }
                ...
                const message = `convert ${key} with value ${safeJsonStringify(preparedValue)} and options ${safeJsonStringify(preparedOptions)} for device '${this.devLabel(deviceId, model)}' ${stateDesc.epname ? `with Endpoint ${epName}` : 'without Endpoint'}`;
                if (has_elevated_debug) {
                ...
                if (model === 'group') {
                    ...
                    if (this.debugActive) this.debug(`target: ${safeJsonStringify(utils.groupData(target, {group:true}))}`);
                } else {
                    target = await this.resolveEntity(deviceId, epName);
                    target = target.endpoint;
                    if (this.debugActive) this.debug(`target: ${safeJsonStringify(utils.endpointData(target, {}))}`);
                }


                meta.endpoint_name= stateDesc.epname;
                meta.options=preparedOptions;
                meta.message = {[key]: preparedValue};

                const metaStates = utils.getEntryArray(options?.meta_state);
                const metaMessages = utils.getEntryArray(options?.meta_message);
                const brightness = (key === 'brightness') ? Number(value) : undefined;
                for (const metaKey of metaStates) {
                    if (meta.state[metaKey] === undefined) meta.state[metaKey] = '';
                }
                if (metaStates.includes(key)) {
                    meta.state[key] = preparedValue;
                }
                for (const mm of metaMessages) {
                    if (meta.state[mm] != undefined && meta.message[mm] == undefined) meta.message[mm] = meta.state[mm];
                }
                if (meta.message.color) {
                    if (meta.message.color.hue || meta.message.color.saturation) {
                        meta.message.color = {
                            h: meta.message.color.hue ?? 1,
                            s: meta.message.color.saturation ?? 100,
                            v: brightness ?? meta.state.brightness ?? 100
                        }
                    }
                }
                if (meta.state?.color) {
                    if (meta.state.color.hue || meta.state.color.saturation) {
                        meta.state.color = {
                            h: meta.state.color.hue ?? 1,
                            s: meta.state.color.saturation ?? 100,
                            v: brightness ?? meta.state.color.brightness ?? 100
                        }
                    }
                }

                /*
                if (meta.message.brightness && key == 'state') {
                    if (metaMessages.includes('brightness')) {
                        meta.state.offset = -(meta.state.offset ? meta.state.offset : 1)
                        meta.message.brightness += meta.state.offset;
                    }
                    if (meta.state.brightness != undefined) meta.state.brightness = meta.message.brightness;
                }
                */
                const rms = !metaStates.includes('state');
                ...
                if (preparedOptions !== undefined) {
                    if (preparedOptions.hasOwnProperty('state')) {
                        meta.state.state = preparedOptions.state;
                    }
                }
                let retry = 2;
                let OCnt = 5;
                ...
                do
                {
                    try {
                        ...
                        if (metaMessages.includes('brightness') && key == 'state' && preparedValue == 'ON') {
                            offset = meta.state.brightness == undefined ? 0 : meta.state.brightness < 127 ? 1 : -1
                        }
                        if (offset) {
                            meta.state.brightness += offset;
                            meta.message.brightness += offset;
                        }
                        if (has_elevated_debug) {
                            const message = `Attempt ${3 - retry} at converting data for ${key} with preparedValue ${JSON.stringify(preparedValue)}, meta.state ${JSON.stringify(meta.state)}, meta.message ${JSON.stringify(meta.message)}`;
                            this.emit('device_debug', { ID:debugID, data: { flag: `0${OCnt++}`, states:[{id:stateDesc.id, value:value, payload:preparedValue, ep:stateDesc.epname}] , IO:false }, message});
                        }
                        let result = await converter.convertSet(target, key, preparedValue, meta);
                        if (options.store_colormode) {
                            if (result?.state?.color_mode)
                                meta.state.color_mode = result.state.color_mode;
                        }
                        if (offset) {
                            meta.state.brightness -= offset;
                            meta.message.brightness -= offset;
                            if (has_elevated_debug) {
                                const message = `meta_message:brightness: attempting to correct brightness: key:'brigthtness', preparedValue ${JSON.stringify(meta.state.brightness)}, meta.state ${JSON.stringify(meta.state)}, meta.message ${JSON.stringify(meta.message)}`;
                                this.emit('device_debug', { ID:debugID, data: { flag: `0${OCnt++}`, states:[{id:'brightness', value:meta.state.brightness, payload:meta.state.brightness, ep:stateDesc.epname}] , IO:false }, message});
                            }
                            result = await converter.convertSet(target, 'brightness', meta.state.brightness, meta)
                        }
                        /*
                        if (meta.message.offset) {
                            if (meta.message.brightness) meta.message.brightness -= meta.message.offset;
                            if (meta.state.brightness != undefined) meta.state.brightness = meta.message.brightness;
                        }
                        */
                        utils.updateMeta(this.adapter, `${utils.zbIdorIeeetoAdId(this.adapter, deviceId, false)}`,meta, [...metaStates, ...metaMessages, 'color_mode']);
                        ...
                        const message = `convert result ${safeJsonStringify(result)} for device '${this.devLabel(deviceId, model)}'`;
                        if (isGroup)
                        ...
                        if (result !== undefined) {
                            ...
                            if (has_elevated_debug) {
                                ...
                                const message = `Convert does not return a result result for ${key} with ${stringvalue} on device '${this.devLabel(deviceId, model)}'.`;
                        ...
                        if (readKeys) {
                            for (const readKey of readKeys) {
                                if (readKey.converter) {
                                    ...
                                    const c = readKey.converter;
                                    setTimeout(async () => {
                                        try {
                                            await c.convertGet(target, key, {device:entity.device}) }
                                        catch (error) {
                                            this.warn(`error reading ${key} from '${this.devLabel(deviceId, model)}' : ${error?.message}`);
                                        }
                                    }, tt ? tt * 1000+250 : 1000);
                                }
                            }
                        ...
                        if (error.code === 25) {
                            if (retry > 0) {
                                this.warn(`Error ${error.code} on send command to '${this.devLabel(deviceId, model)}'. (${retry} tries left.), Error: ${error.message}`);
                                retry--;
                                ...
                                this.adapter.filterError(`Error ${error.code} on send command to '${this.devLabel(deviceId, model)}'.` +
                                ` Error: ${error.message}`, `Send command to ${deviceId} failed with`, error, `send:${String(deviceId).replace('0x', '')}`);
                                if (has_elevated_debug) {
                                    const message = `caught error ${error?.message ?? 'no reason given'} when setting value for device '${this.devLabel(deviceId, model)}'.`;
                            ...
                            ` Error: ${error.message}`, `Send command to ${deviceId} failed with`, error, `send:${String(deviceId).replace('0x', '')}`);
                            if (has_elevated_debug) {
                                const message = `caught error ${error?.message ?? 'no reason given'} when setting value for device '${this.devLabel(deviceId, model)}' (${error?.stack ?? ''}).`;
                                this.emit('device_debug', { ID:debugID, data: { error: `EXSET${error.code == 25 ? retry : ''}` , IO:false },message:message});
                            }
                        }
                    }
            ...
            const message = `No entity for '${this.devLabel(deviceId, model)}' : ${err && err.message ? err.message : 'no error message'}`;
            if (has_elevated_debug) {
                this.emit('device_debug', { ID:debugID, data: { error: 'EXPUB' , IO:false }, message:message});
            }
            else this.error(`error EXPUB: ${message}`);
        }
    }
    ...
    async publishPayload(payload, debugID, has_elevated_debug) {
        ...
        if (payloadObj.hasOwnProperty('device') && payloadObj.hasOwnProperty('payload')) {
            try {
                ...
                try {
                    await this.publishFromState(`0x${payloadObj.device}`, payloadObj.model, payloadObj.stateModel, stateList, payloadObj.options, debugID, has_elevated_debug);
                    if (payloadObj.acknowledge) {
                        this.emit('acknowledge_state', payloadObj.device, payloadObj.model, { id:'send_payload' }, undefined);
                    }
                    return {success: true};
                } catch (error) {
                    // payloadObj, not payload: payload may be the raw JSON string (no .device)
                    this.error(`Error ${error.code} on send command to '${this.devLabel(payloadObj.device, mappedModel.model)}'.` + ` Error: ${error.stack} ` + `Send command to '${this.devLabel(payloadObj.device, mappedModel.model)}' failed with ` + error);
                    this.adapter.filterError(`Error ${error.code} on send command to '${this.devLabel(payloadObj.device, mappedModel.model)}'.` + ` Error: ${error.stack}`, `Send command to '${this.devLabel(payloadObj.device, mappedModel.model)}' failed with`, error);
    ...
    recursiveCollectStates(exposes) {
        const rv = [];
        for (const expose of exposes) {
            if (expose.features && expose.type != 'composite') rv.push(...this.recursiveCollectStates(expose.features));
            const candidate = expose.name ?? expose.property;
            if (candidate) rv.push({key:candidate, epName:expose.endpoint});
        }
        return rv;
    ...
    async doDeviceQuery(deviceId, debugID, elevated) {
        const entity = await this.resolveEntity(deviceId);
        const model = entity?.device?.modelID ?? 'unknown';
        if (!(entity?.type === 'device')) return; // not supported
        ...
        if (mappedModel.model === 'Coordinator') return; // no states to read for the coordinator
        ...
        if (elevated) {
            const message  = `Device query for '${this.devLabel(entity.device.ieeeAddr, mappedModel.model)}' triggered`;
            ...
            this.info(`Device query for '${this.devLabel(entity.device.ieeeAddr, mappedModel.model)}' started`);
        }
        const meta = await utils.getMeta(this.adapter, `${utils.zbIdorIeeetoAdId(this.adapter, deviceId, false)}`);

        meta.device= entity.device;
        meta.mapped = mappedModel;
        meta.logger = this;
        meta.state = meta.state ?? {};
        meta.publish = function(data) {};
        ...
        for (const target of targets) {
            ...
            for (const candidate of source) {
                ...
                if (typeof candidate == 'function') {
                    this.warn('function encountered');
                }
                // keyless converters - only to be used if there is no matching keyed converter
                if (!converter && !(candidate.key)) {
                    converter = candidate;
                }
            ...
            else delete meta.endpoint_name;
            ...
            try {
                const ep = this.enpdointFromEpName(entity, target.epName);
                await converter.convertGet( ep,target.key, meta);
                //await new Promise((resolve) => { setTimeout(resolve, 2000); });
                read++;
                if (elevated) {
                    const message = `reading ${target.key} from Device ${this.devLabel(entity.device.ieeeAddr, mappedModel.model)}} ${target.epName ? 'Endpoint '+target.epName : ''} in ${(Date.now()-start)} ms`;
                    this.emit('device_debug', { ID:debugID, data: { flag: `qs${attempt}` , IO:false }, message:message});
                }
            }
            catch (e) {
                if (elevated) {
                    const message = `failed to read ${target.key} from Device ${this.devLabel(entity.device.ieeeAddr, mappedModel.model)}} ${target.epName ? 'Endpoint '+target.epName : ''} after ${(Date.now()-start)} ms`;
                    this.emit('device_debug', { ID:debugID, data: { flag: `qe${attempt}` , IO:false }, message:message});
                }
                else this.warn(`Error while querying device: ${e?.message ?? 'no message given'}`);
            }
        }

        if (elevated) {
            const message = `Device query for '${this.devLabel(entity.device.ieeeAddr, mappedModel.model)}}' complete`;
            this.emit('device_debug', { ID:debugID, data: { flag: 'SUCCESS' , IO:false , payload: `queried ${read} of ${targets.length} possible states` }, message:message});
        }
        else
            this.info(`Device query for '${this.devLabel(entity.device.ieeeAddr, mappedModel.model)}' complete, read ${read} of ${targets.length} possible states`);
        ...
        try {
            ...
            if (epid != undefined) {
                for (const ep of entity.endpoints) {
                    if (this.debugActive) this.debug(`checking ep ${ep.ID} of '${this.devLabel(devId, entity.mapped?.model)}' (${epid})`);
                    if (ep.ID == epid) {
                        ...
                        else this.error(`cluster genGroups not supported for endpoint ${epid} of '${this.devLabel(devId, entity.mapped?.model)}'`);
                    }
                }
            } else {
                if (entity.endpoint.inputClusters.includes(4)) {
                    this.info(`adding endpoint ${entity.endpoint.ID} of '${this.devLabel(devId, entity.mapped?.model)}' to group`);
    ...
    async removeDevFromGroup(devId, groupId, epid) {
        ...
        try {
                ...
                if (epid != undefined) {
                    for (const ep of entity.endpoints) {
                        ...
                        if (ep.ID == epid && (ep.inputClusters.includes(4) || ep.outputClusters.includes(4))) {
                            ...
                            this.info(`removing endpoint ${ep.ID} of '${this.devLabel(devId, entity.mapped?.model)}' from group ${groupId}`);
                        }
                    }
                } else {
                    await entity.endpoint.removeFromGroup(group);
                    this.info(`removing endpoint ${entity.endpoint.ID} of '${this.devLabel(devId, entity.mapped?.model)}' from group ${groupId}`);
                }
            }
            else {
                const msg = `Unable to resolve group ${groupId} when removing '${this.devLabel(devId, entity.mapped?.model)}' (ep ${epid ? epid : (entity ? entity.endpoint.ID : '')}) from group`
                this.error(msg);
                return {error: msg};
            }
            ...
            this.sendError(error);
            const msg = `Exception when trying remove '${this.devLabel(devId, entity?.mapped?.model)}' (ep ${epid ? epid : (entity ? entity.endpoint.ID : '')}) from group ${groupId}`
    ...
    async reset(mode, callback) {
        try {
            await this.herdsman.reset(mode);
            callback && callback();
        } catch (error) {
            this.sendError(error);
