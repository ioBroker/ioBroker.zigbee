...
class ZigbeeController extends EventEmitter {
    ...
    */
    ...
    async permitJoin(permitTime, devid, failure) {
        ...
        try {
            if (permitTime && !this.herdsman.getPermitJoin()) {
                ...
                await this.herdsman.adapter.permitJoin(this._permitJoinTime);
                await this.herdsman.greenPower.permitJoin(this._permitJoinTime);
                ...
                this._permitJoinInterval = setInterval(async () => {
                    ...
                    if (this._permitJoinTime === 0) {
                        ...
                        await this.herdsman.adapter.permitJoin(0);
    ...
    {
        try {
            this.warn(`Error in PermitJoinChanged:  ${JSON.Stringify(error.message)}`);
        }
            catch (error) {
            this.error(`Error in PermitJoinChanged:  ${error.message}`);    
        }
...
/*
        try {
            ...
            */
           this.error('Map currently not supported');
    }
