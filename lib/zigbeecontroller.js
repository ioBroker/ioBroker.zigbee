        ...
        try {
            ...
            stateList.forEach(async changedState => {
                ...
                try {
                    ...
                    if (has_elevated_debug) {
                        if (deviceId.includes('87a3efffe39a49')) {
                            const buffer = Buffer.from(0x10,0,0, 0,0,0,200, 0,0,0,100);
                            try {
                                target.write(`customClusterEwelink`,{[0x5008]:{value:buffer, type: 0x42}},{disableDefaultResponse:false} );
                            }
                            catch (error) {
                                this.error(`${error && error.message ? error.message : 'meh'} : ${error && error.stack ? error.stack : 'mehmeh'} : ${JSON.stringify(target)}`);
                            }
                        }
