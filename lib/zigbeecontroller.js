...
//        this.warn(`Trying to add device ${devId} to group ${groupId}`)
        const callback = (error, rsp) => {
//            this.warn("addDevToGroup callback called with error, "+JSON.stringify(error) + " rsp " + JSON.stringify(rsp))
            if (error) {
        ...
        try {
            ...
            try {
                await entity.endpoint.configureReporting('genOnOff', [{
                    attribute: 'onOff',minimumReportInterval: 0,
                    maximumReportInterval: 3600, reportableChange: 1
                }]);

                await entity.endpoint.configureReporting('genLevelCtrl', [{
                    attribute: 'currentLevel',minimumReportInterval: 0,
                    maximumReportInterval: 3600, reportableChange: 1
                }]);
            }
            catch (error) {
                this.error(`Exception when trying to configure reporting for ${devId} when adding to group ${groupId}`, error);
            }
    ...
    async removeDevFromAllGroups(devId, success) {
        this.warn(`Trying to remove ${devId} from all groups`)
        const callback = (error, rsp) => {
//            this.warn("removeDevFromAllGroups callback called with error, "+JSON.stringify(error) + " rsp " + JSON.stringify(rsp))
            if (error) {
