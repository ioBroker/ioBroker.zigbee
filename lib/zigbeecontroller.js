...
class ZigbeeController extends EventEmitter {
    ...
    */
    ...
    async start() {
        ...
        if (this.disableLed) {
            try {
              ...
              this.warn('Adapter Start:', "Unable to disable LED, unsupported function.");
            }
        }
       // Call extensions
    ...
    // Remove device
    async remove(deviceID, callback) {
        ...
        try {
            ...
            for (const device of devices.filter((d) => d.type != 'EndDevice')) {
                ...
                try {
                    ...
                    this.error(`Failed to execute LQI for '${resolved.name}'. ${safeJsonStringify(error.stack)}`);
                }
                ...
                if (includeRoutes) {
                    try {
                        ...
                        this.error(`Failed to execute routing table for '${resolved.name}'. ${safeJsonStringify(error.stack)}`);
                    }
                }
            }
    ...
    async publish(deviceID, cid, cmd, zclData, cfg, ep, type, callback) {
        ...
        this.debug('endpoint command returned ' + JSON.stringify(endpoint.command(cid, cmd, zclData)));
    }
