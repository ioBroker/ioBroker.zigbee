...
class ZigbeeController extends EventEmitter {
    ...
    */
            ...
            if (group) {
                ...
                for (const member of groupMembers) {
                    ...
                    if (device && device.ieeeAddr) {
                        ...
                        debugMessage.push(utils.endpointData(member, false))
                    }
                }
            } else {
                return undefined;
    ...
    recursiveObjectCompare(a, b) {
        if (typeof a != typeof b) return false;
        if (typeof a == 'object') {
            for (const key of Object.keys(a)) {
                if (this.recursiveObjectCompare(a[key], b[key])) continue;
                return false;
            }
            return true;
        }
        ...
        try {
            ...
            for (const changedState of stateList) {
                ...
                if (model === 'group') {
                    ...
                    if (this.debugActive) this.debug(`target: ${safeJsonStringify(utils.groupData(target, true))}`);
                } else {
                ...
                const metaStates = utils.getMetaStates(options?.meta_state ?? '');
                const metaMessages = utils.getMetaStates(options?.meta_message ?? '');
                ...
                if (converter.hasOwnProperty('convertGet')) {
                    ...
                    for (const source of sources) {
                        for (const k of converter.key)
                            if (keyList === undefined || keyList.includes(k))
                                try {
