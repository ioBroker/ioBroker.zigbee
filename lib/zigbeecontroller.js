...
class ZigbeeController extends EventEmitter {
    ...
    */
    ...
    constructor(shepherd) {
        ...
        /**
         * Setup command queue.
         * The command queue ensures that not too many commands are executed at a time.
         * Else some commands may fail or coordinator may crash
         */
        this.queue = new Queue((task, cb) => {
            task.action(cb);
        }, {
            afterProcessDelay: 250,
            concurrent: 4,
            maxTimeout: 500,
            priority: (task, cb) => {
                return cb(null, task.priority);
            }
        });
    ...
    publish(deviceID, cid, cmd, zclData, cfg, ep, type, callback, options) {
        ...
        this.queue.push({
            ...
            priority: options && options.hasOwnProperty('priority') ? options.priority : prio.low
        });
    }
    ...
    removeDevFromAllGroups(devId, success) {
        const callback = (error, rsp) => {
            if (error) {
                this.error(`Failed to remove dev ${devId} from all groups`, error);
            } else {
                this.debug(`Successfully removed dev ${devId} from all groups: ${safeJsonStringify(rsp)}`);
                success && success(rsp);
            }
        };
    ...
    startRouterPoll() {
        ...
        this.pollTimer = setInterval(() => {
            ...
            this.debug(`PublishToZb stats (since start): max pending items: ${queueStats.peak}, total processed items ${queueStats.total}`);
        }, 60 * 1000); // 1minute
    }
