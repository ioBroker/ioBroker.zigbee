...
class ZigbeeController extends EventEmitter {
    ...
    */
    ...
    async addDevToGroup(devId, groupId, epid) {
        try {
            this.warn(`addDevFromGroup with ${devId}, ${groupId}, ${epid}`)
                  ...
                  this.warn(`checking ep ${ep.ID} of ${devId} (${epid})`)
                    if (ep.ID == epid) {
                      if (ep.inputClusters.includes(4) || ep.outputClusters.includes(4)) {
                        this.warn(`adding endpoint ${ep.ID} (${epid}) to group ${groupId}`);
                      ...
                      else this.warn(`cluster genGroups not supported for endpoint ${epid} of ${devId}`)
                    }
                }
            } else {
