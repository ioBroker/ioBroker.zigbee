    ...
    async publishFromState(deviceId, model, stateModel, stateList, options, debugID, has_elevated_debug) {
        ...
        try {
            ...
            stateList.forEach(async changedState => {
                ...
                //-=-=-//
                if (options?.meta_state) {
                    const parts = options.meta_state.split(',');
                    for (const s of parts) {
                        const state = s.trim();
                        const sD = stateModel.states.find((o) => o.id === state)
                        if (!(meta.state && meta.state.hasOwnProperty(state))) {
                            const s = await this.adapter.getState(`${utils.zbIdorIeeetoAdId(this.adapter, deviceId, false)}.${state}`);
                            if (sD.setter) {
                                meta.state[state] = sD.setter(s.val)
                            } else meta.state[state] = s.val;
                        }
                    }
                }
