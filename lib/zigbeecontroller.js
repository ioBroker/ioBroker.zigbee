...
*/
    ...
    async tryConfigureReporting(entity, cluster, attributes) {
        if (!entity.inputClusters.includes(cluster)) return;
        ...
        try {
            this.warn(`tryConfigureReporting Entity: ${JSON.stringify(entity)} Cluster: ${cluster} attributes: ${JSON.stringify(attr)}`)
            await entity.configureReporting(cluster, reportables)
            this.warn('reporting configured');
        } catch (error) {
            this.warn(`error on configure reporting: ${error && error.message ? error.message : 'no details given'} ${error && error.stack ? error.stack : ''}`);
        }

    }

    async triggerReporting(entity) {
        // check if the cluster is supported
        //this.tryConfigureReporting(entity, 6,['onOff']);
        //this.tryConfigureReporting(entity, 8,['currentLevel']);
        //this.tryConfigureReporting(entity, 768,['currentHue','currentSaturation','currentX','currentY','colorTemp']);
        this.tryConfigureReporting(entity, 6,[0]);
        this.tryConfigureReporting(entity, 8,[0]);
        this.tryConfigureReporting(entity, 768,[0,1,3,4,7]);
    }
