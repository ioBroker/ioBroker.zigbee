...
//            this.error('error getting group for ' + id + ' ' + (error && error.message ? error.message : 'without message'));
    ...
    // resolvEntity accepts:
    // IEEE address as string without the leading 0x
    // The text 'coordinator'
    // The numeric ID of a group
    // the text group_x, with x being the numeric ID of a group
    // an object
/*
    async resolveEntity(key, ep, debug) {
        // assert(typeof key === 'string' || key.constructor.name === 'Device', `Wrong type '${typeof key}'`);

        const GroupID = Groups.extractGroupID(key);
        if (GroupID> 0) key = GroupID;
if (debug) this.warn('key is ' + typeof key)
        if (typeof key === 'string') {
            ...
            const _key = (key.startsWith('0x') ? key : `0x${key}`);
            const device = await this.herdsman.getDeviceByIeeeAddr(_key);
            if (device) {
