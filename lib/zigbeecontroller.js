    ...
    */
    ...
    async getGroups() {
        try {
            if (this.herdsman) {
                const rv = [];
                const groupArr =  await this.herdsman.getGroups();
                for (const g of groupArr) {
                    const memberCount = await this.getGroupMembersFromController(g.groupID).length;
                    rv.push({ id: g.groupID, size: memberCount });
                }
                return rv;
            } else {
                return null;
