...
class ZigbeeController extends EventEmitter {
    ...
    */
    ...
    async updateMeta(devId, meta, metaStates) {
        ...
        try {
            const existingMeta = await utils.getMeta(this.adapter, devId);
            ...
            if (changed) {
                if (Object.keys(existingMeta).length() < 1)
                    await this.adapter.extendObject(devId, {native: { meta: undefined }});
                else
            ...
            for (const changedState of stateList) {
                ...
                do
                {
                    try {
                        ...
                        utils.updateMeta(this.adapter, `${utils.zbIdorIeeetoAdId(this.adapter, deviceId, false)}`,meta, metaStates)
