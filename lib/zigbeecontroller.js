    ...
    */
    ...
    start(callback) {
        ...
        this.shepherd.start((err) => {
            if (err) {
                this.info('Error while starting zigbee-shepherd, attemping to fix... (takes 60 seconds)');
    ...
    findGroup(groupId, success, fail) {
        const _callback = (error, rsp) => {
            if (error) {
                this.debug(`Failed to find group in coordinator`, error);
                if (fail) fail(error);
            } else {
                this.debug(`Successfully find group in coordinator: ${safeJsonStringify(rsp)}`);
                if (success) success(rsp);
            }
        };
        this.shepherd.controller.request('ZDO', 'extFindGroup', {endpoint: 1, groupid: groupId}, _callback);
    }

    addGroup(groupId, name, success) {
        const callback = (error, rsp) => {
            if (error) {
                this.error(`Failed to add group to coordinator`, error);
            } else {
                this.debug(`Successfully add group to coordinator: ${safeJsonStringify(rsp)}`);
                if (success) success(rsp);
            }
        };
        this.findGroup(groupId, null, (error) => {
            const payload = {endpoint: 1, groupid: groupId, namelen: name.length, groupname: name};
    ...
    removeGroup(groupId, success, fail) {
        const _callback = (error, rsp) => {
            if (error) {
                this.error(`Failed to remove group from coordinator`, error);
                if (fail) fail(error);
            } else {
                this.debug(`Successfully remove group from coordinator: ${safeJsonStringify(rsp)}`);
                if (success) success(rsp);
            }
        };
        this.shepherd.controller.request('ZDO', 'extRemoveGroup', {endpoint: 1, groupid: groupId}, _callback);
    }
    ...
    addDevToGroup(devId, groupId, success) {
        const callback = (error, rsp) => {
            if (error) {
                this.error(`Failed to add device to group`, error);
            } else {
                this.debug(`Successfully add device to group: ${safeJsonStringify(rsp)}`);
                if (success) success(rsp);
            }
        };
        this.publish(devId, 'genGroups', 'add', {groupid: groupId, groupname: ''}, null, null, 'functional', callback);
    }

    removeDevFromAllGroups(devId, success) {
        const callback = (error, rsp) => {
            if (error) {
                this.error(`Failed to revove dev from all groups`, error);
            } else {
                this.debug(`Successfully revove dev from all groups: ${safeJsonStringify(rsp)}`);
                if (success) success(rsp);
            }
        };
    ...
    startRouterPoll() {
        ...
        this.pollTimer = setInterval(() => {
            this.getAllClients()
                ...
                .forEach((d) => this.checkOnline(d.ieeeAddr)); // Ping devices.
        }, 60 * 1000); // 1minute
    }
}
