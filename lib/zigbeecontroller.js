    ...
    */
    ...
    start(callback) {
        ...
        this.shepherd.start((err) => {
            if (err) {
                this.error('Error while starting zigbee-shepherd!', err);
    ...
    handleReady() {
        ...
        this.info('zigbee-shepherd ready');
        this.emit('ready');
    }
    ...
    permitJoin(permit, failure) {
        ...
        this.shepherd.permitJoin(permit ? permit : 0, (error) => {
            if (error) {
    ...
    remove(deviceID, callback) {
        this.shepherd.remove(deviceID, { reJoin: false }, (error) => {
            if (error) {
    ...
    publish(deviceID, cid, cmd, zclData, ep, callback) {
        ...
        this.debug(`Zigbee publish to '${deviceID}', ${cid} - ${cmd} - ${JSON.stringify(zclData)} - ${ep}`);
