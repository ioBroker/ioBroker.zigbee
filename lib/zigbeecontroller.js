    ...
    */
    ...
    setDeviceEnable(id, enable) {
        try {
            ...
            if (enable) {
                this.disabledDevices.add(ieee);
            } else {
                this.disabledDevices.delete(ieee);
            }
    ...
    async testConnectable(port) {
        ...
        if (netAddress && netAddress.strAddress) {
            return netAddress.strAddress;
        }
        try {
    ...
    async start() {
        try {
            ...
            if (this.debugActive) {
                this.debug(`Network parameters: panID=${debNetworkParam.panID} channel=${debNetworkParam.channel} extendedPanID=${this.reverseIEEE(extendedPanIDDebug)}`);
            }
        } catch (e) {
            try {
                const debNetworkParam = JSON.parse(JSON.stringify(await this.herdsman.getNetworkParameters()));
        ...
        try {
            ...
            for (const group of Groups) {
                if (this.ListDevicesAtStart) {
                    this.info(`${group.stateName} (addr ${group.id}): Group with ${group.size} members.`);
                }
            }
    ...
    filterHerdsmanError(message) {
        ...
        if (message.match(this.herdsmanTimeoutRegexp)) {
            return 'Timeout';
        }
        return message;
    }
    ...
    getClientIterator(all) {
        if (this.herdsman.database) {
            return this.herdsman.getDevicesIterator(function (device) {
                return (all ? true : device.type !== 'Coordinator')
            });
        }
        return [].values();
    ...
    async getCoordinatorIeee() {
        ...
        return coordinatorCandidates[0]?.ieeeAddr ?? '0xffffffffffffffff';
    }
                ...
                if (ep_binds && ep_binds.length) {
                    ...
                    for (const bind of ep_binds) {
                        ...
                        if (!bindObj.binds.hasOwnProperty(ckey)) {
                            ...
                            bindObj.binds[ckey].clusters.push(bind.cluster.ID);
                        }
    ...
    async reconfigure(devId) {
        if (!this.herdsmanStarted) {
            return {error: 'No active connection to Zigbee Hardware!'};
        }
    ...
    async resolveEntity(key, ep) {
        try {
            ...
            if (_key.kind === 'group') {
                ...
                if (!group) {
                    group = await this.herdsman.createGroup(_key.key);
                }
                group.toZigbee = groupConverters;
                group.model = 'group';
                return {
        ...
        try {
            ...
            if (this.adapter.stController.checkDebugDevice(friendlyName)) {
                this.emit('device_debug', {
                    ID: Date.now(),
                    data: {flag: 'dl', states: [{id: '--', value: '--', payload: message}], IO: true},
                    message: `Device '${friendlyName}' has left the network`
                });
            } else {
    ...
    async handleDeviceAnnounce(message) {
        ...
        if (this.adapter.stController.checkDebugDevice(friendlyName)) {
            this.emit('device_debug', {
                ID: Date.now(),
                data: {flag: 'da', states: [{id: '--', value: '--', payload: message}], IO: true},
                message: `Device '${friendlyName}' announced itself`
            });
        }
        ...
        */
        try {
            if (entity && entity.mapped) {
                ...
                } else if (this.readAtAnnounce) {
                    await this.doDeviceQuery(message.device || message.ieeeAddr, Date.now(), false);
                }
    ...
    async handleDeviceInterview(message) {
        if (this.debugActive) {
            this.debug('handleDeviceInterview', JSON.stringify(message));
        }
            ...
            switch (message.status) {
                case 'successful': {
                    ...
                    if (entity.mapped) {
                        ...
                        if (model) {
                            msgArr.push(`model: ${model}`);
                        } else {
                            msgArr.push(`model: unknown`);
                        }

                        if (vendor) {
                            msgArr.push(`vendor: ${vendor}`);
                        }
                        if (description) {
                            msgArr.push(`description: ${description}`);
                        }
                        msgArr.push('supported:true');
                        onZigbeeEventparameters[0] = {
                            ...message,
        ...
        try {
            ...
            for (const device of devIter) {
                ...
                if (attemptRouting) {
                    ...
                    errors.push(`Omitted collecting routing table for '${resolved?.name || 'unresolved device'} (${resolved?.device?.modelID || 'unknown'}') : LQI timed out`);
                }
    ...
    async getMeta(devId) {
        try {
            ...
            this.error(`error getting meta for ${devId} - ${error?.message ?? 'no message given'}`);
        }
        return {};
    }
    ...
    async updateMeta(devId, meta) {
        ...
        delete meta.mapped;
        delete meta.message;
        delete meta.endpoint_name;
        try {
            const existingMeta = await this.getMeta(devId);
            const message = JSON.stringify(existingMeta);
            let changed = false;
            for (const key of Object.keys(meta)) {
                if (!this.recursiveObjectCompare(meta[key], existingMeta[key])) {
                    existingMeta[key] = meta[key];
                    changed = true;
                }
            }
            if (changed) {
                // this.warn(`zbController updated meta for ${devId} from ${message} to ${JSON.stringify(existingMeta)}`);
                await this.adapter.extendObject(devId, {native: {meta: existingMeta}});
            }
            //else this.warn(`zbController no update meta for ${devId} => meta was unchanged`);
        } catch (error) {
            this.warn(`zbController error updating meta for ${devId} : ${error?.message ?? 'no error message given'}`);
        }
    }
    ...
    getReadKeys(stateDesc, deviceId, model) {
        try {
            ...
            if (allowedKeys && stateDesc.readKeys) {
                return stateDesc.readKeys.map(function (o) {
                    return {key: o, converter: undefined}
                }).filter((o) => allowedKeys.includes(o.key));
            }
            return undefined;
        } catch (error) {
    ...
    async publishFromState(deviceId, model, stateModel, stateList, options, debugID, has_elevated_debug) {
        ...
        if (has_elevated_debug) {
            ...
            this.debug(`main publishFromState : ${deviceId} ${model} ${safeJsonStringify(stateList)}`);
        }
        if (model === 'group') {
            isGroup = true;
        ...
        try {
            ...
            if (this.debugActive) {
                this.debug(`entity: ${deviceId} ${model} ${safeJsonStringify(entity)}`);
            }
            const mappedModel = entity ? entity.mapped : undefined;

            if (!mappedModel) {
                if (this.debugActive) {
                    this.debug(`No mapped model for ${model}`);
                }
                if (has_elevated_debug) {
            ...
            const meta = await this.getMeta(`${utils.zbIdorIeeetoAdId(this.adapter, deviceId, false)}`);
            ...
            for (const changedState of stateList) {
                ...
                if (stateDesc.isOption) {
                    if (has_elevated_debug) {
                        ...
                        this.debug(`No converter needed on option state for ${deviceId} of type ${model}`);
                    }
                ...
                for (const c of mappedModel.toZigbee) {
                    ...
                    if (!c.hasOwnProperty('convertSet')) {
                        continue;
                    }
                    if (this.debugActive) {
                        this.debug(`Type of toZigbee is '${typeof c}', Contains key ${(c.hasOwnProperty('key') ? JSON.stringify(c.key) : 'false ')}`)
                    }
                    if (!c.hasOwnProperty('key')) {
                        if (converter === undefined) {
                            if (readKeys && c.hasOwnProperty('convertGet')) {
                                readKeys.forEach(function (rk) {
                                    if (rk.key && rk.converter === undefined) {
                                        rk.converter = c
                                    }
                                });
                            }
                            ...
                            if (has_elevated_debug) {
                                ...
                                this.emit('device_debug', {
                                    ID: debugID,
                                    data: {flag: `s4.${msg_counter}`, IO: false},
                                    message: message
                                });
                            } else if (this.debugActive) {
                                this.debug(`Setting converter to keyless converter for ${deviceId} of type ${model}`);
                            }
                            msg_counter++;
                        } else {
                            if (has_elevated_debug) {
                                ...
                                this.emit('device_debug', {
                                    ID: debugID,
                                    data: {flag: `i4.${msg_counter}`, IO: false},
                                    message: message
                                });
                            } else if (this.debugActive) {
                                this.debug(`ignoring keyless converter for ${deviceId} of type ${model}`);
                            }
                            msg_counter++;
                        }
                        continue;
                    ...
                    if (c.key.includes(stateDesc.prop) || c.key.includes(stateDesc.setattr) || c.key.includes(stateDesc.id)) {
                        ...
                        if (has_elevated_debug) {
                            this.emit('device_debugug', {
                                ID: debugID,
                                data: {flag: `${converter === undefined ? 's' : 'o'}4.${msg_counter}`, IO: false},
                                message: message
                            });
                ...
                if (converter === undefined) {
                    ...
                    if (has_elevated_debug) {
                        this.emit('device_debug', {
                            ...
                            data: {
                                error: 'NOCONV',
                                states: [{id: stateDesc.id, value: value, payload: 'no converter'}],
                                IO: false
                            },
                            message: message
                        });
                ...
                if (has_elevated_debug) {
                    this.emit('device_debug', {
                        ...
                        data: {
                            flag: '04',
                            payload: {key: key, ep: stateDesc.epname, value: preparedValue, options: preparedOptions},
                            IO: false
                        },
                        message: message
                    });
                ...
                do {
                    try {
                        ...
                        this.updateMeta(`${utils.zbIdorIeeetoAdId(this.adapter, deviceId, false)}`, meta)
                        ...
                        if (has_elevated_debug) {
                            ...
                            this.emit('device_debug', {
                                ID: debugID,
                                data: {error: `EXSET${error.code == 25 ? retry : ''}`, IO: false},
                                message: message
                            });
                        }
        ...
        try {
            if (endpoints) {
                for (const ep of Object.keys(endpoints)) {
                    if (key.endsWith(`_${ep}`)) {
                        return {setattr: key.replace(`_${ep}`, ''), epname: ep}
                    }
                }
            }
        } catch {
    ...
    async publishPayload(payload, debugID, has_elevated_debug) {
        ...
        if (typeof payload === 'string') {
            ...
            return {success: false, error: `illegal type of payload: ${typeof payload}`};
        }
    ...
    async doDeviceQuery(deviceId, debugID, elevated, keyList) {
        const entity = await this.resolveEntity(deviceId);
        if (this.debugActive) {
            this.debug(`doDeviceQuery: resolveEntity for entity: ${deviceId} is ${safeJsonStringify(entity)}`);
        }
        const mappedModel = entity ? entity.mapped : undefined;
        if (mappedModel) {
            ...
            if (elevated) {
                ...
                this.emit('device_debug', {
                    ID: debugID,
                    data: {flag: 'qs', states: [{id: 'device_query', value: true, payload: 'device_query'}], IO: false},
                    message: message
                });
            } else if (this.debugActive) {
                this.debug(`Device query for '${entity.device.ieeeAddr}' started`);
            } else {
                this.info(`Device query for '${entity.device.ieeeAddr}' started`);
            }
            ...
            for (const converter of mappedModel.toZigbee) {
                if (converter.hasOwnProperty('convertGet')) {
                    ...
                    if (!converter.endpoints) {
                        sources.add(entity.device.endpoints[0]);
                    }
                ...
                for (const ep of entity.endpoints) {
                    ...
                    if (ep.ID == epid) {
                        if (ep.inputClusters.includes(4) || ep.outputClusters.includes(4)) {
                            ...
                            this.error(`cluster genGroups not supported for endpoint ${epid} of ${devId}`);
                        }
                    }
                }
            } else {
