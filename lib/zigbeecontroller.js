...
class ZigbeeController extends EventEmitter {
    ...
    */
    ...
    async verifyGroupExists(id) {
      ...
      this.warn('verifyGroupExists: checking group ' + nid);
        let group = await this.herdsman.getGroupByID(nid);
        if (!group) {
            this.warn('verifyGroupExists: trying to create group ' + nid);
            group = await this.herdsman.createGroup(nid);
            group.toZigbee = groupConverters;
            group.model = 'group';
            this.warn('verifyGroupExists: created group ' + nid);
        }
        else {
            this.warn('verifyGroupExists: group ' + nid + ' exists');
        }
