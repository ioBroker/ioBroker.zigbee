...
class ZigbeeController extends EventEmitter {
    ...
    */
    ...
    // only for CC1352P and CC26X2R1   
        if (this.transmitPower != 0) {
            this.info("  --> transmitPower : " + this.transmitPower);
        }
    ...
    async remove(deviceID, callback) {
        try {
         ...
         // check without     await device.removeFromNetwork();
        ...
        try {
            ...
            for (const device of devices.filter((d) => d.type != 'EndDevice')) {
                ...
                try {
                    ...
                    if (result !== undefined) {
                      for (const dev of result.neighbors) {
                        ...
                        if (dev.ieeeAddr !== "0xffffffffffffffff" &&
                            dev !== undefined) {
                          lqis.push({
                    ...
                    this.warn(`Failed to execute LQI for '${resolved.name}'. ${safeJsonStringify(error.stack)}`);
                }
                ...
                try {
                    ...
                    this.warn(`Failed to execute routing table for '${resolved.name}'. ${safeJsonStringify(error.stack)}`);
                }
