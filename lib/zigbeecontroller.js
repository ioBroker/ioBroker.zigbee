...
//            if (callback) callback();
    ...
    findGroup(groupId, success, fail) {
        const _callback = (error, rsp) => {
            if (error) {
                this.error(`Failed to find group in coordinator`, error);
                if (fail) fail(error);
            } else {
                this.info(`Successfully find group in coordinator: ${safeJsonStringify(rsp)}`);
    ...
    addGroup(groupId, name, success) {
        const callback = (error, rsp) => {
            if (error) {
                ...
                this.info(`Successfully add group to coordinator: ${safeJsonStringify(rsp)}`);
                if (success) success(rsp);
            }
        };
    ...
    getGroups(success) {
        const callback = (error, rsp) => {
            if (error) {
                ...
                this.info(`Successfully get groups from coordinator: ${safeJsonStringify(rsp)}`);
    ...
    removeAllGroup(success, fail) {
        const _callback = (error, rsp) => {
            if (error) {
                ...
                this.info(`Successfully remove groups in coordinator: ${safeJsonStringify(rsp)}`);
                if (success) success(rsp);
            }
        };
