    ...
    */
    ...
    async resolveEntity(key, ep) {
        ...
        try {
            ...
            if (device) {
                const t = Date.now();
                const mapped = await zigbeeHerdsmanConverters.findByDevice(device, true);
                if (!mapped) this.warn(`Resolve Entity did not manage to find a mapped device for ${device}`);
