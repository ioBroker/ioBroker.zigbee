...
class Exclude {
    ...
    async addExclude(from, command, params, callback) {
        try {
            ...
            const exclude_id = params.exclude_model._id;
        ...
        try {
            ...
            this.adapter.getStateAsync(stateId)
                .then(async (stateV) => {
                    ...
                    this.adapter.deleteState(null, 'exclude', exclude_id, async (err) => {
                        callback();
                    });
                });
        } catch (error) {
...
function getAllExclude() {
    try {
        const excAll = [];
        const adt = `${this.adapter.namespace}.exclude.all`;
        return this.adapter.getStateAsync('exclude.all')
            .then(stateVa => {
                if (stateVa !== null) {
                    const val = {
                        id: this.extractExcludeId(state._id),
        ...
        this.error(`Failed to getAllExclude ${error.stack}`);
    }
}

module.exports = Exclude;
module.exports.getAllExclude = getAllExclude;
