'use strict';

const fs = require('fs');
const path = require('path');
const utils = require('@iobroker/adapter-core'); // Get common adapter utils

const EventEmitter = require('events').EventEmitter;


/**
 *
 */
class localConfig extends EventEmitter {
    /**
     *
     * @param adapter
     * @param options
     */
    constructor(adapter, options) {
        super();
        this.adapter = adapter;
        this.name = 'localConfig';
        this.localData = {by_id: {}, by_model: {}};
        this.filename = undefined;
        this.basefolder = undefined;
        this.retTimeoutHanlde = undefined;
        this.adapter.on('ready', () => this.onReady());
    }

    // adapter option possible values:
    // string - free text to enter
    // numeric - a number, possibli with value_min/value_max, value_step
    // binary - true/false
    // enum - selection, one of the possibilities
    // set - selection, any number of the possibilities

    static AdapterOptions = [{
        label: 'resend States',
        name: 'resend_states',
        type: 'string',
        byDevice: true,
        byModel: true,
        cVal: true,
        cProp: undefined,
    }, {
        label: 'read Brightness/State',
        name: 'link_states',
        type: 'set',
        byDevice: true,
        byModel: true,
        cVal: true,
        cProp: 'readKeys',
    }, {
        label: 'Legacy device',
        name: 'legacy',
        type: 'binary',
        byDevice: false,
        byModel: true,
        cVal: true,
        cProp: 'hasLegacyDef',
    },  /*{ //Option removed as non functional.
        label:'delay Delete',
        name:'delay_device_delete',
        type:'numeric',
        value_min:0,
        value_max:60,
        value_step:1,
        byDevice: false,
        byModel: true,
        cVal: true,
        cProp: undefined,
    }, */ {
        label: 'offline timeout',
        name: 'offline_timeout',
        type: 'numeric',
        value_min: 1,
        value_max: 25,
        value_step: 1,
        byDevice: true,
        byModel: true,
        cVal: 'EndDevice',
        cProp: 'type',
    }, {
        label: 'filter state',
        name: 'filter_state',
        type: 'binary',
        byModel: true,
        cVal: true,
        cProp: 'hasState',
    }, {
        label: 'filter state',
        name: 'meta_state',
        type: 'text',
        byModel: true,
        cVal: true,
        cProp: 'hasState',
    }];

    static version = 1.0;
    static DefaultModelOptions = {
        models: {
            S60ZBTPF: [{key: 'meta_state', value: 'state'}],
            'WT-A03E': [{key: 'meta_state', value: 'sensor'}]
        },
        options: {
            meta_state: ['S60ZBTPF', 'WT-A03E'],
        }
    };

    /**
     *
     */
    async onReady() {
        this.init();
    }

    /**
     *
     * @param callback
     */
    async onUnload(callback) {
        this.retainData();
        // No callback here -
        //callback();
    }

    /**
     *
     * @param message
     */
    info(message) {
        this.adapter.log.info(message);
        this.emit('log', 'info', message);
    }

    /**
     *
     * @param message
     * @param errobj
     */
    error(message, errobj) {
        this.adapter.log.error(`${this.name}:${message}`);
        this.emit('log', 'error', `${this.name}:${message}`, errobj);
    }

    /**
     *
     * @param message
     */
    warn(message) {
        this.adapter.log.warn(`${this.name}:${message}`);
        this.emit('log', 'warn', `${this.name}:${message}`);
    }

    /**
     *
     * @param message
     */
    debug(message) {
        this.adapter.log.debug(`${this.name}:${message}`);
        this.emit('log', 'debug', `${this.name}:${message}`);
    }

    /**
     *
     * @param id
     * @param name
     */
    async updateDeviceName(id, name) {
        this.debug(`updateDev with ${id} and .${name}.`);
        if (typeof id != 'string') {
            this.error(`update called with illegal device entry:${JSON.stringify(id)}`)
            return true;
        }
        if (typeof name != 'string' || name.trim().length < 1) {
            if (this.localData.by_id.hasOwnProperty(id)) {
                delete this.localData.by_id[id].name;
            }
        } else {
            if (this.localData.by_id.hasOwnProperty(id)) {
                this.localData.by_id[id].name = name;
            } else {
                this.localData.by_id[id] = {name: name};
            }
        }
        return true;
    }

    /**
     *
     * @param data
     */
    isValid(data) {
        const t = typeof data;
        const invalidators = ['none', '##REMOVE##'];
        switch (t) {
            case 'number':
                return true;
            case 'string':
                return invalidators.indexOf(data) < 0;
            case 'object':
                return true;
            default:
                return false;
        }
    }

    /**
     *
     * @param _target
     * @param model
     * @param key
     * @param data
     * @param global
     */
    async updateLocalOverride(_target, model, key, data, global) {
        const target = (global ? model : _target);
        this.debug(`updating local data: (${global ? 'global' : 'local'}) : ${target}:${key}:${JSON.stringify(data)}`);

        if (typeof target != 'string' || typeof key != 'string') {
            this.error(`update called with illegal id data:${JSON.stringify(target)}:${JSON.stringify(key)}:${JSON.stringify(data)}`)
            return false;
        }
        const base = global ? this.localData.by_model[target] || {} : this.localData.by_id[target] || {};


        if (data && Object.keys(data).length > 0 && this.isValid(data)) {
            if (key == 'icon') {
                base[key] = data.replace(this.basefolder, '.');
            } else {
                base[key] = data;
            }
        } else {
            if (base.hasOwnProperty(key)) {
                delete base[key]
            }
        }
        if (global) {
            if (base == {}) {
                delete this.localData.by_model[target];
            } else {
                this.localData.by_model[target] = base;
            }
        } else {
            if (base == {}) {
                delete this.localData.by_id[target];
            } else {
                this.localData.by_id[target] = base;
            }
        }
        this.debug(`Local Data for ${target} is ${JSON.stringify(base)} after update`);
        this.retainData();
        return true;
    }

    /**
     *
     * @param target
     * @param model
     * @param key
     * @param global
     */
    getLocalOverride(target, model, key, global) {
        this.debug(`getting local data: (${global ? 'global' : 'local'}) : ${target}:${key}`);

        if (typeof target != 'string' || typeof key != 'string') {
            this.error(`update called with illegal id data:${JSON.stringify(target)}:${JSON.stringify(key)}`)
            return false;
        }
        const rv = {};
        if ((this.localData.by_model[model] || {}).hasOwnProperty(key)) {
            rv[key] = this.localData.by_model[model][key];
        }
        if (global) {
            return rv;
        }
        if ((this.localData.by_id[target] || {}).hasOwnProperty(key)) {
            rv[key] = this.localData.by_id[target][key];
        }
        return rv;
    }

    /**
     *
     * @param id
     * @param model
     * @param defaultName
     */
    NameForId(id, model, defaultName) {
        this.debug(`name for id with ${id} and ${defaultName} from ${JSON.stringify(this.localData)}`);
        const nameObj = this.getLocalOverride(id, model, 'name', false);
        if (nameObj.name) {
            return nameObj.name
        }
        return defaultName;
    }

    /**
     *
     * @param dev
     * @param model
     */
    removeLocalData(dev, model) {
        delete this.localData.by_id[dev];
        if (model) {
            delete this.localData.by_model[model];
        }
        this.retainData();
    }

    /**
     *
     * @param id
     * @param model
     * @param defaultIcon
     */
    IconForId(id, model, defaultIcon) {
        let modeloverride = {};
        this.debug(`Icon for id with ${id}, ${model} and ${defaultIcon}`);
        if (this.localData.by_id.hasOwnProperty(id)) {
            modeloverride = this.localData.by_id[id]
        }
        if (!modeloverride.icon) {
            if (this.localData.by_model.hasOwnProperty(model)) {
                modeloverride = this.localData.by_model[model];
            }
        }
        const iconPath = modeloverride.icon;
        this.debug(`icon Path is  ${JSON.stringify(iconPath)}`);
        if (typeof iconPath != 'string') {
            this.debug(`icon path is no string, returning ${JSON.stringify(defaultIcon)}`);
            return defaultIcon;
        }
        if (iconPath.startsWith('http')) {
            return iconPath;
        }
        const namespace = `${this.adapter.name}.admin`;
        const rv = `img/${path.basename(iconPath)}`;
        try {
            this.adapter.fileExists(namespace, rv, (err, result) => {
                if (result) {
                    return;
                }
                const src = this.adapter.expandFileName(iconPath);
                fs.readFile(src, (err, data) => {
                    if (err) {
                        this.error(`unable to read ${src} : ${err && err.message ? err.message : ' no message given'}`)
                        return;
                    }
                    if (data) {
                        this.adapter.writeFile(namespace, rv, data, (err) => {
                            if (err) {
                                this.error(`error writing file ${path}${JSON.stringify(err)}`)
                                return;
                            }
                            this.debug(`Updated image file ${rv}`)
                        })
                    }
                })

            })
        } catch (error) {
            this.error(`Error accessing target image: ${error && error.message ? error.message : 'no error message'}`);
        }
        return rv;
    }


    /**
     *
     * @param src
     */
    async copyDeviceImage(src) {
        const dst = `${this.adapter.adapterDir}/admin/img/${path.basename(src)}`;
        const _src = this.adapter.expandFileName(src);
        if (fs.existsSync(src) && !fs.existsSync(dst)) {
            try {
                this.log.info(`copying image from :${src} to ${dst}`)
                fs.copyFileSync(src, dst)
            } catch {
                this.log.debug(`failed to copy from :${src} to ${dst}`)
            }
        }
    }

    /**
     *
     * @param target
     * @param isGlobal
     */
    getOverrideData(target, isGlobal) {
        const base = (isGlobal ? this.localData.by_model : this.localData.by_id);
        if (base.hasOwnProperty(target)) {
            return base[target];
        }
        return {};
    }

    /**
     *
     * @param target
     * @param isGlobal
     * @param data
     */
    setOverrideData(target, isGlobal, data) {
        const base = (isGlobal ? this.localData.by_model : this.localData.by_id);
        if (typeof target != 'string') {
            this.error(`illegal target for override data: ${JSON.stringify(target)}`);
            return;
        }
        base[target] = data;
    };

    /**
     *
     */
    async getLegacyModels() {
        const legacyModels = [];
        for (const model in this.localData.by_model) {
            if (this.localData.by_model[model].hasOwnProperty('legacy') && this.localData.by_model[model].legacy) {
                legacyModels.push(model);
            }
        }
        return legacyModels;
    }

    /**
     *
     * @param key
     * @param isGlobal
     */
    getOverridesWithKey(key, isGlobal) {
        const base = (isGlobal ? this.localData.by_model : this.localData.by_id);
        const rv = [];
        for (const prop in base) {
            if (base[prop].hasOwnProperty(key)) {
                rv.push({key: prop, value: base[prop][key]});
            }
        }
        return rv;
    }

    /**
     *
     * @param target
     * @param key
     * @param isGlobal
     */
    getOverrideWithTargetAndKey(target, key, isGlobal) {
        let targetdata = this.getOverrideData(target, isGlobal);
        const keyparts = key.split('.');
        while (keyparts.length > 1) {
            targetdata = targetdata[keyparts[0]] || {};
            keyparts.splice(0, 1);
        }
        if (targetdata && targetdata.hasOwnProperty(keyparts[0])) {
            return targetdata[keyparts[0]];
        }
        return undefined;
    }

    /**
     *
     */
    async updateFromDeviceNames() {
        const fn = this.adapter.expandFileName('dev_names.json');
        this.info('Initializing localConfig from dev_names.json')
        fs.readFile(fn, (err, content) => {
            if (!err) {
                let data_js = {};
                try {
                    data_js = JSON.parse(content);
                } catch (error) {
                    this.error(`unable to parse data read from ${fn} : ${error.message ? error.message : 'undefined error'}`)
                    return;
                }
                try {
                    for (const prop in data_js) {
                        if (data_js[prop] != 'undefined') {
                            this.debug(`updating device name for ${prop} as ${data_js[prop]}`);
                            this.updateDeviceName(prop, data_js[prop]);
                        }
                    }
                } catch (error) {
                    this.error(`error in updateFromDeviceNames : ${error.message ? error.message : 'undefined error'}`)
                    return;

                }
                this.retainData();
            }
        });
    }

    /**
     *
     */
    async init() {
        this.info('init localConfig');
        const fn = this.adapter.expandFileName('LocalOverrides').replace('zigbee.', 'zigbee_').concat('.json');
        this.filename = fn;
        this.basefolder = path.dirname(fn);

        try {
            const content = fs.readFileSync(fn);
            try {
                const data_js = JSON.parse(content);
                this.localData = data_js;
            } catch (error) {
                this.error(`unable to parse data read from ${fn} : ${error.message ? error.message : 'undefined error'}`);
            }
        } catch (error) {
            await this.updateFromDeviceNames();
        }
        this.loadedversion = this.localData.version ?? 0;
        if (this.loadedversion != localConfig.version) {
            // fix the legacy change. Any 'legacy' option found on a model will be moved to 'options.legacy'
            //
            let changed = false;
            for (const mopt of Object.values(this.localData.by_model)) {
                if (mopt?.hasOwnProperty('legacy')) {
                    const opt = mopt?.options || {};
                    if (!opt.hasOwnProperty('legacy')) {
                        opt.legacy = mopt.legacy;
                        mopt.options = opt;
                    }
                    delete mopt.legacy;
                    changed = true;
                }
            }

            if (changed) {
                this.retainData(false);
            }

        }
    }

    /**
     *
     */
    writeData() {
        try {
            this.localData.version = localConfig.version;
            fs.writeFileSync(this.filename, JSON.stringify(this.localData, null, 2))
            this.loadedversion = localConfig.version;
            this.info('Saved local configuration data');
        } catch (error) {
            this.error(`error saving local config: ${error.message}`);
        }
        this.delayRetain = null;
    }

    /**
     *
     * @param isStopping
     */
    async retainData(isStopping) {
        if (isStopping) {
            if (this.delayRetain) {
                clearTimeout(this.delayRetain);
            }
            this.writeData(this);
            return;
        }
        if (this.delayRetain) {
            return;
        }
        const t = this;
        this.delayRetain = setTimeout(() => this.writeData(t), 500);
    }

    /**
     *
     * @param _path
     */
    enumerateImages(_path) {
        const rv = [];
        try {
            const files = fs.readdirSync(_path, {
                withFileTypes: true,
                recursive: true
            }).filter(item => (!item.isDirectory() && item.name.endsWith('.png')));
            files.forEach((item) => {
                const fn = path.join(item.parentPath, item.name);
                rv.push({
                    file: fn,
                    name: item.name,
                    data: fs.readFileSync(path.join(item.parentPath, item.name), 'base64'),
                    isBase64: true
                });
            });
        } catch (error) {
            this.error(`error in enumerateImages : ${error.message ? error.message : 'undefined error'}`)
        }
        return rv;
    }

    /**
     *
     * @param _dev_id
     * @param model_id
     */
    getOptions(_dev_id, model_id) {
        function extractOptions(target, options) {
            if (typeof target != 'object') {
                target = {};
            }
            if (typeof options != 'object') {
                return target;
            }
            Object.keys(options).forEach((option) => target[option] = options[option]);
            return target;
        }

        const dev_id = _dev_id.startsWith('0x') ? _dev_id.substring(2) : _dev_id

        const ld = this.localData.by_id[dev_id];
        const gd = this.localData.by_model[model_id];
        const rv = {};
        if (gd) {
            extractOptions(rv, gd.options);
        }
        if (ld) {
            extractOptions(rv, ld.options);
        }
        this.debug(`getOptions for ${dev_id} : ${JSON.stringify(rv)}`);
        return rv;
    }

    /**
     *
     * @param source
     * @param key
     * @param option
     * @param defaultvalue
     */
    getOption(source, key, option, defaultvalue) {
        const sourceEntries = source[key] || {};
        if (sourceEntries.hasOwnProperty(option)) {
            return sourceEntries[option];
        }
        if (typeof sourceEntries.options == 'object') {
            return sourceEntries.options.hasOwnProperty(option) ? sourceEntries.options[option] : defaultvalue;
        }
        return defaultvalue;
    }

    /**
     *
     * @param model_id
     * @param option
     * @param defaultvalue
     */
    getModelOption(model_id, option, defaultvalue) {
        if (this.loadedversion < localConfig.version) {
            const defaultOption = localConfig.DefaultModelOptions.models[model_id];
            if (defaultOption) {
                const modeloptions = this.localData.by_model[model_id]?.options ?? {};
                for (const opt of defaultOption) {
                    if (modeloptions.hasOwnProperty(opt.key)) {
                        continue;
                    }
                    modeloptions[opt.key] = opt.value;
                }
                const mopt = this.localData.by_model[model_id] ?? {};
                mopt.options = modeloptions;
                this.localData.by_model[model_id] = mopt;
            }
        }
        return this.getOption(this.localData.by_model, model_id, option, defaultvalue);
    }

    /**
     *
     * @param device_id
     * @param model_id
     * @param option
     * @param dOnly
     */
    getDeviceOption(device_id, model_id, option, dOnly) {
        const modelOption = dOnly ? this.getModelOption(model_id, option) : undefined;
        const dId = device_id.replace('0x', '');
        const deviceOption = this.getOption(this.localData.by_id, dId, option);
        if (modelOption && typeof modelOption == 'object' && deviceOption && typeof deviceOption == 'object') {
            for (const key of Object.keys(modelOption)) {
                if (modelOption[key] != undefined && deviceOption[key] == undefined) {
                    deviceOption[key] = modelOption[key];
                }
            }
            return deviceOption
        }
        return deviceOption || modelOption
    }

    /**
     *
     * @param id
     */
    getByModel(id) {
        return this.localData.by_model[id] || {};
    }

    /**
     *
     * @param id
     */
    getByDevice(id) {
        return this.localData.by_id[id] || {};
    }

}

module.exports = localConfig;
