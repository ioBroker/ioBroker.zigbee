...
class localConfig extends EventEmitter {
    ...
    */
    ...
    async updateLocalOverride(target, key, data, global) 
    {
        if (typeof target != 'string' || typeof key != 'string') {
            this.error(`update called with illegal id data:${JSON.stringify(id)}:${JSON.stringify(key)}:${JSON.stringify(data)}`)
            return false;
        }
        const base = global ? this.localData.by_model[id] || {} : this.localData.by_id[id] || {};
