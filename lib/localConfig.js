...
class localConfig extends EventEmitter {
    ...
    */
    ...
    async updateLocalOverride(_target, model, key, data, global)
    {
        ...
        //this.warn(`updateLocalOverride (1:${global ? 'global':'local'}) : ${target}:${key}:${data}`);
        ...
        if (global) {
            ...
            this.debug(`updateLocalOverride (2) : ${JSON.stringify(base)}:${JSON.stringify(this.localData)}`)
        //this.retainData();
        return true;
    }
    ...
    IconForId(id, model, defaultIcon) {
        ...
        try {
            this.debug('testing if ' + dst +  ' exists')
            if (!fs.existsSync(dst)) {
                const src = this.adapter.expandFileName(iconPath).replace('.','_');
                this.debug('trying to copy' + src +  ' to ' + dst)
