...
const { parseIeeeList } = require('./utils');
...
class localConfig extends EventEmitter {
    ...
    // adapter option possible values:
    // string - free text to enter
    // numeric - a number, possibli with value_min/value_max, value_step
    // binary - true/false
    // enum - selection, one of the possibilities
    // set - selection, any number of the possibilities

    static AdapterOptions = [{
        label:'resend States',
        name:'resend_states',
        type:'string',
        byDevice: true,
        ...
        label:'use first matching converter',
        name:'first_converter',
        type:'binary',
        byDevice: false,
        byModel: true,
        cVal: true,
        ...
        label:'store colormode',
        name:'store_colormode',
        type:'binary',
        byModel: true,
        cVal: true,
        cProp: undefined,
    ...
    static DefaultModelOptions = {
        models: {
            'S60ZBTPF': [{key:'meta_state', value:'state', version:1.0}],
            'WT-A03E': [{key:'meta_state', value:'sensor',version:1.0}],
            'CK-BL702-AL-01': [{key:'meta_message', value:'brightness', version:1.1}],
            'CK-BL702-AL-01(7008_Z102LG01-1)': [{key:'meta_message', value:'brightness', version:1.1}],
            'BSD-2': [{key:'meta_state', value:'manual_smoke_alarm,manual_burglar_alarm,broadcast_alarms', version:1.2}]
        },
        options: {
            'meta_state': ['S60ZBTPF', 'WT-A03E', 'BSD-2'],
            'meta_message': ['CK-BL702-AL-01', 'CK-BL702-AL-01(7008_Z102LG01-1)']
        }
    };
    ...
    // ---- join blocklist: IEEE addresses the adapter refuses when they try to join the network ----

    /** The blocklist as lower-case '0x...' strings (a copy). */
    getBlocklist() {
        return Array.isArray(this.localData.blocklist) ? [...this.localData.blocklist] : [];
    }

    /** Replace the blocklist. Returns the stored list and the entries that were no IEEE address. */
    setBlocklist(value) {
        const parsed = parseIeeeList(value);
        this.localData.blocklist = [...parsed.ieees].sort();
        this.retainData();
        return { blocklist: this.getBlocklist(), invalid: parsed.invalid };
    }

    isBlocked(ieeeAddr) {
        for (const ieee of parseIeeeList([ieeeAddr]).ieees) return this.getBlocklist().includes(ieee);
        return false;
    }
    ...
    async init() {
        ...
        this.loadedversion = this.localData.version ?? 0;
        if (this.loadedversion != localConfig.version) {
    ...
    writeData() {
        try {
            this.localData.version = localConfig.version;
            fs.writeFileSync(this.filename, JSON.stringify(this.localData, null, 2))
            this.loadedversion = localConfig.version;
    ...
    getModelOption(model_id, option, defaultvalue) {
        if (this.loadedversion < localConfig.version) {
            const defaultOption = localConfig.DefaultModelOptions.models[model_id];
            if (defaultOption) {
                const modeloptions = this.localData.by_model[model_id]?.options ?? {};
                for (const opt of defaultOption) {
                    if (modeloptions.hasOwnProperty(opt.key)) continue;
                    if (opt.version > this.loadedversion) modeloptions[opt.key] = opt.value;
                }
                const mopt = this.localData.by_model[model_id] ?? {};
                mopt.options = modeloptions;
                this.localData.by_model[model_id] = mopt;
            }
        }
    ...
    checkForOption(section, option) {
        ...
        for (const val of Object.values(target))
            if ((val?.options ?? {}).hasOwnProperty(option)) return true;
        return false;
    }


    optionExists(option) {
        if (this.checkForOption(this.localData.by_model, option)) return true;
        if (this.checkForOption(this.localData.by_id, option)) return true;
        return false;
    }
