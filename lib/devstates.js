...
const states = {
   ...
   smoke_detected2: {   // for Heiman
    ...
    hvacThermostat_occupancy: {
        ...
        read: true,
        type: 'boolean',
        getter: (payload) => (payload.occupancy % 1 == 1) ? true : false,
    ...
    hvacThermostat_pi_heating_demand: {
        ...
        unit: '%',
        getter: payload => {
            if (payload.pi_heating_demand) {
                return Math.round(payload.pi_heating_demand / 2.55);
            } else {
                return 0;
            }
        },
    ...
    SPZB0001_system_status: { //0x4000
        // 1 = manual valve control (use valve_position)
        // 2 = auto (default)
        id: 'spz_trv_mode',
        name: 'TRV Mode',
    ...
    SPZB0001_system_status_message: { //0x4002
        id: 'spz_error_status',
        ...
        type: 'string',
        getter: payload => {
            if (typeof payload.eurotronic_error_status == 'number') {
                var errnum = payload.eurotronic_error_status & 0x38;
                if (errnum === 0) return 'OK';
                var errstring = 'ERROR';
                if (errnum & 0x08 === 0x08) errstring = errstring + ' E1';
                if (errnum & 0x10 === 0x10) errstring = errstring + ' E2';
                if (errnum & 0x20 === 0x20) errstring = errstring + ' E3';
                return errstring;
            } else {
                return 'UNKNOWN '+JSON.stringify(payload);
            }
        },
    },
    SPZB0001_current_heating_setpoint: { //0x4002
    ...
    SPZB0001_system_mode: { //0x4008
        // Bit 2 = Boost
        // Bit 4 = Window open?
        // Bit 7 = Child protection
        id: 'spz_system_mode',
        name: 'Eurotronic System Mode',
...
const devices = [{
    ...
    },
    {
        ...
        models: [
            ...
            'DL 110 N', 'DL 110 W', 'SL 110 N', 'SL 110 M', 'SL 110 W', 'RS 125'
        ],
        icon: 'img/innr1.png',
        states: lightStates,
        linkedStates: [comb.brightnessAndState],
    ...
    {
        vendor: 'Heiman',
        models: ['SMOK_V16', 'b5db59bfd81e4f1f95dc57fdbba17931', 'SMOK_YDLV10', 'SmokeSensor-EM'],
        icon: 'img/hs1sa.png',
    ...
    // Eurotronic
    {
        ...
        states: [states.battery,
                ...
                states.hvacThermostat_unoccupied_heating_setpoint, states.hvacThermostat_occupied_heating_setpoint,
                states.hvacThermostat_system_mode, states.hvacThermostat_pi_heating_demand,
                states.SPZB0001_system_status, states.SPZB0001_valve_position, 
                states.SPZB0001_system_status_message, states.SPZB0001_current_heating_setpoint,
                states.SPZB0001_system_mode],
    },
    // Immax
    {
        vendor: 'Immax',
