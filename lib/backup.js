...
class Backup {
    ...
    backup(callback) {
        ...
        if (options.disableBackup) {
            ...
            if (fs.existsSync(pathLib.join(options.dbDir, options.backupPath))) {
files.push(options.backupPath);
}
            if (fs.existsSync(pathLib.join(options.dbDir, options.dbPath))) {
files.push(options.dbPath);
}
            if (fs.existsSync(pathLib.join(options.dbDir, options.localConfigPath))) {
files.push(options.localConfigPath);
}
            if (files.length == 0) {
return;
}
