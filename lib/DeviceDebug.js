...
class DeviceDebug extends EventEmitter {
    ...
    onDebugEvent(message) {
        if (typeof message === 'object' && message.hasOwnProperty('ID')) {
            ...
            if (message.hasOwnProperty('data')) {
                ...
                const item = this.dataByID[dataId] ? this.dataByID[dataId] : {
                    dataID: dataId,
                    deviceID: 'unknown',
                    states: [],
                    flags: [],
                ...
                if (data.error && item.errors.indexOf(data.error) < 0) {
                    item.errors.push(data.error);
                }
                if (data.states) {
                    item.states.push(...data.states);
                }
                if (data.flag && item.flags.indexOf(data.flag) < 0) {
                    item.flags.push(data.flag);
                }
                if (data.payload && !item.payload) {
                ...
                if (data.error) {
                    ...
                    if (data.flag === 'SUCCESS') {
                        ...
                        flag = item.IO ? `I${data.flag}` : `O${data.flag}`;
                    }
                } else {
                    flag = item.IO ? 'IN' : 'OUT';
                }
                if (message.message) {
                    let msgIdx = 0;
                    while (item.messages.hasOwnProperty(`${flag}${msgIdx ? `-${msgIdx}` : ''}`)) {
                        msgIdx++;
                    }
                ...
                if (data.ID && data.ID !== item.deviceID) {
                    ...
                    const DevData = this.dataByDevice[item.deviceID] ? this.dataByDevice[item.deviceID] : {
                        IN: [],
                        OUT: []
                    };
    ...
    deleteMessagesFor(item) {
        ...
        for (const id in this.dataByID) {
            if (this.dataByID[id].deviceID === item) {
                obsoleteIDs.push(id)
            }
        }
