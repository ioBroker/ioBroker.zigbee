                ...
                .then(async (result) => {
                    ...
                    for (const devInfo of result) {
                        if (devInfo._id.indexOf('group') > 0) {
                            ...
                            this.warn('matching ' + JSON.stringify(devInfo._id) + " to " + JSON.stringify(match));
                            if (match && match.length > 1) {
                                const groupmembers = await this.zbController.getGroupMembersFromController(match[1])
                                this.warn('group members: ' + JSON.stringify(groupmembers));
                                if (groupmembers && groupmembers.length > 0) {
                                    ...
                                    this.warn('memberinfo: ' + JSON.stringify(devInfo.memberinfo));
                                }
                            }
                        }
                        else
