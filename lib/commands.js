        ...
        if (typeof obj === 'object' && obj.command) {
            switch (obj.command) {
                ...
                case 'getCoordinatorInfo':
                    if (obj && obj.message && typeof obj.message === 'object') {
                        this.getCoordinatorInfo(obj.from, obj.command, obj.message, obj.callback);
                    }
                    break;
                }
        }
        ...
        if (this.zbController) {
            ...
            await this.adapter.getForeignObject("system.adapter." + this.adapter.namespace, (err, obj) => {
                ...
                if (coordinatorVersion && coordinatorVersion.type && coordinatorVersion.meta) {
                    ...
                    if (meta && meta.revision && meta.product && meta.majorrel&& meta.minorrel && meta.maintrel) {
                        coordinatorinfo.revision = meta.revision;
                        coordinatorinfo.version = meta.product +'.' + meta.majorrel + '.' + meta.minorrel + '.' + meta.maintrel; 
                    }
                }
