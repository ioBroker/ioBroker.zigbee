...
const disallowedDashStates = ['link_quality', 'available', 'battery', 'groups', 'device_query',
    ...
    async getDevices(from, command, id, callback) {
        if (this.zbController) {
            ...
            this.adapter.getEnumsAsync('enum.rooms')
                ...
                .then((result) => {
                    // not groups
//                    return result.filter((devInfo) => {
//                        return devInfo.common.type !== 'group' && (!id || (id && devInfo._id == id));
//                    });
                   return result;
                })
                .then(async (result) => {
                    ...
                    result.forEach((devInfo) => {
                        if (devInfo._id) {
                            const isGroup = (devInfo._id.indexOf('group') > 0);
                            // groups
                            ...
                            if (isGroup)  {
                                this.warn('states for : ' + JSON.stringify(devInfo._id) + ' ' + JSON.stringify(states));
                            }
                            ...
                            if (isGroup)  {
                                this.warn('states for : ' + JSON.stringify(devInfo._id) + ' ' + JSON.stringify(states));
                                this.warn('Device for '+ JSON.stringify(devInfo._id) + ' ' + JSON.stringify(devInfo));
                            }
                ...
                .then(async (result) => {
                    ...
                    for (const devInfo of result) {
                        const isGroup = (devInfo._id.indexOf('group') > 0);
                        if (isGroup) {
                            this.warn('common for '+ JSON.stringify(devInfo._id) + ' ' + JSON.stringify(devInfo.common));
