    ...
    async delNvBackup(from, command, msg, callback) {
        try {
            if (this.zbController)
            {
                ...
                require('fs').unlink(name, async (err) => {
                    const rv={};
                    if (err) {
        ...
        if (this.zbController && this.zbController.herdsmanStarted) {
            ...
            this.adapter.getEnumsAsync('enum.rooms')
                ...
                .then(devices => {
                    ...
                    this.adapter.sendTo(from, command, { devices:devices, inLog:this.adapter.deviceDebug.logStatus, byId:this.adapter.deviceDebug.collectDebugData(), clean:this.stController.CleanupRequired(), errors:this.stController.getStashedErrors() }, callback);
                })
                .catch(err => {
