            ...
            await this.adapter.getForeignObject("system.adapter." + this.adapter.namespace, (err, obj) => {
                ...
                try
                {
                    ...
                    if (coordinatorVersion && coordinatorVersion.type && coordinatorVersion.meta) {
                        ...
                        if (meta) {
                            if (revision) coordinator.revision = meta.revision;
                            let vt = (meta.hasOwnProperty('transportrev') ? meta.transportrev.toString() + '-': 'x-');
                            let vt = vt.concat(meta.hasOwnProperty('product') ? meta.product.toString() + '.': 'x.');
                            let vt = vt.concat(meta.hasOwnProperty('majorrel') ? meta.majorrel.toString() + '.': 'x.');
                            let vt = vt.concat(meta.hasOwnProperty('minorrel') ? meta.minorrel.toString() + '.': 'x.');
                            let vt = vt.concat(meta.hasOwnProperty('maintrel') ? meta.maintrel.toString() + '.': 'x.');
                            coordiatorinfo.version = vt;
                        }
                    }
                }
