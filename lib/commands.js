...
class Commands {
                    ...
                    case 'getDeviceCleanupRequired':
                        if (this.stController) {
                            this.adapter.sendTo(obj.from, obj.command, {
                                clean: this.stController.CleanupRequired(),
                                errors: this.stController.getStashedErrors()
                            }, obj.callback);
                        }
    ...
    async fillInfo(device, entity, device_stateDefs, all_states, models) {
        device.statesDef = (device_stateDefs || []).filter(stateDef => {
            ...
            if (stateDef.common.color || names.length > 2) {
                return false;
            }
    ...
    async getCoordinatorInfo(from, command, callback) {
        ...
        const coordinatorCandidates = this.zbController && this.zbController.herdsmanStarted ? await this.adapter.zbController.herdsman?.getDevicesByType('Coordinator') ?? [] : [];
        if (coordinatorCandidates.length == 0) {
            coordinatorinfo.ieee = '0x0000000000000000';
        } else {
            coordinatorinfo.ieee = coordinatorCandidates[0]?.ieeeAddr ?? '0xffffffffffffffff';
        }
    ...
    async deleteZigbeeDevice(from, command, msg, callback) {
        if (this.zbController && this.zbController.herdsmanStarted && this.stController) {
            ...
            if (!dev) {
                ...
                if (err != '') {
                    this.adapter.sendTo(from, command, {errror: err}, callback);
                } else {
                    this.adapter.sendTo(from, command, {}, callback);
                }
                return;
            ...
            */
            this.zbController.remove(sysid, force, async (err) => {
                if (!err) {
                    ...
                    this.adapter.sendTo(from, command, {}, callback);
                    if (msg.dev) {
                        this.adapter.stController.localConfig.removeLocalData(devId, msg.model);
                    }
                } else {
                    this.adapter.sendTo(from, command, {error: err}, callback);
                }
    ...
    async cleanDeviceStates(from, command, force, callback) {
        ...
        for (const device of devicesFromDB) {
            ...
            if (entity) {
                ...
                this.stController.deleteOrphanedDeviceStates(device.ieeeAddr, model, force, (msg) => {
                    messages.push(msg)
                });
            }
        }
        try {
            for (const group of groupsFromDb) {
                this.stController.deleteOrphanedDeviceStates(group.id, 'group', force, (msg) => {
                    messages.push(msg)
                });
            }
        } catch (error) {
    ...
    async updateDeviceImage(from, command, msg, callback) {
        if (this.stController) {
            ...
            if (!msg.global) {
                ...
                if (entity) {
                    this.stController.updateDev(target, entity.mapped.model, entity.mapped.model, () => {
                        this.adapter.sendTo(from, command, {imageData: result}, callback)
                    });
                } else {
                    this.stController.updateDev(target, undefined, 'group', () => {
                        this.adapter.sendTo(from, command, {imageData: result}, callback)
                    });
                }
            } else {
    ...
    async getLocalConfigItems(from, command, msg, callback) {
        ...
        if (this.stController) {
            ...
            rv.error = 'stController not initialized - no Data sent'
        }
