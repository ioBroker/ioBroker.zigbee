...
class Commands {
    ...
    async fillInfo(device, states) {
        device.statesDef = (states || []).filter(stateDef => {
            ...
            return {
                ...
                val: states[stateDef._id] ? states[stateDef._id].val : undefined,
    ...
    async getDevices(from, command, id, callback) {
        ...
        for (const devInfo of deviceObjects) {
            if (devInfo._id.indexOf('group') > -1) {
                ...
                const lqS = alls[`${devInfo._id}.link_quality`];
                devInfo.link_quality = lqS ? lqS.val : -1;
                devInfo.link_quality_lc = lqS ? lqS.lc : undefined;
            ...
            PromiseChain.push(this.fillInfo(devInfo, allst.filter(item => item._id.startsWith(devInfo._id))))
        }
        if (!id) {
    ...
    async getDevices_o(from, command, id, callback) {
        if (this.zbController && this.zbController.herdsmanStarted) {
            ...
            this.adapter.getEnumsAsync('enum.rooms')
                ...
                .then(devices => {
                    ...
                    const rv = { devices:devices, inLog:this.adapter.deviceDebug.logStatus, }
                    if (!id) rv.byId = this.adapter.deviceDebug.collectDebugData();
                    if (this.stController) {
                        rv.clean = this.stController.CleanupRequired();
                        rv.errors = this.stController.getStashedErrors();
