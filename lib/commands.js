...
const { getZbId, reverseByteString, parseIeeeList } = require('./utils');
...
class Commands {
            ...
            if (obj) {
                switch (obj.command) {
                    ...
                    case 'clearErrors':
                        ...
                        break;
                    case 'getJoinBlocklist':
                    case 'updateJoinBlocklist':
                        this.joinBlocklist(obj.from, obj.command, obj.message || {}, obj.callback);
                        break;
                    default:
    ...
    /**
     * getJoinBlocklist: the blocklist and the devices seen since the start. updateJoinBlocklist: add or remove
     * one IEEE address ({ add: '0x...' } / { remove: '0x...' }), then the same reply; an entry that is no IEEE
     * address leaves the list untouched and is reported in 'error'.
     */
    async joinBlocklist(from, command, msg, callback) {
        const rv = {};
        // the list lives in the adapter's localConfig - it can be edited while the zigbee subsystem is down
        const localConfig = this.adapter.localConfig || (this.stController ? this.stController.localConfig : undefined);
        if (!localConfig) {
            rv.error = 'localConfig not initialized - no data sent';
        } else {
            if (command === 'updateJoinBlocklist') {
                const list = new Set(localConfig.getBlocklist());
                if (msg.remove !== undefined) {
                    for (const ieee of parseIeeeList([msg.remove]).ieees) list.delete(ieee);
                }
                if (msg.add !== undefined) {
                    const parsed = parseIeeeList([msg.add]);
                    if (parsed.invalid.length > 0) rv.error = `'${msg.add}' is not an IEEE address`;
                    for (const ieee of parsed.ieees) list.add(ieee);
                }
                if (!rv.error) localConfig.setBlocklist([...list]);
            }
            rv.blocklist = localConfig.getBlocklist();
            const zbController = this.zbController || this.adapter.zbController;
            rv.seen = zbController && typeof zbController.getSeenDevices === 'function' ? zbController.getSeenDevices() : [];
        }
