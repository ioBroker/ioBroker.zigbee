            ...
            if (typeof obj === 'object' && obj.command) {
                switch (obj.command) {
                    ...
                    case 'updateDeviceData':
                        ...
                        break;
                    case 'getUploadRequired':
                        if (obj) {
                            let UploadNeeded = false;
                            try
                            {
                                const src = `${this.adapter.adapterDir}/admin/img`
                                const dst = utils.getAbsoluteInstanceDataDir(this.adapter).replace('.','_').concat('/../files/zigbee.admin/img');
                                const filesToUpload = fs.readdirSync(src, {withFileTypes: true, recursive: true}).filter(item => (!item.isDirectory() && (item.name.endsWith('.png') || item.name.endsWith('.jpg') || item.name.endsWith('.jpeg')))).map(item => item.name);
                                const uploadedFiles = fs.readdirSync(dst, {withFileTypes: true, recursive: true}).filter(item => (!item.isDirectory() && (item.name.endsWith('.png') || item.name.endsWith('.jpg') || item.name.endsWith('.jpeg')))).map(item => item.name);
                                /* for (const candidate of filesToUpload) {
                                    const idx = uploadedFiles.indexOf(candidate);
                                    if (idx == -1) {
                                        this.warn('Upload needed, file ' + JSON.stringify(candidate) + ' is missing from uploaded files')
                                        UploadNeeded = true;
                                        break;
                                    }
                                }
                                */
                                UploadNeeded = filesToUpload.length != uploadedFiles.length;
                                /* if (UploadNeeded) {
                                ...
                                this.error('getUploadRequired raised an error: ' + (error && error.message ? error.message : 'no message'));
                            }
                            this.adapter.sendTo(obj.from, obj.command, {upload: UploadNeeded, clean:this.stController.CleanupRequired()}, obj.callback);
                        }
                }
            }
        }
