...
class Commands {
    ...
    buildDeviceInfo(device) {
        ...
        try {
            ...
            if (device.memberinfo) {
                rv.device.members= device.memberinfo.length;
                for (let m = 0; m < device.memberinfo.length; m++) {
                    rv.device[`member ${m}`] = `${device.memberinfo[m].device}.${device.memberinfo[m].epid} (${dev.memberinfo[m].ieee}/${device.memberinfo[m].epid}`;
                }
            }
        }
        catch (error) {
