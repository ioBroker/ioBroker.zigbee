    ...
    onMessage(obj) {
        if (typeof obj === 'object' && obj.command) {
            switch (obj.command) {
                ...
                case 'setState':
                    if (obj && obj.message && typeof obj.message === 'object') {
                        this.adapter.setState(obj.message.id, obj.message.val, obj.callback);
                    }
                    break;
            }
        }
    ...
    async getDevices(from, command, callback) {
        if (this.zbController) {
            ...
            this.adapter.getEnumsAsync('enum.rooms')
                ...
                .then(async (result) => {
                    const alls = await this.adapter.getStatesAsync('*');
                    const allst = await this.adapter.getStatesOfAsync();
