    ...
    brightness: {
        ...
        setterOpt: (value, options) => {
            ...
            if (hasTransitionTime) {
                so.transition = options.transition_time;
            }
...
async function addExposeToDevices(device, model, logger) {
    ...
    if (hM) {
        ...
        if (typeof hM.exposes == 'function') {
            ...
            adapterModelDefinition.exposes = (typeof hM.exposes == 'object') ? hM.exposes : {};
        }
        ...
        const {
            ...
            message,
            error
        } = await applyHerdsmanModel(adapterModelDefinition, {newCompositeMethod: true}, logger);
        if (error) {
            if (logger) {
            ...
            // currently: not.
            //
            if (adapterModelDefinition.UUID) {
                newModel.UUID = adapterModelDefinition.UUID;
            }
            return newModel;
        }
    }
...
function getStateDefinition(name, type, prop) {
    if (typeof name != 'string') {
        return getStateDefinition('ilstate', type, prop);
    }
    if (states.hasOwnProperty(name)) {
        return states[name];
    }
    if (legacyStates.hasOwnProperty(name)) {
        return legacyStates[name];
    }
    return {
        id: name,
        prop: prop || name,
