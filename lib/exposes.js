...
async function applyExposeForDevice(mappedDevices, byModel, device, logger) {
    ...
    if (!deviceDef)  {
        logger.warn(`ZHC02,${Date.now()},${device && device.ieeeAddr ? device.ieeeAddr : '0x0'}, failed`);
        return false;
    }
    logger.warn(`ZHC02,${Date.now()},${device && device.ieeeAddr ? device.ieeeAddr : '0x0'}, passed`);
