...
const colorconvert = require('zigbee-herdsman-converters/lib/color');
...
function genState(expose, overrides) {
    ...
    const is_inComplex = typeof(name) == 'string' ? name.includes('.') : false;
    ...
    switch (expose.type) {
        case 'binary':
            state = {
                ...
                name: stateName,
                icon: undefined,
                role: role || (readable || is_inComplex ? 'state' : 'button'),
                write: writable,
                read: readable,
                type: `boolean`,
            };
    ...
    state.category = state.category ?? expose.category;
...
function generateCompositeStates(expose, _channelID, options)
{
    ...
    if (options.newCompositeMethod) {
        ...
        stateData.states.push({
            ...
            category : expose.category ?? 'compmaster',
            getter: (value) => {
...
function createFromExposes(model, def, device, options, log) {
    ...
    function pushToStates(state, access) {
        ...
        let modifiers = {};
        if (options?.modify_state) {
            try {
                modifiers = JSON.parse(options.modify_state)[state.id] ?? {};
            }
            catch (e)
            {
                __logger.warn(`Unable to parse ${options.modify_state} - ${e?.message ?? 'no error given'}`);
            }

        }
        if (modifiers) for (const key of Object.keys(modifiers)) {
            if (modifiers[key]== ``) delete state[key];
            else state[key] = modifiers[key];
        }
        const stateExists = states.findIndex((element, index, array) => element.id === state.id);
        if (stateExists < 0) {
            if (state.prop === state.id) delete state.prop;
            ...
            switch (expose.type) {
                    ...
                    if (hasColorXY || hasColorHS) {
                        ...
                        if (hasColorXY) pushToStates({
                            ...
                            setterOpt: (value, options) => {
                                ...
                                const so = {...options, transition: transitionTime};
                                if (hasTransitionTime) delete so['transition_time'];
                                return so;
                            },
                            getter: payload => {
                                // Requires testing!
                            ...
                            epname: expose.endpoint,
                            setattr: 'color',
                            category: expose.category ?? 'color',
                ...
                case 'numeric':
                    if (expose.endpoint) {
                        ...
                        const numericRolePreset = (expose.access & ea.SET) ? 'level' : 'value';
                        ...
                        switch (expose.name) {
                            ...
                            case 'power':
                                ...
                                break;
                            case 'valve_position':
                                state = genState(expose, {role:`${numericRolePreset}.valve`});
                                //state = getStateDefinition('load_power');
                                break;
                            ...
                            default: {
                                switch (expose.unit) {
                                    ...
                                    case '°F':
                                    case 'K':
                                        state = genState(expose, { role: `${numericRolePreset}.temperature`});
                                        break;
                                    case '%':
                                    case 'Wh':
                                    case 'kWh':
                                        state = genState(expose, { role: `${numericRolePreset}.energy`});
                                        break;
                                    case 'W':
                                    case 'kW':
                                        state = genState(expose, { role: `${numericRolePreset}.power`});
                                        break;
                                    case 'V':
                                    case 'mV':
                                        state = genState(expose, { role: `${numericRolePreset}.voltage`});
                                        break;
                                    case 'A':
                                    case 'mA':
                                        state = genState(expose, { role: `${numericRolePreset}.current`});
                                        break;
                                    case 'Hz':
                                        state = genState(expose, { role: `${numericRolePreset}.frequency`});
                                        break;
                                    default:
                                        state = genState(expose, { role: numericRolePreset });
                                        break;
                                }
                                break;
                            }
                ...
                case 'cover':
                    if (expose?.features?.[Symbol.iterator]) for (const prop of expose.features) {
                        if (prop.type === 'composite') {
                            //if (__logger?.warn) __logger.warn(`pushing composite state generation for feature ${prop.property} for device ${device.ieeeAddr} (${model})`);
                            const cStates = generateCompositeStates(prop, '', options);
                            ...
                            //if (__logger?.warn) __logger.warn(`pushing state generation for feature ${prop.property} for device ${device.ieeeAddr} (${model})`);
                            switch (prop.name) {
                                case 'state':
                                    pushToStates(genState(prop, {role:`switch${expose.type == 'lock' ? '.lock' : ''}`}), prop.access);
                                    break;
                                default:
                                    pushToStates(genState(prop), prop.access);
                                    break;
                    ...
                    if (expose?.features?.[Symbol.iterator]) for (const prop of expose.features) {
                        switch (prop.name) {
                            case 'pi_heating_demand':
                                pushToStates(genState(prop, { role: 'level.valve'}), prop.access);
                                break;
                            default:
                                if (prop.type === 'composite') {
                                    //if (__logger?.warn) __logger.warn(`pushing composite state generation for feature ${prop.property} for device ${device.ieeeAddr} (${model})`);
                                    ...
                                    //if (__logger?.warn) __logger.warn(`pushing state generation for feature ${prop.property} for device ${device.ieeeAddr} (${model})`);
                                    if (['°C','°F','K'].includes(prop.unit)) {
                                        pushToStates(genState(prop, { role: `${(prop.access & ea.SET) ? 'level' : 'value'}.temperature` }) , prop.access);
                                    }
                                    else
                                        pushToStates(genState(prop), prop.access);
                                    //pushToStates(prop.includes('setpoint') ? genState(prop, { role: 'level.temperature' }) : genState(prop), prop.access);
                                }
                ...
                case 'composite':
                {
                    //if (__logger?.warn) __logger.warn(`pushing composite state generation for feature ${expose.property} for device ${device.ieeeAddr} (${model})`);
                ...
                default:
                    ...
                    if (__logger?.warn) __logger.warn(`Unhandled expose type ${expose.type} for device ${device.ieeeAddr} of type ${model}`);
...
async function applyHerdsmanModel(modelDesc, options, logger) {
    __logger = logger;
    try {
