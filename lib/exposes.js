...
function applyExposes(mappedDevices, byModel, logger) {
...
async function applyExposeForDevice(mappedDevices, byModel, device, logger, model) {
    const deviceDef = await zigbeeHerdsmanConverters.findByDevice(device);
    if (!deviceDef) return false;
    applyDeviceDef(mappedDevices, byModel, deviceDef, device, model == '6735/6736/6737');
    return true;
}

function applyDeviceDef(mappedDevices, byModel, deviceDef, device, log) {
    ...
    if (deviceDef.hasOwnProperty('exposes') && (!existsMap || !existsMap.hasOwnProperty('states'))) {
        try {
            const newDevice = createFromExposes(stripModel, deviceDef, device, log);
