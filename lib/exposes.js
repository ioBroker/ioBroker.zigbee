...
function generateCompositeStates(expose, _channelID, options) {
    ...
    if (expose?.features?.[Symbol.iterator]) {
        for (const prop of expose.features) {
            if (prop.type === 'composite' || prop.type === 'list') {
                ...
                stateData.states.push(genState(prop, options.newCompositeMethod ? {
                    fromComposite: true,
                    name: `${channelID}${prop.property}`,
                    channelID: stateId
                } : {fromComposite: true}))
                stateData.keys.push(prop.property)
            }
    ...
    if (options.newCompositeMethod) {
        ...
        stateData.states.push({
            ...
            getter: (value) => {
                ...
                if (myval != undefined && typeof myval === 'object') {
                    return JSON.stringify(myval);
                }
                return myval;
            },
    ...
    function genStateFromExpose(expose, deviceDefinition) {
        ...
        switch (expose.type) {
                    ...
                    if (hasColorXY) {
                        pushToStates({
                            ...
                            getter: payload => {
                                ...
                                if (p.length < 7 && m && m[0].length == p.length) {
                ...
                switch (expose.name) {
                    ...
                    case 'action': {
                        ...
                        for (const actionName of expose.values) {
                            ...
                            if (release === 'IGNORE') {
                                continue
                            } // a release message for which a press or hold exists.
            ...
            default:
                ...
                if (__logger?.warn) {
                    __logger.warn(`Unhandled expose type ${expose.type} for device ${device.ieeeAddr} of type ${model}`)
                }
