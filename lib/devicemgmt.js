...
class dmZigbee extends dmUtils.DeviceManagement {
    ...
    async handleNewDevice(context) {
        ...
        await delay(500);
        const maxcnt = permitTime > 99 ? 99 : permitTime;
        for (let i = 1; i <= maxcnt; i += 1) {
            await delay(permitTime > 99 ? permitTime * 10 : 1000);
            const val = permitTime > 99 ? i : i / maxcnt * 100;
            await progress.update({ value: i, label: `${Math.floor(val)}%` });
        }
    ...
    async handleDeleteDevice(id, context) {
        ...
        const deviceInfo = await context.adapter.zbController.getDevice('0x'+  devId);
    ...
    async handleQueryDevice(id, context) {
        await context.adapter.zbController.deviceQuery(id, -1, false)
        return {refresh: true};
    }
    ...
    async createDeviceInfo(device) {
        ...
        const HIDDEN_CATEGORIES = new Set(['config','diagnostic','dimming']);
        const FORBIDDEN_CATEGORIES = new Set(['compmaster', 'noshow']);
        const status = {};
        ...
        const res = {
            ...
            actions: [
                {
                    id: 'query',
                    handler: this.handleQueryDevice,
                },
                /* {
                    id: 'rename',
                    icon: 'forward',
                ...
                {
                    id: 'delete',
                    handler: this.handleDeleteDevice,
                },
            ],
        };

        if (Array.isArray(device.statesDef)) {
            const tileItems   = {};   // kurze Labels → Kachel
            ...
            for (const state of device.statesDef) {
                ...
                if (FORBIDDEN_CATEGORIES.has(state.category ?? '')) { continue; }
                if (state.category === 'singleaction' && !this.adapter.config.devmgrsingleaction) continue;
                ...
                if (state.write) {
                    let target = configItems;
                    if (WRITE_FRONT_PAGE_ROLE.includes((`${state?.role ?? ''}. `).split('.')[0])) target = tileItems;
                    if (HIDDEN_CATEGORIES.has(state.cateory??'') || label.length > MAX_CARD_LABEL_LEN) target = configItems;
                    ...
                    if (isSchedule) {
                        ...
                        target[state.id] = {
                            ...
                            trueTextStyle: { color: 'green'}, falseTextStyle: { color: 'red' },
                            newLine: true,
                        };
                    ...
                    if (detailItems[state.id] || tileItems[state.id]) { continue; }
                    ...
                    if (label.length > MAX_CARD_LABEL_LEN) {
                        ...
                        if (HIDDEN_CATEGORIES.has(state.cateory??''))
                            configItems[state.id] = item;
                        else
                            tileItems[state.id] = item;
                    }
                }
            }
            ...
            if (Object.keys(tileItems).length > 0) {
                const sortedItems = Object.keys(tileItems)
                    .sort((a, b) => a.localeCompare(b))
                    .reduce((acc, key) => { acc[key] = tileItems[key]; return acc; }, {});
            ...
            if (Object.keys(configItems).length > 0) {
                res.actions.push({
                    ...
                    handler: async (_id, ctx) => {
                        await ctx.showForm(
                            { type: 'panel', label: displayName, items: configItems },
    ...
    async getDeviceDetails(id, _action, _context) {
        ...
        if (device.common.type == 'group') {
            infoItems['_h1']    = { type: 'header',    size:16, text: 'Group Members', newLine: true};
            ...
            infoItems['kind']   = { size:16, type: 'staticInfo',  label: 'Device Type',    data: device.info.mapped?.type   ?? '—',             addColon: true, newLine: true };
        } else {
            ...
            const channel = `${this.adapter.config.channel} (${CHANNELS.includes(Number(this.adapter.config.channel)) ? 'Zigbee Light Link' : 'Zigbee'})`;
            ...
            infoItems['ieee']     = { size:16, type: 'staticInfo',  label: 'IEEE Address',   data: ieee,                                     addColon: true, newLine: true };
            infoItems['channel']  = { size:16, type: 'staticInfo',  label: 'Channel',        data:channel, textColor:'red', addColon: true, newLine: true };
            infoItems['panid']    = { size:16, type: 'staticInfo',  label: 'Pan / ExtPan',   data:panid,  addColon: true, newLine: true };
            infoItems['enc']      = { size:16, type: 'staticInfo',  label: 'Encryption key', data:enc,  addColon: true, newLine: true };
        ...
        // ── Tab 2: Verbindung ─────────────────────────────────────────────────
        const connItems = {};
        connItems['_h1']    = { type: 'header',      size:16,  text:  'Zigbee Connection',   newLine: true };
        ...
        if (hasDetails) {
            ...
            tabs._tab_values = {
                ...
                label: 'Values',
                items: {
                    _h1: { size:16, type: 'header', text: 'Additional Values', newLine: true },
        ...
        if (hasSchedule) {
            tabs._tab_schedule = {
                ...
                label: 'Schedule',
                items: {
                    _h1: { size:16, type: 'header',  text: 'Weekly Schedule', newLine: true },
                    _d1: { type: 'divider', color: 'primary' },
                    ...scheduleItems,
                },
            };
