...
const CC_DEFS = {
    COLORS_ALL: [['hsb'], ['hsl'], ['hsv'],
        ['h', 's', 'b'], ['h', 's', 'v'], ['h', 's', 'l'],
        ['hue', 'saturation'], ['h', 's'],
        ['hex'], ['r', 'g', 'b'], ['rgb'],
        ['x', 'y']],
    COLORS_HS3: [['hsb'], ['hsl'], ['hsv'],
        ['h', 's', 'b'], ['h', 's', 'v'], ['h', 's', 'l']],
    COLORS_HS2: [['hue', 'saturation'], ['h', 's']],
    COLORS_HS: [['hsb'], ['hsl'], ['hsv'],
        ['h', 's', 'b'], ['h', 's', 'v'], ['h', 's', 'l'],
        ['hue', 'saturation'], ['h', 's']],
    COLORS_RGB: [['hex'], ['r', 'g', 'b'], ['rgb']],
    COLORS_XY: [['x', 'y']]
};
 ...
 * @param value
 * @param returnObject
 * @param colorcapabilities
 */
function complexColor(value, returnObject, colorcapabilities) {
    ...
    try {
        const cc = colorcapabilities ? colorcapabilities : CC_DEFS.COLORS_ALL;
        const colorJSON = typeof value === 'string' ? JSON.parse(value.replaceAll("'", '"')) : value;
        const matchedRV = returnObject ? colorJSON : JSON.stringify(colorJSON);
        const numProp = Object.keys(colorJSON).length;
        for (const capability of cc) {
            if (hasMultipleProperties(colorJSON, capability, numProp)) {
                return matchedRV;
            }
        }
    } catch (error) {
...
exports.CC_DEFS = CC_DEFS;
exports.complexColor = complexColor;
