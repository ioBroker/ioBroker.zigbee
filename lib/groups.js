...
class Groups {
    ...
    setAvgVal(target, values) {
        ...
        if (cnt > 0) {
            if (hasBooleanSrc) {
                this.adapter.setState(target, sum / cnt > 0.49999999, true);
            } else {
                this.adapter.setState(target, sum / cnt, true)
            }
        }
    }
    ...
    async onGroupStatePublished(deviceId, model, stateModel, stateList, options, debugId) {
        if (Groups.extractGroupID(deviceId) >= 0) { // the states are group states
            for (const state of stateList) {
                ...
                if (state.stateDesc.id === 'stateupdate') {
                    ...
                    for (const item in this.GroupData) {
                        if (this.GroupData[item] && this.GroupData[item].hasOwnProperty('stateupdate')) {
                            if (this.GroupData[item].stateupdate && this.GroupData[item].stateupdate == 'off') {
                                continue;
                            }
                ...
                if (state.stateDesc.isCommonState) {
                    continue;
                }
                if (this.GroupData[GroupDataKey].memberupdate) {
                    this.readGroupMemberStatus(this, {id: deviceId, state: state.stateDesc.id});
                }
            }
        }
    }
    ...
    async onDeviceStateChanged(deviceId, model, stateModel, stateList, options, debugId) {
        if (Groups.extractGroupID(deviceId) < 0) { // the states are device states
            for (const state of stateList) {
                ...
                const id = deviceId;
                if (sd.isCommonState || !this.anyGroupStateUpdate) {
                    continue
                } // common states are never valid for groups
                const sid = `${this.adapter.namespace}.${deviceId.split('x')[1]}.${state.stateDesc.id}`;
                if (this.GroupData.states[sid]) {
                    this.GroupData.states[sid].val = state.value;
                }
                ...
                if (typeof affectedStates == 'object' && affectedStates.targets.length > 0) {
                    ...
                    for (const target of targetsByGroup) {
                        ...
                        if (typeof gData == 'object' && gData.hasOwnProperty('stateupdate') && typeof this.GroupData.groupStates == 'object') {
                            ...
                            const values = [];
                            if (typeof sources != 'object' || method === 'off') {
                                continue;
                            }
                            ...
                            switch (method) {
                                ...
                                case 'avg':
                                    ...
                                    break;
                                case 'mat':
                                    if (values.filter(c => c == values[0]).length == values.length) {
                                        this.adapter.setState(target, values[0], true);
                                    }
                                    break;
                            }
        ...
        if (toRead.cluster) {
            if (device && endpoint) {
                if (entity.endpoint.inputClusters.includes(toRead.cluster)) {
                    ...
                    this.debug(`omitting cluster ${toRead.cluster} - not supported`);
                }
            } else {
    ...
    buildGroupID(id, withInstance) {
        const parts = [];
        if (withInstance) {
            parts.push(this.adapter.namespace);
        }
        if (Number(id) > 0) {
            parts.push(Groups.generateGroupID(id));
        } else {
            if (Groups.extractGroupID(id) > 0) {
                parts.push(id);
            }
        }
        return parts.join('.');
    ...
    async getGroupMembersFromController(id) {
        ...
        try {
            ...
            if (error) {
                ...
                this.error('unidentifed error in getGroupMembersFromController');
            }
        }
        return members;
    }
    ...
    addMissingState(arr, states) {
        ...
        for (const state of [...states]) {
            ...
            if (arr.find((candidate) => candidate == state.id) === undefined) {
                arr.push(state.id)
            }
        }
        return arr;
    ...
    /**
     *
     * @param numericGroupID
     * @param allowedDevices
     */
    async removeGroupMemberStateList(numericGroupID, allowedDevices) {
        ...
        const t = this;
        if (!devices && this.GroupData.hasOwnProperty(groupID)) {
            delete this.GroupData[groupID];
        }
        ...
        for (const key of keys) {
            ...
            if (devices && devices.includes(getZbId(key))) {
                continue;
            }
            if (trackedStates[key] && typeof trackedStates[key].targets == 'object') {
                ...
                trackedStates[key].targets = [];
            }
            if (trackedStates[key].targets.length < 1) {
                delete trackedStates[key]
            }
        }
        ;
            ...
            devStates.states.forEach(state => {
                ...
                if (key) {
                    if (memberInfo.capabilities && memberInfo.capabilities.find(candidate => candidate == key)) {
                        if (member.epname && member.epname == state.epname) {
                            return;
                        }
                        UpdatableStates.push({
                            id: `${t.adapter.namespace}.${devId}.${state.id}`,
                            grpid: `${t.adapter.namespace}.${groupID}.${key}`
                        })
                    }
                }
            })
    ...
    async getGroupMemberValue(id) {
        if (!this.GroupData?.states[id]) {
            return undefined;
        }
        const GDI = this.GroupData.states[id];
    ...
    async syncGroups(changed, group_id) {
        ...
        if (group_id) {
            group_id.forEach(gid => numericGroupIdArray.push(Groups.extractGroupID(gid)));
        }
        ...
        for (const j in groups) {
            if (numericGroupIdArray.length > 0 && !numericGroupIdArray.includes(Number(j))) {
                continue;
            } // skip groups we didnt ask for
            ...
            if (groups.hasOwnProperty(j)) {
                ...
                if (mu) {
                    this.GroupData[id].memberupdate = mu.val;
                } else {
                    this.GroupData[id].memberupdate = false;
                }
                const su = await this.adapter.getStateAsync(`${this.adapter.namespace}.group_${j}.stateupdate`);
                if (su) {
                    this.GroupData[id].stateupdate = (typeof su.val == 'string' ? su.val : 'off');
                } else {
                    this.GroupData[id].stateupdate = 'off';
                }
                if (this.GroupData[id].stateupdate != 'off') {
                    this.anyGroupStateUpdate = true;
                }
