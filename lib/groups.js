...
class Groups {
    ...
    async onStateChanged(deviceId, model, stateModel, stateList, options, debugId) {
        ...
        else { // the states are device states
            for (const state of stateList) {
                const GroupDataKey =`group_${deviceId}`
                    ...
                    this.adapter.setObjectNotExists(id, {
                        ...
                        for (const stateInd in statesMapping.groupStates) {
                            ...
                            const common = {
                                ...
                                min: statedesc.min,
                                max: statedesc.max,
                                color: memberInfo.capabilities.find((candidate) => candidate == statedesc.id) ? null: '#888888',
                            };
