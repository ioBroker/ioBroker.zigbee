...
class Groups {
    ...
    async removeGroupMemberStateList(numericGroupID) {
        const groupID = `group_${numericGroupID}`;
        const gd = this.GroupData;

        const trackedStates = gd && gd.states ? gd.states : {};
        const t = this;
        this.warn(`removeGMSL GD is (1): ${JSON.stringify(this.GroupData)}`)
        if (this.GroupData.hasOwnProperty(groupID)) delete this.GroupData[groupID];
        ...
        this.warn(`removeGMSL GD is (2): ${JSON.stringify(this.GroupData)}`)

    }


    async rebuildGroupMemberStateList(numericGroupID, memberInfo) {
        this.warn(`rebuildGMSL (1) ${JSON.stringify(this.GroupData.states)}`)
        for (const key in this.GroupData) {
            if (key.startsWith('group'))
                this.warn(`group ${key} : ${JSON.stringify(this.GroupData[key].members)}, ${JSON.stringify(this.GroupData[key].states)}.`)
        }
        const groupID = `group_${numericGroupID}`;
...
*/
        ...
        for (const member of memberInfo.members) {
            ...
            devStates.states.forEach(state => {
                ...
                if (key) {
                    if (memberInfo.capabilities && memberInfo.capabilities.find(candidate => candidate.id == key)) {
        ...
        t.warn(`trackedStates were ${JSON.stringify(this.GroupData.states)}`)
        UpdatableStates.forEach(entry => {
            const ged = this.GroupData.states[entry.id]
            if (ged) {
                if (!ged.includes(entry.grpid))
                    ged.push(entry.grpid);
            }
            else
            {
                this.GroupData.states[entry.id] = [entry.grpid];
            }
        })
        t.warn(`trackedStates are ${JSON.stringify(this.GroupData.states)}`)
        this.warn(`rebuildGMSL (2) ${JSON.stringify(this.GroupData.states)}`)
        for (const key in this.GroupData) {
            if (key.startsWith('group'))
                this.warn(`group ${key} : ${JSON.stringify(this.GroupData[key].members)}, ${JSON.stringify(this.GroupData[key].states)}.`)
        }
    ...
    async syncGroups(numericGroupIdArray) {
        // get all group id's from the database and the respective names from the local overrides (if present)
        const groups = await this.getGroups();
        const chain = [];
        const usedGroupsIds = [];
        for (const j in groups) {
            if (numericGroupIdArray && !numericGroupIdArray.includes(Number(j))) continue; // skip groups we didnt ask for
            ...
            if (groups.hasOwnProperty(j)) {
                ...
                if (GroupMembersChanged) {
                    ...
                    this.warn('rebuild triggered');
                    this.rebuildGroupMemberStateList(j, memberInfo);
                    this.warn('post rebuild trigger');
                }
