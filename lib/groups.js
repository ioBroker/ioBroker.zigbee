            ...
            for (const epid in groups) {
              for (const gpid of groups[epid]) {
                ...
                if (gpidn < 0) {
                  ...
                  if (response && response.error) errors.push(response.error);
                  this.warn("remove dev from group Error: "+ JSON.stringify(response.error));
                }
                else if (gpidn > 0){
                  let response = await this.zbController.addDevToGroup(sysid,(gpidn), epid);
                  if (response && response.error) errors.push(response.error);
                  this.warn("add dev to group Error: "+ JSON.stringify(response.error));
                }
