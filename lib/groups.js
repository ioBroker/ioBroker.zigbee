...
class Groups {
    ...
    groupDevices(from, command, devGroups, callback) {
        this.warn(`groupDevices called with ${JSON.stringify(from)} ${JSON.stringify(command)} ${JSON.stringify(devGroups)} ${JSON.stringify(callback)}`)
        for (const j in devGroups) {
            if (devGroups.hasOwnProperty(j)) {
                ...
                const sysid = j.replace(this.adapter.namespace + '.', '0x');
