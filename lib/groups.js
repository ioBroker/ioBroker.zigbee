...
class Groups {
    ...
    async getGroups(obj) {
        this.warn('get groupes called with ' + JSON.stringify(obj));
        ...
        if (isEnable.val) {
            try {
                ...
                this.warn(`getGroups result: ${JSON.stringify(groups)} ( ${JSON.stringify(herdsmanGroups)})`);
    ...
    async renameGroup(from, command, message) {
        ...
        try {
            ...
            const members = this.zbController.getGroupMembersFromController(message.id);
            membercount = (members && members.length ? members.length : 0);
        } catch (e) {
        ...
        if (!group) {
            ...
            this.adapter.setObjectNotExists(id, {
                type: 'device',
                common: {name: (name ? name : `Group ${message.id}` ), type: 'group', icon:`img/group_${membercount}.png`},
                native: {id}
            }, () => {
                this.adapter.extendObject(id, {common: {name, type: 'group', icon:`img/group_${membercount}.png`}});
            ...
            this.stController.renameDevice(id, name)
        }
    }
