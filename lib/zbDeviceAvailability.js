            ...
            try {
                if (!this.pingCluster || !this.pingCluster.hasOwnProperty('id')) {
                    ...
                    if (has_elevated_debug) {
                        this.zigbee.emit('device_debug', { ID:debugID, data: { ID: device.ieeeAddr, flag:'PI', IO:false, payload: { method:'ZH Ping'}}, message:message});
                    }
                    ...
                    if (has_elevated_debug) {
                        this.zigbee.emit('device_debug', { ID:debugID, data: { ID: device.ieeeAddr, flag:'PI', IO:false, payload: { cluster:this.pingCluster.id, command:'read', zcl:zclData}}, message:message});
                    }
                    else this.debug(message)
