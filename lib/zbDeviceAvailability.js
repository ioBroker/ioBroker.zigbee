...
class DeviceAvailability extends BaseExtension {
    ...
    async handleInterval(device) {
        ...
        try {
            ...
            this.debug(`Successfully pinged ${ieeeAddr} ${modelID}`);
        } catch (error) {
            this.publishAvailability(device, false);
            this[level](`Failed to ping ${ieeeAddr} ${modelID}`);
        } finally {
        ...
        if (device && device.modelID) {
            ...
            if (mappedDevice) {
                ...
                try {
                    ...
                    const entity = this.zigbee.resolveEntity(device.ieeeAddr);
                    this.error(`Failed to read state of '${device.ieeeAddr}' after reconnect`);
                }
            }
        }
    }
