    ...
    async handleIntervalPingable(device, entity) {
        ...
        if (this.isPingable(device)) {
            ...
            try {
                ...
                this.warn(`Exception in readState of '${device.ieeeAddr}' - error : '${error}'`);
            ...
            try {
                ...
                {
                    this.warn(`Failed to ping ${ieeeAddr} ${device.modelID} with  ${error} for on ${pingCount} consecutive attempts`);
                    this.setTimerPingable(device, pingCount);
                    this.ping_counters[device.ieeeAddr] = pingCount;
                }
                else {
                    this.warn(`Stopping to ping ${ieeeAddr} ${device.modelID} due to ${error}  after ${pingCount} consecutive failed attempts`);
                }
            }
        }
    }
