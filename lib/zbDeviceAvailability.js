    ...
    async handleIntervalPingable(device, entity) {
        ...
        if (this.isPingable(device)) {
            ...
            try {
                ...
                {
                    this.warn(`Failed to ping ${ieeeAddr} ${device.modelID} for on ${pingCount} consecutive attempts`);
                    this.setTimerPingable(device, pingCount);
                    this.ping_counters[device.ieeeAddr] = pingCount;
                }
                else {
                    this.warn(`Stopping to ping ${ieeeAddr} ${device.modelID} for after ${pingCount} consecutive failed attempts`);
                }
            }
        }
    }
