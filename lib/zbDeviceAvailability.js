...
class DeviceAvailability extends BaseExtension {
    ...
    isPingable(device) {
        if (forcedPingable.find((d) => Object.prototype.hasOwnProperty.call(d, 'zigbeeModel') && d.zigbeeModel.includes(device.modelID))) {
            return true;
        }
    ...
    async onReconnect(device) {
        ...
        if (entity && entity.mapeped) {
            const used = [];
            try {
                for (const key of toZigbeeCandidates) {
                    const converter = entity.mapeped.toZigbee.find((tz) => tz.key.includes(key));
    ...
    publishAvailability(device, available) {
        const ieeeAddr = device.ieeeAddr;
        if (Object.prototype.hasOwnProperty.call(this.state, ieeeAddr) && !this.state[ieeeAddr] && available) {
    ...
    onZigbeeEvent(data) {
        ...
        if (this.isPingable(device)) {
            ...
            const online = Object.prototype.hasOwnProperty.call(this.state, device.ieeeAddr) && this.state[device.ieeeAddr];
