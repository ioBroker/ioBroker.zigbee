...
class DeviceAvailability {
    ...
    info(message, data) {
        this.controller.info(message, data);
    }

    error(message, data) {
        this.controller.error(message, data);
    }

    debug(message, data) {
        this.controller.debug(message, data);
    }
    ...
    async handleInterval(device) {
        ...
        const entity = this.controller.resolveEntity(device.ieeeAddr);
        try {
            await device.ping();
            this.publishAvailability(device, true);
            this.debug(`Successfully pinged '${entity.name}'`);
        } catch (error) {
            this.publishAvailability(device, false);
            this[level](`Failed to ping '${entity.name}'`);
        } finally {
    ...
    publishAvailability(device, available) {
        ...
        this.controller.emit('publish', ieeeAddr.substr(2), device.modelID, payload);
    }
