...
class DeviceAvailability extends BaseExtension {
    ...
    async stop() {
        this.isStarted = false;
        if (this.startDevicePingTimeout) {
            clearTimeout(this.startDevicePingTimeout);
            this.startDevicePingTimeout = null;
        }
        for (const ieeeAddr of Object.keys(this.timers)) {
            if (this.timers[ieeeAddr]) {
                clearTimeout(this.timers[ieeeAddr]);
                clearInterval(this.timers[ieeeAddr]);
            }
            delete this.timers[ieeeAddr];
        }
        this.startDevicePingQueue = [];
    }
        ...
        if (this.isPingable(device)) {
            ...
            try {
                if (!this.pingCluster || !this.pingCluster.hasOwnProperty('id')) {
                    ...
                    if (has_elevated_debug) {
                        this.zigbee.emit('device_debug', {
                            ID: debugID,
                            data: {
                                ID: device.ieeeAddr,
                                flag: 'PI',
                                IO: false,
                                states: [{id: 'ping', value: 'zh_default', payload: {method: 'default'}}]
                            },
                            message: message
                        });
                    } else {
                ...
                if (has_elevated_debug) {
                    ...
                    this.zigbee.emit('device_debug', {
                        ID: debugID,
                        data: {ID: device.ieeeAddr, flag: 'SUCCESS', IO: false},
                        message: message
                    });
                }
                ...
                if (error && error.message && error.message.includes('UNSUPPORTED_ATTRIBUTE')) {
                    ...
                    if (has_elevated_debug) {
                        this.zigbee.emit('device_debug', {
                            ID: debugID,
                            data: {ID: device.ieeeAddr, flag: 'SUCCESS', IO: false},
                            message: `Successfully pinged ${ieeeAddr} (${device.modelID}) in ${Date.now() - debugID} ms`
                        });
                    }
                    this.setTimerPingable(device, 1);
                    this.ping_counters[device.ieeeAddr].failed = 0;
                ...
                if (has_elevated_debug) {
                    this.zigbee.emit('device_debug', {
                        ID: debugID,
                        data: {ID: device.ieeeAddr, error: 'PIFAIL', IO: false},
                        message: `Failed to ping ${ieeeAddr} (${device.modelID}) after ${Date.now() - debugID} ms${error && error.message ? ` - ${error.message}` : ''}`
                    });
                }
                ...
                if (pingCount.failed++ <= this.max_ping) {
                    ...
                    if (has_elevated_debug) {
                        this.zigbee.emit('device_debug', {
                            ID: debugID,
                            data: {ID: device.ieeeAddr, error: 'PISTOP', IO: false},
                            message: msg
                        });
                    } else {
                        this.info(msg);
    ...
    async handleIntervalNotPingable(device) {
        ...
        if (ago > Hours25) {
            ...
            if (this.checkDebugDevice(device.ieeeAddr)) {
                this.warn(`ELEVATED: ${msg}`);
            } else {
                this.debug(msg);
            }
            return;
