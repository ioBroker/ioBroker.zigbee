...
class DeviceAvailability extends BaseExtension {
    ...
    setOptions(options) {
        ...
        this.startReadDelay = (typeof options.startReadDelay === 'number' && options.readAllAtStart) ? Math.max(500, Math.mim(10000, options.startReadDelay * 1000)) : 0;
        return true;
    }
    ...
    async onZigbeeStarted() {
        ...
        for (const device of clients) {
            if (this.isPingable(device)) {
                readables.push(this.zigbee.doDeviceQuery(device, Date.now(), false));
        ...
        if (this.startReadDelay > 0) {
            setTimeout(() => {
                Promise.all(readables);
            })
        }
    }
