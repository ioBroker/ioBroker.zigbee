...
class Binding {
    ...
    async start(zbController, stController) {
        ...
        this.bindingInfo = this.bindingInfo= await this.zbController.getBindings();
            ...
            if (typeof obj === 'object' && obj.command) {
                switch (obj.command) {
                    ...
                    case 'getHerdsmanBindings':
                        if (obj.message) {
                            this.bindingInfo = this.zbController.getBindings();
    ...
    */
        ...
        try {
            ...
            if (!found && type != 'unbind') {
                ...
                for (const clID of clusters) {
                    ...
                    if (targetValid) {
                        this.warn(`${type}ing cluster '${cluster}' from '${sourceName}' to '${targetName}'`);
                        try {
    ...
    async addBinding(from, command, params, callback) {
        try {
            ...
            await this.doBindUnbind('bind', params.s_address, params.s_ep, params.clusters.length > 0 ? params.cluster : ['all'], params.b_address, params.b_ep, params.allowAllClusters, (err) => {
                if (callback)callback(err ? {error: err} : {})
            });
        } catch (error) {
    ...
    async delBinding(from, command, params, callback) {
        try {
            ...
            await this.doBindUnbind('unbind', params.s_address, params.s_ep, ['all'], params.b_address, params.b_ep, params.allowAllClusters, (err) => {
                if (callback)callback(err ? {error: err} : {})
            });
        } catch (error) {
