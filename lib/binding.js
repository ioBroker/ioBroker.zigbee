    ...
    onMessage(obj) {
        if (typeof obj === "object" && obj.command) {
            switch (obj.command) {
                ...
                case 'editBinding':
                    if (obj && obj.message && typeof obj.message === 'object') {
                        this.editBinding(obj.from, obj.command, obj.callback);
                    }
                    break;
                case 'getBinding':
                    if (obj && obj.message && typeof obj.message === 'object') {
                        this.getBinding(obj.from, obj.command, obj.message, obj.callback);
                    }
                    break;
                case 'delBinding':
                    if (obj && obj.message && typeof obj.message === 'object') {
    ...
    async addBinding(from, command, params, callback) {
        try {
            ...
            if (!source || !target) {
                ...
                this.adapter.sendTo(from, command, {error: 'Devices not found'}, callback);
                return;
            }
            ...
            for (const cluster of clusters) {
                ...
                if (source.endpoint.supportsOutputCluster(cluster) && targetValid) {
                    ...
                    try {
                        ...
                        this.adapter.sendTo(from, command, {error: `Failed to ${type} cluster '${cluster}' from '${sourceName}' to ` +
                            `'${targetName}' (${error})`}, callback);
                    }
                }
            }
            if (!found) {
                this.debug(`No bind clusters`);
                this.adapter.sendTo(from, command, {error: `No bind clusters`}, callback);
            }
        } catch (error) {
